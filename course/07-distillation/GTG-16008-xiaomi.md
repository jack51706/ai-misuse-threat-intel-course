...
- **F6（思維鏈／系統提示套取）**：本案核心目的仍是蒸餾——把 Claude 的推理／能力套取去訓練自家 MiMo。機制較特殊：小米**先儲存自家 MiMo 使用者的真實工作階段（完整請求＋回應），事後離線批次重放給 Claude**，用真實使用者的請求分布當蒸餾查詢集，產出 SFT／RL 資料。全章 CoT 套取逐字原文在 p.145–146。— 證據等級 ★★☆（報告詳述「儲存後重放」的蒸餾手法）
- **F5（工具／記憶中介）**：重放常透過 **OpenClaw／OpenCode** 兩個 coding harness 執行，並經第三方模型路由服務流動——用工具／harness 當中介，把「真正的蒸餾意圖」從任何單一請求前移走。— 證據等級 ★☆☆
- **（誠實界線）** 「藉 MiMo-V2-Pro 免費試用期衝國際開發者用量再蒸餾」與經第三方路由匯流，屬**存取層（路徑 A）**的衝量與匿蹤，不是操作模型推理的 F 族。

**對地端 LLM 的意義**

小米案是「規模最小、隱蔽性最高」的一類：它不即時代答，而是**把真實使用者 session 存起來離線重放**——這對自架者有兩重意義。其一，若你的地端模型是別人的上游（被當 API 供人呼叫），你的回應與推理可能被**存檔後拿去蒸餾**，且離線發生、你的即時遙測看不到。其二，若你透過 harness／第三方路由使用外部模型，你的使用者請求（本案外洩了數百名使用者、十幾種語言的姓名、聯絡資訊、公司內部資料）可能被中介層留存重放。
...
5. **縱深多層＋存取層**：本案主力在存取層（免費試用衝量、proxy 匯流），地端要同時補存取層（帳號叢集、proxy 池偵測）與輸出層（不吐 CoT），不能只守一層。
