...
| 主張 | 證據等級 | 依據 |
|---|---|---|
| 核心手法族 **F6（思維鏈套取）**，鎖定 Opus 4.8 | 一手（報告明載） | p.150–151 CoT 擷取管線、273 帳號 |
| 把擷取的 CoT **回灌 Claude「清洗」**；770,609 過清洗器 | 一手 | p.151（6 月、10 天、訓練 GLM 泛用） |
| GLM 5.3 前用**公開漏洞資料集做 CTF 題**萃取／評測 cyber 能力、Opus 4.6 當評分者 | 一手（機制）＋推測（CTF 框架動機） | p.151；框架屬 F4 推測 |
| **安全套利**：試蒸餾 Fable 網路能力失敗 → 改打 Opus 4.6 與另一家美廠 | 一手 | p.151「expressly because they assessed the safeguards were weaker」 |
| 全章 CoT 套取逐字原文 | 一手逐字 | p.145–146（未逐句歸因至 Zhipu） |

**本案綜合證據等級：F6 ★★☆；CTF 框架（F4）★☆☆（框架動機屬推測）。**

### 1. 推測的操作序列（雙軌：通用蒸餾＋cyber 能力萃取）

**軌 A｜通用推理蒸餾（回灌清洗）**
1. **存取層鋪墊**：273 個詐欺帳號輪替、經 proxy 偽裝來源。
2. **套取 CoT**：以跨工作階段重放等手法，把 Opus 4.8 的推理軌跡記錄下來。
3. **回灌清洗（本案特色）**：把擷取到的原始 CoT **再送回 Claude**，要它幫忙**清洗**成乾淨、可訓練的格式（10 天內 770,609 次通過「CoT 清洗器」）——用老師模型幫你把偷來的老師推理整理成教材。
4. **訓練 GLM**：清洗後的資料訓練自家 GLM（含 GLM 5.3）。

**軌 B｜攻擊性 cyber 能力萃取（GLM 5.3 發布前）**
5. 把**公開漏洞資料集**改寫成 **CTF 挑戰題**，用來從 Claude 萃取／評測網路攻擊能力，並用 Opus 4.6 當**評分者**（LLM-as-judge）——把「攻擊性 cyber 能力的萃取」包裝成「安全競賽／研究」這類看似正當的框架（F4）。
6. **安全套利（路徑 D）**：原想蒸餾 **Fable** 的網路安全能力，因 Fable 防護已強化而**放棄**，改打防護較弱的 **Opus 4.6 與另一家美國模型**——明說是「因為評估這些的防護較弱」。

### 2. 為何這對模型的推論有效

- **為何回灌清洗有效**：清洗本身是一個看似中性的文字整理任務（去雜訊、統一格式）。分類器若只盯「請把你的推理給我」這類明語，看不出「請幫我整理這批文字」其實是在**把偷來的 CoT 精煉成訓練資料**——這是任務改寫（task laundering）的延伸。
- **為何 CTF 框架能誘出 cyber 能力**：把攻擊性請求包成「奪旗競賽題／可評分的安全研究」，落在雙重用途的模糊地帶；模型被訓練成願意協助合法安全研究，於是良性框架把請求推到「可協助」的一側（同 F4，呼應附錄 H 的授權框定原理）。
- **為何安全套利成立**：蒸餾**不會把安全防護一起帶走**。被強化了 cyber 能力、卻沒繼承任何 guardrail 的 GLM，本身就是風險源；而只要生態系裡有一個防護較弱的模型，攻擊者就往那裡去——**單一模型強化沒用，需要全生態同步抬高地板**。

### 3. 萃取管線與偵測切點（Mermaid）

```mermaid
flowchart TB
    subgraph HARV["軌 A：通用推理蒸餾 惡意所在"]
      S1["273 帳號輪替＋proxy 偽裝"]
      S2["跨 session 重放記錄 Opus 4.8 CoT"]
      S3["把擷取 CoT 回灌 Claude 清洗<br/>770609 次過清洗器"]
      S4["訓練 GLM 含 GLM 5.3"]
    end
    subgraph CYB["軌 B：cyber 能力萃取＋安全套利"]
      B1["公開漏洞資料集改寫為 CTF 題<br/>Opus 4.6 當評分者"]
      B2["試蒸餾 Fable 網路能力被防護擋下"]
      B3["改打防護較弱的 Opus 4.6 與另一家美廠"]
    end
    S1 --> S2 --> S3 --> S4
    B1 --> B2 --> B3
    D1["切點1：不回傳 CoT<br/>對回灌清洗型請求上偵測"] -.->|"斷清洗素材"| S3
    D2["切點2：輸出分類器獨立於 CTF／研究自稱用途再判"] -.->|"不因競賽題框架放行"| B1
    D3["切點3：雙重用途／攻擊性 cyber 能力降載<br/>判用途與下游而非表面框架"] -.->|"壓制能力誘出"| B1
    D4["切點4：最強模型不裸奔<br/>防護拉齊到最弱模型之上"] -.->|"堵安全套利"| B3
    D5["切點5：273 帳號同質萃取叢集關聯＋速率限制"] -.->|"抓群體規模"| S2
    style HARV fill:#ffe8e8
    style CYB fill:#ffe8d0
    style D1 fill:#e8ffe8
    style D2 fill:#e8ffe8
    style D3 fill:#e8ffe8
    style D4 fill:#e8ffe8
    style D5 fill:#e8ffe8
```

### 4. 對地端 LLM 的意義 ＋ 地端防護重點（對映四層）

**雙向性（先講最重要的）**：Zhipu 給自架者兩個警訊，而以下每一條防護同時是**保護你自架模型不被蒸餾**、以及**避免你的地端模型成為下游風險源**的做法。其一，被蒸餾走的不只通用能力，**還包括攻擊性 cyber 能力**，而蒸餾不帶走防護——一個能力被灌強、卻沒有 guardrail 的地端模型，本身就是威脅。其二，「安全防護套利」直接對映地端：**你若同時擺著強弱不一的模型，攻擊者永遠挑最弱那個下手**。
...
3. **④ 架構層（呼應本案套利）**：**最強模型不裸奔**——地端若部署多個模型，防護要拉齊到最弱那個之上，否則等於替攻擊者標好下手點；並認清蒸餾出去的能力不帶防護，下游生態是間接受害者。
4. **② 會話層**：273 帳號共用同一套萃取——對跨帳號同質的萃取行為做叢集關聯與速率限制（抵 F2／F6）。
