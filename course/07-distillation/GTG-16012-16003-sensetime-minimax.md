...
- **F6（思維鏈／系統提示套取）— 間接**：本案兩家都是蒸餾的**買方／收割者**，不自己跑萃取提示。SenseTime **向第三方資料商購買**別人收割好的 Claude 逐字稿（其中即含被萃取的推理）；MiniMax 用**空殼公司 proxy** 收割使用者與美國前沿模型的對話。也就是說，F6 式的 CoT 套取發生在**上游的轉售商生態**，這兩家只是消費端。全章 CoT 套取逐字原文在 p.145–146。— 證據等級 ★☆☆（就本案兩主體自身操作而言為間接）
- **（規避主力在存取層／供應鏈，路徑 A）** 本案真正的核心不是操作模型推理的 F 族，而是**供應鏈犯罪**：買逐字稿（SenseTime）、空殼 proxy 只賣美國模型不賣自家（MiniMax）。這正是路徑 A（存取層規避）的極致——連流量都不必自己產生，直接在資料黑市採購或用掩護門面收割。

**對地端 LLM 的意義**

這一案教自架者看見「看不見的那條路」：**就算沒有人直接對你的地端模型下萃取提示，你的輸出仍可能被中介的路由／轉售商留存、打包、轉賣**。反過來，若你為了省成本去採購「便宜的前沿模型存取」，背後可能是一個空殼 proxy——它一邊賣你存取、一邊收割你送進去的請求（本報告的 GTG-50021 假轉售商更會順手植入憑證竊取器）。偵測難點在於這些行為多半發生在**你的遙測盲區之外**。
