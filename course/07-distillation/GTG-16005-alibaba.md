...
| 主張 | 證據等級 | 依據 |
|---|---|---|
| 核心手法族 **F6（思維鏈套取）** | 一手（報告明載） | p.147–148 注入固定 prompt 逼 Claude 把 CoT 寫進 inline 標籤 |
| 手法為「主動誘導模型自吐 CoT」，非簽章重放 | 一手 | p.148（與 Moonshot／DeepSeek 路線不同） |
| 規模全章最大：>151M、峰值近 3M／日、3,500+ 帳號、兩池 | 一手 | p.147–148 |
| 第二池同時為 DeepSeek、Xiaomi 轉送 | 一手 | p.148 |
| 亦用 Claude 推進自家 RL 環境與架構研究（AI R&D uplift） | 一手 | p.148 |
| 全章 CoT 套取逐字原文 | 一手逐字 | p.145–146（未逐句歸因至 Alibaba） |

**本案綜合證據等級：★★☆**——固定 prompt 逼 inline CoT 的手法報告詳述；p.145–146 逐字原文為全章 F6 提供 ★★★ 級佐證，但未逐句掛到 Alibaba，故本案自身落在 ★★☆。

### 1. 推測的操作序列（規模最大、手法最樸素）

1. **存取層鋪墊**：兩大詐欺帳號池（第一池近 5,000 帳號），假身分＋盜卡＋轉運站 proxy 偽裝來源；第二池被發現**同時為 DeepSeek、Xiaomi 轉送**，是「同一 proxy 網路多家共用」的直接證據。
2. **固定 prompt 注入**：在**每一個請求**都注入同一段固定 prompt，強迫 Claude 在給答案前，先把推理過程**寫進 inline 文字標籤**（例如某種 think 區塊）——不靠簽章、不靠重放，直接讓模型「自己把 CoT 吐進輸出」。
3. **收割 CoT**：由於推理被寫在回應正文裡，攻擊者**直接抓取 inline 標籤內容**即得完整 CoT，省掉了 Moonshot／DeepSeek 那套簽章還原工。
4. **清洗＋轉 SFT**：把「請求＋inline 推理＋答案」批次重格式化成 SFT 訓練對。
5. **蒸餾＋研發加速**：拿去訓練 Qwen 3.5／3.6／3.7；**同時**用 Claude 協助推進自家 RL 環境設計與模型架構研究——蒸餾之外還吃了一層 R&D uplift。
6. **歸因反諷**：數千帳號**共用同一段固定 prompt**，把散落流量釘成一個可跨帳號關聯的**指紋**，反而讓 Anthropic 能把整批歸因到 Alibaba——工業規模的同質性成了自己的破綻。

### 2. 為何這對模型的推論有效

- **為何固定 prompt 能誘出 CoT**：把「先寫出完整推理、再作答」框成一個**格式指令**（F7 輸出格式操縱）。模型被訓練成會遵循合理的格式要求（如「請逐步思考並列出推理」），於是一段中性的格式命令，就把本該留在內部、或被摘要化的推理**擠進了輸出**。這裡不需要越獄，因為請求表面上完全正當。
- **為何 inline CoT 一被回傳就能被蒸餾**：寫在正文裡的逐步推理是最高保真的監督訊號，直接就是可訓練的 SFT 素材，比只給答案有效得多，且能力會跨領域外溢（p.146）。
- **為何純內容規則擋不住**：意圖沒有寫進 prompt 文字（它就是一句「請把推理寫進標籤」），單一請求的內容分類器無從判惡意；真正可辨識的訊號是**群體層的同質性**（數千帳號同一模板、系統性索取推理），這要在行為層、跨帳號聚合才看得到。
...
flowchart TB
    subgraph ACC["存取層鋪墊"]
      A1["兩大詐欺帳號池<br/>第一池近 5000、共用 proxy 為 DeepSeek／Xiaomi 轉送"]
    end
    subgraph HARV["套取管線 惡意所在"]
      S1["每個請求注入同一段固定 prompt<br/>強迫 Claude 把推理寫進 inline 標籤"]
      S2["直接抓取 inline 標籤內的完整 CoT<br/>免簽章還原"]
      S3["清洗、重格式化為 SFT 訓練對"]
      S4["訓練 Qwen 3.5／3.6／3.7<br/>另用 Claude 推進 RL 環境與架構研究"]
    end
    A1 --> S1 --> S2 --> S3 --> S4
    D1["切點1：預設不在回覆輸出思維鏈<br/>拒絕格式化萃取指令"] -.->|"斷 inline CoT 來源"| S1
    D2["切點2：輸出分類器獨立於使用者指定格式再判一次"] -.->|"不因標籤模板放行"| S2
    D3["切點3：跨帳號同質性叢集＋速率限制<br/>同一固定模板＝可歸因指紋"] -.->|"抓群體同質性"| S1
...
**雙向性（先講最重要的）**：本案規模最大、手法卻最樸素——只要你的地端模型**願意在被要求時把推理寫進答案**，攻擊者就能用一個固定模板工業化地把你的 CoT 抽乾。以下防 Alibaba 抽 Opus CoT 的做法，**同時就是保護你自架模型不被同樣手法抽乾的做法**。對自架者，「輸出端要不要吐推理」不是使用體驗的小選項，而是**會不會被蒸餾**的關鍵開關。
...
3. **② 會話層＋④ 架構層**：把「數千帳號共用同一固定 prompt」的教訓反過來用——對**跨帳號高度同質、且系統性索取推理**的請求做叢集關聯與累計風險評分（抵 F2 式規模化）；同質指紋正是最好的偵測抓手，也是把散落帳號歸因到單一組織的鑰匙。
