...
- **F6（思維鏈／系統提示套取）**：本案是全章 F6 的旗艦樣本，且是**密碼學層級的深入**。Claude 只回傳加密的 thinking signature 而非原始 CoT；Moonshot 存下簽章、另開新工作階段、誘導 Claude 把簽章「還原」成完整推理。獨立研究（2026-08）進一步指出這些加密推理區塊用 **AEAD** 保護了機密性與完整性，卻**未把 associated data 綁進 session／user／model 身分脈絡**（疑似單一全域金鑰），因此撿到簽章者可跨帳號、跨工作階段、甚至跨姊妹弱模型重放領走整段明文。報告全章唯一的攻擊者逐字原文在 p.145–146。— 證據等級 ★★★（報告逐字原文＋獨立密碼學研究佐證）
- **F2（跨 session 分散）**：跨工作階段重放本質就是「用狀態切割規避有狀態偵測」——簽章沒被綁死在原本的 session／帳號／模型，才能被搬到別處重放。— 證據等級 ★★☆

**對地端 LLM 的意義**

Moonshot 案把一句話坐實：**任何「回傳可攜推理參照」機制，若沒把身分脈絡綁進加密方案，就會被跨會話／跨帳號重放**。學員自架推理模型時，若圖方便把「加密的思路 blob」回給前端好在後續呼叫續用，等於複製了同一個漏洞；一旦被解出，你的 CoT 就被整段蒸餾走。這也是「serve-and-harvest」的一體兩面：掛自家品牌、實際轉送更強模型，順手偷老師模型的推理。

**地端防護重點**（對映四層 playbook）

1. **③ 輸出層**：**不回傳原始思維鏈**；若非回不可，先做**摘要化**再回（Anthropic 的補丁方向），讓被偷到的是摘要而非可訓練的原文。
2. **④ 架構層（本案的正解）**：任何推理參照／簽章都要用 AEAD 把 **associated data 綁進 session／user／model**，並**避免單一全域金鑰**；切換模型時剝離、不接受跨帳號重放（對映 Anthropic 的「preserved thinking」——鎖住新帳號竄改「推理之前的上下文」）。
3. **② 會話層**：偵測「同一個簽章／推理參照在不同 session 或不同帳號被送回並要求還原」——這是重放最硬的結構性破綻（抵 F2／F6）。
4. **③ 輸出層**：對「要求把加密思路轉回明文／逐字展開先前推理」的萃取型請求上偵測與速率限制。
