...
>
> 該專題歸納出五條規避路徑（存取層規避、內容層繞過、設計外、模型選擇、部署後不可收回）。**本期 GTIG 報告為其中三條提供了跨廠商的獨立佐證：**
>
> - **路徑 A（存取層規避）**：One API／New API 金鑰黑市、Xanthorox 的越獄轉售。
> - **路徑 B（內容層繞過）**：APT31 的「我是資安研究員」。
> - **路徑 C（設計外）**：HONESTCUE 的無害提示。
>
> **本課程的柵欄專題原本完全建立在 Anthropic 的資料上。有了本期，它的前三條路徑都變成雙來源支持。這是本份教材對整套課程最實質的貢獻。**
---
...
**必須誠實告訴學員：本期的每一個平台側主張（誰用 Gemini 做了什麼、蒸餾規模、處置成效）都只有 GTIG 一方的說法，且無法被外部複驗。**

唯一例外是**惡意程式檢體**（HONESTCUE、COINBAIT、ATOMIC）：它們有 VirusTotal 上的樣本，**任何人取得樣本後都可以獨立驗證「這支程式是不是真的會呼叫 Gemini API 生成 C# 程式碼」**。這是本期唯一可外部複驗的技術主張。
...
flowchart TB
    ...
    A --> S3["**單一來源，完全無法外部複驗**<br/>10 萬筆提示的蒸餾行動<br/>各 APT 的 Gemini 使用細節<br/>『即時辨識並降低風險』<br/>『尚未出現突破性能力』<br/>UNC6418 的平台到野外閉環"]
    style S1 fill:#e8ffe8
    style S2 fill:#fff4e0
    style S3 fill:#ffe8e8
```

> **教學重點：媒體報導集中在 S3 那一格（「國家駭客用 Gemini」），而那一格恰好完全無法查證。** 這與 GTIG 2026-05 那一期的結論完全一致：**新聞價值與證據強度經常呈反比。** 這個規律出現兩次，就可以當成通則教。
