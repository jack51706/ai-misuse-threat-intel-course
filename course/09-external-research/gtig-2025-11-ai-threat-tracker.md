...
本課程 `../shared/02-claude-safeguards-and-bypass-paths.html` 把規避路徑分成五條。GTIG 這兩個案例**完整落在「路徑 B：內容層繞過」**，而且是其中技術門檻最低的一種。建議在課堂上把三段原文並排：
...
- 把兩段提示發給學員，請他們**寫出對應的 YARA 規則字串**。這是一個很好的練習，因為它逼學員思考：要抓整句話（容易被改寫繞過）還是抓片語（誤報高）？**推薦答案是抓 `without markdown` 這類「LLM 整合特有」的片語，因為它是功能必需的，攻擊者改掉它會破壞自己的解析。**
...
告警等級：極高
誤報來源：極少。開發者的 AI CLI 幾乎只會由互動式 shell 啟動
...
| 二 | **提示字串的 YARA 設計** | 30 分 | Figure 2 與 3 的提示原文（第 6.2 節） | 分組設計偵測字串，同時評估誤報風險與被繞過的難度。引導出「抓功能必需的片語（`without markdown`）優於抓完整句子」，並討論提示被加密儲存後規則的壽命 |
