...
> **偵測假說［分析，未實測］**：可研究服務帳號建立、AI 服務啟用、配額提升與公開存取設定的時間關聯。「24 小時內三項以上」只是待調校的示意門檻，不是已驗證的低誤報規則。先確認 GCP Cloud Audit Logs 所需事件及欄位是否存在，加入已核准部署、CI/CD、自助開發與正常擴容等反例，再測試誤報與漏報。AWS／Azure 只能作類似行為研究，須另做服務、事件與欄位映射，不能直接搬用 GCP 規則。

### 3.9 UNC6508 與「鎖定專有 AI 智財」：補上本期第二大趨勢的專節（2026-09-15 深化）

GTIG 五大 Q2 趨勢的第二條「Targeting Proprietary AI IP（鎖定專有 AI 智財）」在正文有一整章（`Threat Actors Targeting Proprietary AI Research and Models`），但本教材先前只把 UNC6508 放進 3.1 總表與 4.2、5.4 三處，沒有像其他四條趨勢那樣給它一個專節。本節補上，並把 5.4 節已列的偵測邏輯前移到這裡的機制層。

**趨勢輪廓**［GTIG］：對手竊取的標的從「用 AI」升級為「AI 本身」，包括專有模型權重、原始碼、prompts、skills 與研究資料。受害者**不限於 AI 實驗室**（還有政府、軍事、醫療、媒體娛樂），竊取者也**不限於間諜組織**（資料竊取勒索集團同樣下手）。

**UNC6508 的關鍵手法（本節重點）**［GTIG，3.1 總表］：一個 PRC 關聯間諜群，在**竊得專有 AI 研究後**做了一件偵測工程上極重要的事，**在受害者自己的環境內部署本機開源權重模型（local open-weight model），用來規避商用 API 的監控，同時盜用受害者的算力**。這一步把「攻擊者對 AI 的使用」，從一個**平台看得見**的地方（商用 API 遙測），搬到了一個**平台看不見、受害者也未必看得見**的地方（受害環境內自架的推論服務）。
...
flowchart TB
    subgraph V["受害者環境內"]
        S1["竊得的專有模型權重／原始碼／研究資料"]
        S2["自架本機推論服務<br/>vllm／ollama／llama.cpp 類"]
        S3["盜用受害者 GPU 算力"]
        S1 --> S2
        S3 --> S2
    end
    subgraph CAPI["商用 API 路徑：對照組"]
        A1["呼叫 Gemini／Claude 等商用 API"]
        A2["供應商端遙測：帳號、prompt、計費、濫用分類器"]
        A1 --> A2
    end
    A2 --> SEEN["AI 供應商看得見：意圖、prompt、失敗嘗試"]
    S2 --> BLIND["AI 供應商看不見：本機推論不經任何商用 API"]
    BLIND --> GAP["可觀測性落差：偵測責任整個落回受害者端"]
```

［分析］**這是本期最該被獨立拉出來講的一個防禦轉折。** 商用 API 路徑之所以「可治理」，是因為每一次呼叫都留下供應商端的遙測（帳號、prompt、計費、濫用分類器）；UNC6508 的作法直接繞過這一整層：把偷來的權重灌進受害者環境內的開源推論引擎，模型在受害者的 GPU 上跑，**沒有任何一筆流量經過商用 API，供應商的濫用偵測完全失明**。結果是**偵測責任整個從 AI 供應商端，落回受害者的端點與雲端團隊身上**。

**偵測構想（把 5.4 節的邏輯前移到機制層）**：既然商用 API 遙測看不到，唯一的偵測面就在受害環境內部：

- **非預期的本機推論服務行程**：`vllm`、`ollama`、`llama.cpp`、`text-generation-inference` 等推論引擎，出現在不該有它們的伺服器上；監聽本機推論埠的非預期服務。
- **對 Hugging Face 等模型倉庫的大量下載**：短時間拉取數十 GB 的權重檔（`.safetensors`、`.gguf`、`.bin`），尤其是從不做 ML 的伺服器發起。
- **GPU 使用異常**：非 ML 業務的主機出現持續高 GPU 佔用，或 GPU 記憶體被大模型長期佔滿。
- **配合資料側訊號**：model registry、prompt 庫、skill 庫、研究資料倉的異常大量讀取（見 5.4 節，把這些當皇冠寶石分級保護）。

**與本課程的對照**：Anthropic 的 **GTG-50020**（俄語財務動機行為者，目標是取得**未發布的 Claude 模型權重**）與 UNC6508 是**同一個目標類別、不同動機**：一個是國家間諜、一個是財務動機犯罪，但兩家都看到「AI 智財（尤其是模型權重）成為第一線竊取標的」。**同一塊皇冠寶石，兩種掠奪者。** UNC6508 更進一步示範了「偷到權重之後怎麼用得不被抓」，補上了 GTG-50020 沒有的下半段。對應教材：[`../01-cyber/GTG-50020-ai-supply-chain.html`](../01-cyber/GTG-50020-ai-supply-chain.html)。

### 3.10 UNC5792：本期唯一的監控型行為者（2026-09-15 深化）

UNC5792（俄羅斯）是本期唯一的**監控／情報蒐集**型行為者，先前只在 3.1 總表與 4.2 節各佔一格，本節補一段機制說明。GTIG 記載其把 AI 模型接進一個 Telegram 監控機器人：機器人**排程抓取**目標頻道的訊息，交給 LLM 逐則**分流判定「可疑或中性」**，再把判定結果彙整成**結構化情報報告**。這條管線的重點不在技術突破，而在**把過去需要人力盯訊息、做初篩、寫報告的監控工作，壓縮成一個排程加一個模型呼叫**，讓小團隊也能規模化監看大量頻道。

```mermaid
flowchart LR
    TG["Telegram 目標頻道"] --> SCH["排程抓取<br/>bot 定時輪詢訊息"]
    SCH --> LLM["LLM 逐則分流<br/>判定 可疑 或 中性"]
    LLM --> REP["彙整成結構化情報報告"]
```

**偵測構想**：UNC5792 的可觀測面主要在**平台側與帳號側**，不在受害端（被監看者通常不知情）。(1) 對 Telegram 一類平台：偵測**以機械式節奏、跨大量頻道系統性拉取歷史訊息**的 bot 帳號，正常使用者不會定時全頻道輪詢；(2) 對 AI 供應商：偵測**同一帳號把大批第三方頻道訊息貼進來、要求逐則二分類並產出情報格式摘要**的請求分布，這種「原始訊息流入、結構化情報流出」的形狀本身就是監控管線的指紋；(3) 一般化訊號：把「大量短輸入 → 固定二分類標籤 → 情報報告模板」的請求樣態，當成監控用途的高風險組合。

**與本課程的對照**：UNC5792 與 Anthropic 的 [`../03-surveillance/GTG-14022-public-opinion-monitoring-taiwan.html`](../03-surveillance/GTG-14022-public-opinion-monitoring-taiwan.html)（中國輿情監控，含針對台灣政治人物）、[`../03-surveillance/GTG-50027-mali-mass-interception.html`](../03-surveillance/GTG-50027-mali-mass-interception.html)（馬利全民攔截）是**同型不同國**：三者都是「用 AI 把原始訊息流轉成結構化情報報告」，只是換了平台（Telegram／社群／電信攔截）與國別（俄／中／馬利）。**這是一個跨國、跨平台都成立的通用監控型態**，教學上可以三案並排講，讓學員看到 AI 對監控的 uplift 都是「取代稀缺的分析人力」這個共同結構。
...
| **UNC6508**（PRC，竊取專有 AI 研究，在受害環境內架本機開源權重模型，機制見 3.9 節） | [`GTG-50020`](../01-cyber/GTG-50020-ai-supply-chain.html)（俄語財務動機行為者，目標是未發布的 Claude 模型權重） | 中，同一目標類別、不同動機 | 兩家都看到「AI 智財成為竊取目標」。**動機分歧值得教**：Anthropic 那個是財務動機犯罪，GTIG 這個是國家間諜。同一塊皇冠寶石，兩種掠奪者 |
...
| **UNC5792**（俄羅斯，Telegram 頻道 AI 監控機器人，機制見 3.10 節） | [`GTG-14022`](../03-surveillance/GTG-14022-public-opinion-monitoring-taiwan.html)（中國輿情監控）、[`GTG-50027`](../03-surveillance/GTG-50027-mali-mass-interception.html)（馬利全民攔截） | 中，同型不同國 | 三者都是「用 AI 把原始訊息流轉成結構化情報報告」。**這是跨國、跨平台都成立的通用型態**，教學上可以並排講 |
...
### 5.5 給 SOC 的三項待驗證防禦建議［分析］

以下是可優先評估的控制方向，不代表投報率排名。實施成本與效果取決於既有日誌、權限、端點覆蓋及人工作業；本教材沒有量測誤報或收益。

1. **盤點 AI 工具與 CI/CD 設定的完整性監控。** 依組織實際使用工具確認敏感設定路徑、正常更新方式與負責人，再評估 FIM。路徑變更本身不是惡意，需結合來源、內容與批准記錄。
2. **把 CI/CD 的 workflow 日誌即時外送到 SIEM，不要只留在平台上。** UNC6780 會用 API 刪掉執行紀錄。日誌一旦只存在於被攻陷的平台上，就等於沒有日誌。
3. **讓掃描器的拒答或分析失敗進入可追蹤的處理流程。** 記錄狀態、原因與未完成範圍，再決定人工審查或替代分析；不要自動把拒答當成發現惡意，也不稱它為唯一偵測方式。
...
這些圖的資料性質不同：有些是來源展示的擷取畫面，有些是調查者重建。截圖的來源、完整性與脈絡仍需檢查，重建圖的依據與省略也需揭露；不能只按圖的樣式判定哪家的事件證據較可靠。
...
| 雲端行為 | `Cloud Quotas API` 配額提升請求、GPU／大型執行個體、公用 IAM 綁定、AI 服務啟用 | ［GTIG Table 11］ | 可研究的行為關聯素材，**非已驗證偵測規則**；需要事件欄位、基線與合法部署反例，見第 3.8 節 |
...
把上表三類素材串接成以下**獵捕假說**。它尚未在實際環境驗證，不能預先保證低誤報；需先確認每一步可取得的事件與正常活動：
...
> **對照 Anthropic**：本課[安全防護專題](../_shared/02-claude-safeguards-and-bypass-paths.html)以五個分析視角整理控制限制。可比較兩家實際揭露的拒絕、漏判與處置內容；GTIG 本期未交代某些細節，不代表沒有發生，也不足以推定其完整能力或對整份報告作單一價值排名。
...
**題五（台灣在地）**：本教材所列三篇繁中報導主要轉述 GTIG，未在這些已列材料中辨識到本地實測。這個樣本不足以評價整個台灣媒體或研究社群。若要提出本地風險分析，還需要哪些資產、事件或訪談資料？
...
若組織使用 npm、PyPI 或報告提到的套件，應先盤點版本、依賴更新方式與 CI/CD 信任邊界。本教材沒有台灣產業採用率或治理覆蓋率的調查，不能以報告的全球規模數字推定台灣曝險比例。

**具體建議**：以第 5.5 節作盤點起點，依日誌可得性、現有控制與試行結果決定優先順序，不預設某一項在所有環境成本最低。

**（2）AI 編碼工具的普及率與治理落差。**

對採用 AI 編碼工具的組織，應確認設定、工具伺服器、權限與更新來源是否納入治理。本文沒有台灣企業的代表性調查，不能宣稱多數企業都沒有審核，或特定手法會在「典型台灣環境」暢行無阻。
...
雲端資源遭濫用可能造成額外帳單與服務風險，實際損失取決於資源、費率、持續時間及限制。本教材沒有台灣企業帳單監控的調查；第 3.8 節關聯規則應先確認資料、測試與調校，再討論部署。
...
第 9.2 節的已列媒體樣本可用來比較「轉述來源」與「新增本地證據」。未在這些文章找到訪談，不代表台灣整體沒有研究；可把資產盤點、具名事件與利害關係人訪談列作後續研究題目。
