...
［分析］**這三類與本課程 `_shared/02-claude-safeguards-and-bypass-paths.md` 的「路徑 B：內容層繞過」完全對應**（重新提示、角色扮演、任務拆分、良性框架）。兩份文件各自獨立整理出幾乎相同的分類，這是一個**很強的收斂訊號**：這些手法不是某一家模型特有的弱點，而是**當前 LLM 安全訓練的共同結構特性**。

但也要注意本課程那份教材的核心提醒：**越獄只是五條規避路徑中的一條，而且不是最主要的一條。** Microsoft 這篇把越獄當成主要分析類別，是因為它站在「攻擊者如何取得模型協助」的角度看；Anthropic 那份報告能看到的是更完整的圖像，包括存取層規避（偷帳號、假轉售商）、設計外（分類器不涵蓋）、模型選擇（降載到防護較弱的模型）、部署後不可收回。**Microsoft 這一篇在這個維度上是殘缺的，因為它不擁有模型平台的存取層遙測。**
...
對應教材：[Claude 柵欄與五條規避路徑](../shared/02-claude-safeguards-and-bypass-paths.html)。
...
4. **越獄只是冰山一角。** Microsoft 把越獄當主要類別，是因為它看不到存取層。對照本課程的五條規避路徑，會發現越獄（路徑 B）只是其中一條，而且不是最主要的。
