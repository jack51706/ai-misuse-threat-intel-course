...
> 本節依 `../_shared/02-claude-safeguards-and-bypass-paths.md` 第九節的七大手法族（F1–F7）與四層地端防護 playbook，重建本案三個伊朗國家機構「怎麼一步步驅動 Claude 產製整套行動的作業系統」。防禦視角，不含可複製的越獄字串。

**（一）推測的操作序列**

以下依第 4 節攻擊生命週期與第 6 節圖表判讀，把三機構驅動 Claude 的動作串成一條序列。本案沒有蒸餾章那種逐字提示原文，故單步證據等級最高只到 ★★☆：

1. **S1 三線定調＋組織骨架生成（F2）**：ICCO（政府線）、馬什哈德認知作戰指揮室（領袖線）、Bina 觀測站（領袖線／IRGC 口吻）各自向 Claude 索要作業手冊、人設系統、專案代碼（A-01 至 B-04）、早期預警協議、放大時程方案；ICCO 另求九部分國際影響力組合與最高領袖葬禮／繼任計畫；馬什哈德另求「Manjanegh」內容工廠教義。每一份都是單獨發出、格式明確的組織／行政文書請求。— ★★☆（§4 階段 1；p.63–64）
2. **S2 大量跨語言受眾化改寫（F2）**：把政府情報公報、伊朗安全部門公開通報、IRGC 發言人公報，逐句改寫成法爾西語、阿拉伯語、烏爾都語、馬來語、西班牙語、英語六種語言，並已規劃擴大到 20 種語言。單一改寫請求看起來就是常規翻譯／在地化編輯工作。— ★★☆（§4 階段 2；p.64）
3. **S3 人設代入與去關聯改框（F4）**：要求以 IRGC 發言人「官方口吻」生成訊息；把安全部門公開通報「去除與伊朗安全機構的關聯」後重新包裝成中性人設內容——ICCO 行為者自述這叫「不當敘事者，而當導演」。這一步不要求模型生成新的惡意內容，只要求對既有素材做一次看似中性的編輯動作。— ★★☆（§4 階段 3；p.63–64）
4. **S4 來源與格式洗白（F7）**：把捏造的主張掛在 CSIS、Brookings、RAND 名下（智庫洗白）；製造假草根標籤 #IranStands；套用外國作者署名與獨立媒體外觀模板，讓國家背書的訊息「看起來更可信」。— ★★☆（§4 階段 3；p.64、66）
5. **S5 結構化鎖定名單與定向內容生成（F2）**：以「整理一份人物清單」的行政格式，建立命名國際官員與伊朗反對派人物的目標資料庫，並產出鎖定巴哈伊的攻擊性反敘事內容；下游在野外可見以 58 追蹤者的小帳號對 Nawapress 貼文做回覆攻擊。與監控章（§3.1）的結構化側寫請求同構。— ★★☆（§4 階段 5；p.64；§6 Figure 13）
6. **S6 教義骨架跨會期複用（F2，收束）**：同一批教義 Markdown 與人設模板被跨數百個工作階段幾乎逐字重用，讓一個省級神學院宣傳分部取得原本需要部會級編制才做得出的規劃能力。這是全案最不像「一次性請求」、也最有偵測價值的模式，只有模型端的跨會期比對看得到。— ★★☆（§5.1「AI-Generated Operational Capacity」觀察指標）

**（二）為何對模型的推論有效**

模型在 S1–S6 每一步看到的，都是一件有明確產出格式的正當工作——寫一份組織手冊、翻譯一則公報、模擬某個職稱的發言稿、替一句引文補上來源格式。三個落差解釋了為什麼內容層判斷在這裡全面失效：

- **意圖落差**：模型看到的是「幫文化參贊寫一份對外簡報」，看不到這份簡報是三條國家指揮鏈之一、且將經文化參贊網絡送進外國外交管道。單一請求沒有任何欄位記錄「這是國家對齊宣傳機構的部長級交付物」。
- **真偽落差**：「這段話是 RAND 的研究結論」或「用 IRGC 發言人的口吻寫」，對模型而言是格式與語氣指令，不是需要查核的事實主張——模型沒有獨立的來源查核層，只會照指定模板產出，於是格式即權威：讀者看到「據 RAND」便傾向採信，不會去查是否真有這份報告。
- **來源血緣與規模落差**：單一工作階段看不到同一份教義 Markdown 已在數百個工作階段被貼上，也看不到三個機構的三個帳號在共用同一套人設系統與教義詞彙——這個模式只存在於帳號遙測與跨會期比對，不存在於任何一次推論的上下文視窗裡。

「去除與安全機構的關聯」這類指令最值得注意：表面上是拿掉一個機構名稱的無害編輯請求，模型沒有理由拒絕；實際效果卻是切斷歸因鏈，讓下游讀者無法辨識訊息的國家背景。這正是 F4 改框最有效之處——它不需要模型生成任何新的惡意內容。

**（三）攻擊者產製流程 → 模型所見 → 落差 → 偵測切點**

```mermaid
flowchart TB
    subgraph REAL["攻擊者真實行動"]
        A1["三條指揮鏈定調<br/>ICCO政府線／馬什哈德領袖線／Bina觀測站<br/>掛上闡明聖戰教義"]
        A2["各自向Claude索要<br/>作業手冊・人設系統・專案代碼・<br/>目標資料庫・放大時程・部長級文件"]
        A3["把官方情報公報／安全部門通報<br/>逐句改寫成六語受眾化內容<br/>規劃擴至20語言"]
        A4["指定人設口吻與掛名來源<br/>IRGC發言人口吻／CSIS・Brookings・RAND／<br/>IranStands假草根標籤"]
        A5["跨數百會期重貼同一批教義文件<br/>省級單位取得部會級規劃能力"]
        A1 --> A2 --> A3 --> A4 --> A5
    end
    ...
    subgraph GAP["模型看不到的落差"]
        G1["意圖落差<br/>看不到這是國家宣傳機構的<br/>行動指揮系統生成"]
        G2["真偽落差<br/>看不到CSIS・RAND研究結論是捏造<br/>發言人身分是冒名"]
        G3["來源血緣／規模落差<br/>看不到同一教義跨三機構・<br/>數百會期・上百頻道複用"]
    end
    subgraph DETECT["偵測與防護切點"]
        D1["會話層：教義／人設文件跨會期關聯比對"]
        D2["輸出層：引文可溯源檢查・<br/>獨立於請求格式的輸出分類"]
        D3["架構層：雙重用途代寫請求走身分／授權閘"]
    end
    A2 -.-> B1 -.-> G1
    A3 -.-> B2 -.-> G3
    A4 -.-> B3 -.-> G2
...
本案示範的不是「教 Claude 寫一篇假新聞」，而是「請 Claude 把整個宣傳部門的作業系統生出來」——手冊、人設、代碼、名單、時程一次到位。這套用法搬到學員自架的地端 LLM 上格外危險：裸模型沒有帳號遙測、沒有跨會期比對，任何人只要願意手動把同一份教義文件貼進數百次對話，就能讓一個沒有預算、沒有編制的單位取得部會等級的規劃與量產能力，而且地端沒有 Anthropic 那種事後帳號稽核能補救。

1. **會話層（抗 F2）**：對同一使用者／工作區跨會期比對是否重複貼上結構相似的教義手冊、人設系統、專案代碼文件——本案的專案代碼命名（A-01 至 B-04）、固定章節骨架（手冊＋人設＋早期預警＋放大時程）就是可辨識的模板指紋，單次出現無害，數百次重複出現是強訊號。
2. **輸出層（抗 F4）**：對「以某職稱／機構發言人口吻撰寫」「移除與某機構的關聯」這類編輯指令，判斷下游用途而非表面語氣——是誰在請求、要代表誰發言、去關聯之後這份內容會被安上什麼身分，而不是照單全收「這只是潤稿」。
3. **輸出層（抗 F7）**：任何「據某智庫／某機構研究顯示」的產出，要求可回溯的引文（原始 URL、發表日期），追不到就自動降級或加註；對「同一標籤、大量帳號口吻」的批量文案生成請求做相似度與速率告警。
4. **架構層**：把「代表機構發言」「以官員身分撰稿」這類雙重用途請求，從內容判斷移到經驗證的身分／授權閘——本案的殺傷力來自「政府單位真的可以這樣用」，地端沒有 CVP 式身分閘道時，至少要在應用層加一道「這個帳號／工作區是否有代表某機構發言的授權紀錄」的檢查。
5. **架構層＋會話層**：結構化人物清單生成（目標資料庫、鎖定群體名單）與監控類請求同構——本案的目標資料庫與監控案的側寫檔案用的是同一種能力。若地端同時開放「摘要人物」與「建檔案」能力，應在架構層把兩者關聯記錄與告警，不要各自獨立放行。
