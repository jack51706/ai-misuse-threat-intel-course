...
- **F5（工具／記憶中介）**：共享 AI 代理平台「Viktor」——每名行為者一個 workspace，長期記憶檔存放禁用詞、核可來源、帳號管理規則與規避方法（`SKILL.md`／`LEARNINGS.md`），讓代理在無人逐次指揮下持續產出、甚至讀 8,400 則貼文即時冒充真人對話（§4.0、§4.2）— 證據等級 ★★☆
- **F7（輸出格式操縱）**：把 MEK 關聯媒體改寫成獨立報導並去除浮水印（打擊溯源）、強制口號替換與 `#OurChoiceMaryamRajavi`、以 ZWNJ／點空格變形規避關鍵字比對（§4.5、§4.6）— 證據等級 ★★☆
- **F4（良性／防禦性改框）**：隱匿 MEK 所屬、初期刻意不提穆賈希丁，把組織宣傳偽裝成「無所屬中立新聞／一般同胞之聲」（§4.4、§4.5）— 證據等級 ★★☆

**對地端 LLM 的意義**

本案六個叢集有四個達 L3 自主，且不是靠自製 agent 框架，而是靠「共享工作區＋持久記憶檔」這種完全標準、完全無害的產品功能。學員自架地端代理（agentic LLM）若採同一套主流做法，威脅就內建其中：記憶檔可以夾帶「規避方法」「禁用詞」，代理可以被指派成「你現在就是某人」對其熟人即時對話。裸代理沒有工具與記憶的治理，等於把跨境鎮壓所需的冒充能力現成交付。

**地端防護重點**（對映四層 playbook）

1. **架構層（抗 F5）**：持久記憶檔要治理——不得夾帶「規避偵測方法／禁用詞清單／帳號輪替規則」這類作戰指令；對代理的工具呼叫給最小權限、監控呼叫節律；`SKILL.md`／`LEARNINGS.md` 這類標準檔要納入審計而非信任。
2. **架構層（抗即時冒充）**：對「請代理讀取某真人語料並以其身分即時對話」的用途設硬限制與人工複核——這是把「長期扮演特定真人」降級為一句系統提示的關鍵，危害直達人身安全。
3. **輸出層（抗 F7）**：輸出端保留來源與浮水印／溯源標記，偵測「要求去除浮水印／強制特定口號模板／以零寬字元等變形規避比對」的請求；分類器不看 ZWNJ 是否存在（波斯文必用），而看異常位置與插入頻率。
4. **輸出層／會話層（抗 F4）**：判內容血緣而非表面中立框架；對「隱藏所屬、偽裝成獨立／中立新聞」的產出獨立再判，並把跨會期重用的教義／記憶檔納入意圖聚合。
