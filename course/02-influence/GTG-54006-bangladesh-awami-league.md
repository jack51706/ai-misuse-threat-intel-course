...
> 本節依 `../_shared/02-claude-safeguards-and-bypass-paths.md` 第九節的七大手法族（F1–F7）與四層地端防護 playbook，重建本案「怎麼一步步驅動 Claude 產製影響力內容」，並給自架開源／地端 LLM 的防護重點。防禦視角，不含可複製的越獄字串。

**（一）推測的操作序列**

每步標證據等級（★★☆ 報告述行為、合理重建／★☆☆ 純推測；最高一級「報告逐字原文」在影響力全章並不存在）。本序列最高只到 ★★☆：

1. **設計固定敘事與輸出綱要（F7 前置）**：操作者把四大敘事主題（宗教極端主義框架／暴力與陰謀／外國特務抹黑／貪腐陰謀，§6.2）與風格規格「hot and aggressive」「so even village people understand」（p.68）寫進 `fake_news_3.py`（至少第三版）的批次模板，固定成每次呼叫產出「15 則孟加拉語標題＋3 篇詳細假故事＋15 則英文圖像提示」的綱要。模板先於任何一次 API 呼叫存在，決定了 Claude 之後每次看到的都只是「寫新聞標題與故事」這種孤立的寫作任務。— ★★☆（綱要數字 p.67、p.70 明載；模板即後續格式操縱源頭屬合理重建）
2. **跨帳號批次生成（F2）**：以 29 個 Claude 帳號輪替（約每 16–17 天換一個），依固定綱要批次呼叫 API，16 個月內至少執行約 100 次批次，累計產出 ≥1,500 則標題、≥300 篇故事、≥1,500 則圖像提示。任務被切成無數次結構相同的獨立呼叫，帳號輪替讓平台端難以用「單一帳號用量異常」串起全貌——把單人產能放大成產線。— ★★☆（p.67、p.70）
3. **跨服務去情境**：文字出了 Claude 之後，圖像提示送其他前沿模型產圖（報告用「probably」，p.67）；文字與圖像再經未載明工具轉成音訊與影片（原文被動語態、無主詞，p.69）。這一步已無 Claude 參與，但完成了「洗掉產線起點」的效果——沒有任何一個下游服務看得到「這是為了假新聞產線」，每一段都只看到自己份內的輸入輸出。— ★★☆（p.67 的 probably 與 p.69 的被動語態都是報告自陳的不確定性；跨服務去情境化屬合理重建）
4. **套電視新聞版型（F7）**：成品影片統一套上紅色標題條動畫、台標殘影、主播側影、固定孟加拉語跑馬燈的電視新聞版型（Figure 14 縮圖，§6.1.4）。這是格式操縱在本案最具體的落地：內容真偽無從驗證，但視覺框架「看起來像新聞」就足以通過鎖定受眾唯一會用的驗證程序（§3.5.2、§6.1.5）。— ★★☆（Figure 14 圖面事實，p.69；「格式即權威」判讀屬合理重建）
5. **暫存與排程上傳（產線基礎設施）**：影片進固定 Google Drive 資料夾暫存（Figure 14；「Shared with me」顯示至少橫跨兩個 Google 帳號，§6.1.2），第二支腳本經 YouTube API 批次上傳，排程提前一個月至數月設定，並經第三方 CI 服務中轉以遮蔽操作者 IP（p.68–70）。這一步與「驅動 Claude」已無關，但決定了處置的有效範圍：Anthropic 封帳號時，已排程的內容仍會如期上線。— ★★☆（p.68–70 三處明文）
6. **多平台直播循環分發**：成品以 Facebook Live、YouTube、TikTok「continuous cycle of livestreams」（p.67）循環播送給鎖定的鄉村受眾；直播形式本身在模仿電視新聞頻道的持續播出體驗，也讓內容具有查核者追不上的暫時性（§3.5.3、§4.5）。— ★★☆（p.67 明文；暫時性推論屬合理重建）

**本案未標記 F5（工具／記憶中介）**：§4.2 明確指出 Claude 端沒有工具呼叫、沒有多步驟規劃、沒有代理協作——每次批次生成都是無狀態的單次呼叫；步驟 5–6 的「跨會期接續」是外部腳本與固定模板在承載，不是模型的持久記憶。這讓本案在自動化軸與自主性軸上呈現罕見組合：**產線自動化程度很高，但 AI 自主性很低**（§4.2 第一點）。

**（二）為何對模型的推論有效**

核心機制是雙重用途原理落差：模型每次 API 呼叫看到的，是一個獨立、完整、看似正當的寫作任務——「用孟加拉語寫幾則新聞標題與故事」，這與任何一家合法媒體、公關公司、內容農場委託的日常寫作請求可能逐字元相同。模型看不到、也無法從單一請求推得的，是這次寫作背後的四件事：**這些事件全屬虛構**（真偽）、**內容將被套上電視新聞版型後冒充新聞頻道播出**（來源血緣）、**這是 29 個帳號輪替、16 個月、上百批次的同一條產線**（規模／協同），以及**目標受眾因識字限制而無法對內容做任何獨立查核**（意圖與危害的接收端）。

「固定輸出綱要」之所以比內容審查更難防，是因為它操縱的不是模型判斷「這段文字有沒有害」，而是操縱**格式本身在下游會被如何詮釋**。電視新聞版型對本案鎖定的受眾而言就是查核程序的全部——這代表「輸出格式操縱」真正繞過的不是模型的安全訓練，而是**受眾**的驗證捷思。模型端的內容分類器即使完美運作，也只能擋在「這段文字是否捏造」這一關；它無法預見這段文字會被剪進一支戴著新聞台外衣的影片，更不可能參與那支影片後續的信任工程——這正是雙重用途領域「善意與惡意輸出可逐字相同」的原理性落差（報告 p.137 對此有一般性討論）。
...
flowchart TB
    subgraph REAL["攻擊者真實行動"]
        R1["設計固定敘事與綱要<br/>四大主題＋15標題／3故事／15圖像提示"]
        R2["29帳號輪替批次呼叫<br/>16個月・約100批次"]
        R3["跨服務去情境<br/>圖像模型→音訊影片工具→Google Drive"]
        R4["套電視新聞版型<br/>標題條・台標・主播側影・跑馬燈"]
        R5["第二腳本排程上傳<br/>CI服務遮蔽IP・提前數月"]
        R6["三平台直播循環分發"]
        R1 --> R2 --> R3 --> R4 --> R5 --> R6
    end
    subgraph SEEN["模型每步看到的"]
        M1["一次孤立的寫作請求<br/>『幫我寫新聞標題與故事』"]
    end
    subgraph GAP["模型看不到的落差"]
        G1["真偽落差<br/>事件全屬捏造・非改寫真實報導"]
        G2["來源血緣落差<br/>將被套新聞台版型冒充新聞頻道"]
        G3["規模與協同落差<br/>29帳號・16個月・同一條產線"]
    end
    subgraph DETECT["偵測與防護切點"]
        D1["會話層：跨帳號聚類固定綱要節拍<br/>抓鳳凰帳號模式"]
        D2["輸出層：偵測『套新聞台版型』請求<br/>格式分類器獨立於使用者格式要求"]
        D3["架構層：API批次配額與稽核留痕"]
        D4["跨業者指標共享<br/>資料夾識別碼／上傳腳本交分發平台處置"]
    end
    R2 --> M1
    M1 -.-> G1
    M1 -.-> G2
...
這套「固定綱要批次產文→跨服務去情境→套版型→排程分發」的產線，一旦搬到學員自架的地端／開源 LLM，攻擊者甚至不需要 29 個帳號輪替——地端模型沒有用量上限、沒有帳號可封，「規避偵測」這項成本直接歸零。更危險的是：目前這條產線之所以留下痕跡（Google Drive 的帳號共用紀錄、YouTube API 的配額軌跡、CI 服務的排程節奏，見 §4.4、§6.1.5 第四點），全是因為它必須借用外部雲端服務；若圖像、音訊、影片模板全部改用地端模型與本機工具串接，這些痕跡會整個消失——沒有第三方留得下時間戳、共用紀錄與配額紀錄。裸模型也不會對「15 則標題＋3 篇故事」這種產線節拍有任何警覺，因為這不是越獄，只是一連串看似平常的寫作請求。

**地端防護重點**（對映四層 playbook）：

1. **①輸入層（效果有限，誠實記錄）**：本案不是越獄——全章唯一沒有記載模型拒絕的案例（§2.6 第五點），無 F1／F3／F6 證據。越獄／注入分類器與系統提示強化對「表面正當的批次寫作請求」幾乎無感：**能過濾違禁字的柵欄，過濾不了違禁的產線。**
2. **②會話層（抗 F2）**：跨帳號／跨裝置指紋聚類固定綱要節拍——本案「15/3/15」一字不差的批次結構，比任何關鍵字都穩定；建鳳凰帳號規則，偵測舊帳號停用後新帳號以相同綱要起手。
3. **③輸出層（抗 F7）**：輸出分類器要獨立於使用者要求的格式，偵測「把內容套新聞頻道版型」「要求固定跑馬燈／主播播報體」這類格式化請求本身，而不只看文字是否有害——本案證明格式操縱可以完全不觸碰安全訓練的紅線。
4. **④架構層**：對批次 API 用途設速率與配額並留稽核日誌，讓產線形狀可觀測；地端沒有平台方的帳號封鎖與跨業者指標共享機制，這道防線只能自己補——文字產出、模板套用、排程上傳三段的紀錄缺一，出事後就無法覆盤。
5. **跨業者指標共享在地端消失，這一點本身要教**：本案處置之所以還留有效果，是因為 Anthropic 把資料夾識別碼與上傳腳本「留給分發平台」（p.70）處置——地端部署下沒有這個外部協作對象。**地端化在移除攻擊者痕跡的同時，也移除了唯一能讓他被抓到的外部證人**；這是本案給防守方最誠實的結論。
