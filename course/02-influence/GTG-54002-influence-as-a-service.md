...
- **F2（任務拆解＋跨 session 分散）**：以程式把產製切成固定批次呼叫，每個請求（給一篇來源＋目標受眾／政治角度→要一篇成品）單看都像內容行銷公司的日常工作，惡意只存在於外部編排（§4.3）— 證據等級 ★★☆
- **F7（輸出格式操縱）**：所有提示強制固定 JSON 結構、格式化 HTML、精確字數、每篇 3–4 內鏈；同一來源改寫成相反意識形態版本、為原本無政治性的故事加政治角度、把故事跨境洗到不相關地區並剝除原始脈絡（§4.3）— 證據等級 ★★☆

**對地端 LLM 的意義**

這是「一個帳號＝一座內容農場」的範本。學員自架地端 LLM 若只做「單一請求分類」，會完全接不住這種攻擊：每一次呼叫都是無害的改寫，8,913 篇的規模與跨境洗白只在把這些呼叫串起來時才顯形。裸模型沒有請求速率／批次節律的觀測，也沒有輸出端血緣把關，正好是這種工業化產線最理想的引擎。

**地端防護重點**（對映四層 playbook）

1. **會話層（抗 F2）**：對同一主體做跨請求關聯與累計風險評分；把「固定輸出綱要（同一 JSON schema／字數／內鏈數）高頻重複」當成產線節拍的行為簽章，而非逐篇判斷。
2. **輸出層（抗 F7）**：輸出分類器獨立於使用者指定的 JSON／HTML 模板；偵測「同一來源被要求改寫成對立立場」「剝除原始出處與地理脈絡後重新在地化」的產出型態。
3. **架構層**：對以 API 批次驅動的自動化用途設速率與配額，並保留可稽核日誌（誰、何時、批次多大、輸入來源）——這是把「產線形狀」變成可觀測訊號的前提。
4. **架構層**：認清縱深多層——單一內容分類器對「看似正當的內容行銷」幾乎無效，必須靠會話層的規模與協同訊號補上。
