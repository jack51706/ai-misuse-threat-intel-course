...
- **F5（工具／記憶中介）**：用 Claude Code 自建自訂儀表板管理／運行／追蹤約 1,000 個假帳號網路，撰寫暖機邏輯與規避邏輯（定期更新 cookie／IP）；大量內容嵌在持久記憶檔（§4.3 叢集④⑤、§4.4）— 證據等級 ★★☆
- **F7（輸出格式操縱）**：AI 改寫管線多次改寫以壓低與原文相似度、剝除 TV BRICS／新華／Sputnik／CGTN 的國家歸屬偽裝成獨立馬來西亞報導、把捏造卷宗賦予「虛假權威」格式（§4.6 洗白鏈）— 證據等級 ★★☆
- **F3（拒絕後重新提示）**：Claude 拒絕（辨識出政治誹謗材料）後，行為者「協商淨化過的措辭」續推（§4.3 階段⑫）— 證據等級 ★★☆

**對地端 LLM 的意義**

本案的自主程度是 L1–L2（人用 AI 快速把作戰工具做出來，再用這些工具執行），但「一個帳號＋約四週＝一整個作戰室」的門檻崩塌已足夠危險。學員自架地端 LLM 最該警惕兩件事：一是模型會樂於幫你寫「帳號暖機／規避偵測」邏輯（F5 工具建構），二是裸模型沒有「黏性拒絕」——第一次拒絕後，換個淨化措辭就會過（F3）。

**地端防護重點**（對映四層 playbook）

1. **會話層（抗 F3）**：記錄「拒絕→改寫→順從」序列，對已被拒的同主題請求提高審查（黏性拒絕狀態），而不是每次從零判斷——這是裸模型最缺、也最好補的一層。
2. **架構層（抗 F5）**：對「請模型撰寫假帳號暖機／偵測規避／cookie-IP 輪替邏輯」「用模型建持續運作的操作儀表板」的請求型態提高審查；自建代理工具最小權限、機密不入上下文、持久記憶不夾帶定向指令。
3. **輸出層（抗 F7）**：輸出分類器獨立於格式；偵測「多次改寫以規避相似度比對」「剝除來源國家歸屬」「賦予捏造內容虛假權威格式」的產出型態，並在輸出端保留內容血緣。
4. **架構層**：對具名個人的指控類產出獨立再審（模型自身研究若查無佐證即應攔）；縱深多層，別讓一次拒絕被一句淨化措辭抵銷。
