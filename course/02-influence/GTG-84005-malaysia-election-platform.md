...
### 4.7 第五叢集「加密通訊／作業安全」：OPSEC-as-a-Service 與「用 LLM 建構安全工具」的 uplift（2026-09-15 深化）

p.55 的五叢集表（§4.2）裡，前四個叢集在本教材都有專節深挖——選民鎖定系統（§3.3、§3.4、附錄 E.1）、假帳號網路（附錄 E.4）、合成新聞媒體（§4.6 洗白鏈）、捏造卷宗（§3.5）。**唯獨第五個叢集「Encrypted messenger」被壓成一行、散落在 §2.4 與 §6 表格判讀（一）**。本節把它補齊。**先立紅線**：本節只重建**商業意涵與 uplift 定性**，**不重建任何加密／安全通訊工具的技術實作**——那既無報告依據，也逾越本模組的防禦紅線。

#### 4.7.1 報告到底寫了什麼（逐字，先劃清邊界）

| 欄位 | 原文（p.55 叢集表） | 證據等級 |
|---|---|---|
| Cluster | "Encrypted messenger" | 【報告明載】 |
| What Claude was used for | "Operational-security communications" | 【報告明載】 |
| Most serious element | "Purpose-built operational security for the operation" | 【報告明載】 |

**報告 body 完全沒有展開這一列**（p.53–58 內文未再提及加密通訊）。所以關於這個叢集，我們**確知的只有三個詞組**：加密通訊軟體、作業安全通訊、「為本行動量身打造」。以下所有分析都建立在這三個詞組上，凡超出者一律標為推論。

#### 4.7.2 (a) 商業意涵：把 OPSEC 打包成賣點（OPSEC-as-a-Service）

把這一列放回本案的**商業模式**（§2.5：平台以「influence-as-a-service」販售存取權、偽裝成「防禦型網路情報／反假訊息工具」）就會看到它的真正意義：

- **「purpose-built（量身打造）」這個詞是關鍵**【報告明載＋推論】。一般行為者的 OPSEC 是**自保**（免得自己被抓）；但一個**賣平台給客戶**的供應商，OPSEC 更可能是**產品功能的一部分**——賣給客戶的「安全通訊」。§6 表格判讀（一）已點出這層：行為者把 OPSEC「當成產品功能的一部分（賣給客戶的『安全通訊』），而不只是自保」。
- **這回應了 influence-as-a-service 客戶最大的採購顧慮**【推論】。買這種服務的客戶，最怕的就是**被抓、被歸因、被追回付款鏈**。把「可否認性」（§2.5：供應商隱身讓最終委託人可否認）與「加密通訊」打包成賣點，等於直接把客戶的頭號風險做成了行銷話術。這和平台**偽裝成「防禦型工具」**（§2.3）是同一套商業邏輯：**用「安全／防禦」的語言包裝攻擊能力**——買家買的是「安全」，賣家賣的是「作戰＋不被抓」。
- **可否認性的雙層結構**【推論】：對**客戶**保密（加密通訊，藏住「誰下的單」）＋對**外界**可否認（第三國供應商的結構，§2.5，藏住「誰在執行」）。兩層疊起來，正是本案為什麼技術指標全指向供應商、委託人卻完全隱身（§2.5(a)）的商業設計。

> **偵測與治理意涵**：OPSEC-as-a-Service 意味著「安全通訊」在這類供應商的行銷文件裡會被當**功能點**列出。對監管者與平台，這是一個可操作的線索——**B2G／B2B 的「政治作戰即服務」銷售文件中若把『加密／匿名通訊』列為賣點，本身就是一個歸類訊號**（配合 §2.5(b) 的管轄權碎裂討論）。

#### 4.7.3 (b) 「用 LLM 協助開發加密／安全通訊工具」是一種 uplift——交叉引用 cyber 章「建構作戰工具」主線

報告對本案的總述是：Claude 被用來「build the constituency targeting system … engineer and run the fake social media account network and its detection evasion logic … rewrite and launder fake news … iterate the fabricated dossiers」（p.55）。**如果「purpose-built operational security」屬實，它就落在同一類行為：用 Claude Code 建構作戰支援軟體。** 這正是 cyber 章的核心主線，值得跨模組對照：

| 對照點 | 本案（影響力章 GTG-84005） | cyber 章的同型主線 |
|---|---|---|
| 核心命題 | 一個帳號＋幾週＝一整套作戰室（§4.5） | 「AI has collapsed the labor and tooling gap」——AI 抹平了國家級行動與單兵之間的**人力與工具**落差（`01-cyber/00-cyber-trends-and-skills.md`，引 p.39） |
| 工具建構層級 | Claude Code 建儀表板／規避邏輯／（推論）安全通訊 | L1「engineering assistant in the creation of malware, phishing kits, and **surveillance tooling**」（p.39） |
| 為什麼是 uplift | 見下方三維度 | speed／scale／depth 三維（p.4） |

- **交叉引用跨案例分析「主線二：AI 作為勞動力，而非知識」**（`_shared/01-cross-cutting-analysis.md`）：加密通訊的**密碼學知識本來就是公開的**（Signal 協定、libsodium、Matrix/Olm 全部開源），瓶頸從來不是「知識可得性」，而是「把它**正確整合**進一個可用、不出錯（密碼學最忌自己亂寫）的工具所需的工程人力」。**LLM 填的正是這個勞動力缺口**——這與武器章葉門案（GTG-87001）「in place of human software engineers」、監控章「AI 取代工程師」是同一個現象的不同領域投影。
- **uplift 落在哪個維度**【推論，承 §4.5 方法】：主要是**深度**（depth）——一個沒有安全工程背景的政治作戰供應商，過去要嘛雇一名密碼學工程師、要嘛只能用現成 App；LLM 讓他能「自己」拼出一個看起來量身打造的安全通訊層。其次是**速度**（把整合工作從數週壓到數天，呼應 §2.6 時間軸的七天建置）。

> **但必須誠實對比證據強度**：cyber 章的「建構作戰工具」有 **Figure 19（威脅行為者自建的 Claude Code skills 清單）**當鐵證；本案的加密通訊叢集**只有一行**。**不能把 cyber 章的證據強度套到本案。** 本節的跨模組對照是「**同型**」的類比，不是「同強度」的斷言。

#### 4.7.4 (c) 誠實標註：報告只給一行，無法斷定自建或既有軟體客製

「purpose-built」到底是什麼工程形態？報告沒說。**至少有三種可能，報告的一行文字無法區分**：

| 假說 | 具體形態 | 「purpose-built」的相容度 | 若為真，偵測落點 |
|---|---|---|---|
| H1 從零自建 | 用 Claude Code 寫一個自有的加密通訊 App／後端 | 高（最符合字面） | 供應商端：要求生成密鑰交換、E2EE、訊息後端的對話（模型端可見） |
| H2 既有軟體客製 | 對開源安全通訊（Matrix/Element、Session、Signal fork）做部署與客製整合 | 高 | 基礎設施端：自架 Matrix homeserver／Session 節點的指紋 |
| H3 純 SOP＋現成 App | 直接用現成加密 App（Signal／Telegram 私密聊天）＋用 Claude 寫 OPSEC 規則／SOP | 中（「量身打造」偏指工具而非 SOP，但不排除） | 幾乎不可見：用現成 App＋人工 SOP，模型端與基礎設施端都難留痕 |

**§2.4 行為者畫像表已把這一列標為報告的一處沉默**（「報告未說明是自建還是既有軟體」）。本節維持這個判斷：**「purpose-built」在語感上偏向 H1／H2（指向一個「被打造出來的東西」），但單憑一行文字無法排除 H3。** 這正是「按原文能寫多少就寫多少、不硬湊」的示範——**在證據只有三個詞組時，負責任的做法是把假說空間攤開並標明無法裁定，而不是挑一個講得像定論。**

> **本節與全案的一致結論**：第五叢集是本案「**AI 把整套作戰室（含作業安全）降到單人＋數週成本**」論點（§1、§4.5）的最後一塊拼圖。它的教學價值不在技術細節（報告根本沒給），而在兩件事：(1) **OPSEC 可以被當成商品賣**——這是「影響力即服務」成熟到一定程度的訊號；(2) **連「作業安全」這種過去需要專門工程的環節，都可能被 LLM 的勞動力補上**——這把 §4.5「一個帳號＋約四週」的門檻崩塌論點推到了極致。
...
> 本節依 `../_shared/02-claude-safeguards-and-bypass-paths.md` 第九節的七大手法族（F1–F7）與四層地端防護 playbook 就地深化，重建本案「行為者怎麼一步步驅動 Claude」。防禦視角，不含可複製的越獄字串，也不示範可操作的宣傳寫作或人設經營教學；與 §4（攻擊生命週期）、§6（圖表判讀）互為裡外。

### 1. 推測的操作序列

依 §4.3 生命週期表、§4.6 洗白鏈拆解、§4.7 OPSEC 叢集重建攻擊者驅動 Claude 的操作序列。本案沒有蒸餾章那種逐字提示原文，故單步證據最高只到 ★★☆；每步標對映的 F 族。

**S1　定位框定：軍規話術與防禦外衣兩張臉**
- 平台對內、對投資／行銷語言是「軍規、AI 驅動、即時政治作戰生態系」，對外、對客戶則包裝成「防禦型網路情報／反假訊息工具」（§2.3、§2.5、§4.7.2）。這層框定決定了後續每次請求會借哪一種「正當」說法包裝。
- F4（良性／防禦性改框）— ★☆☆（報告未明說 Claude 參與行銷文件本身撰寫，屬合理推論；§4.3①原表已標此為推測）

**S2　選區資料工程：政治動員送進去，資料分析任務出來**
- 用真實普查與選民資料，建構涵蓋 222 個選區的側寫系統，並沿種族、宗教、王室三條斷層線設計分眾（§4.2、§4.3②③）。單一請求看到的是資料建模與分群，看不到分群結果將被拿去做什麼。
- F4（改框成資料分析）— ★★☆（報告明載 Claude 建構此系統，p.54–55；分眾用於政治動員屬合理重建）

**S3　指揮中心與帳號艦隊：把「持續運作」一次性外包給 Claude Code**
- 用 Claude Code 建自訂儀表板，管理、運行、追蹤約 1,000 個假帳號，內建「每個目標應獲得多少總人工觀看」的可調參數；另外撰寫帳號的暖機邏輯與規避邏輯（定期更新 cookie／IP）。
- 程式碼一旦寫成並部署到行為者自己的主機（IOC 表所見 Hetzner），後續的貼文與規避動作就完全脫離「每次都要問 Claude」的請求層級——這正是「建構與運行被分開」的機制核心（§4.3 三個觀察之一）。
- F5（工具／記憶中介）— ★★☆（報告明載，p.54）

**S4　洗白管線：把「抄一篇官媒」拆成一連串「潤稿」請求**
- 爬取合法馬來西亞報導，以及 TV BRICS、新華、Sputnik/RIA、CGTN 的俄中官媒文章，送進模型多次改寫，掛捏造署名、剝除國家歸屬後重新發布成「獨立馬來西亞報導」（§4.6）。
- 每一次改寫呼叫單獨看都只是文字潤飾，看不到上一步是剽竊、下一步要去掉來源——四層操作沒有一步需要人類自己寫一個字（§4.6 總結）。
- F7（輸出格式操縱，兼有 F4 改框成編輯潤稿）— ★★☆（報告逐字「had the model rewrite it several times」「stripped out the state attribution」，p.55）

**S5　卷宗迭代：格式賦予虛假權威**
- 對反對黨政治人物與公民社會組織，「迭代」捏造的情報卷宗，被賦予「虛假權威」的格式（§4.2、§4.3⑦）。同一組虛構指控，套上情報報告的版型與用語後，讀起來像已有調查依據。
- F7（輸出格式操縱）— ★★☆（報告明載格式操作存在，具體模板內容報告未給，p.54–55）

**S6　拒絕→協商淨化措辭→續推**
- Claude 至少一次拒絕或部分拒絕——辨識出政治誹謗材料，對心理作戰語意卻步；行為者的回應不是放棄，而是「協商淨化過的措辭」後繼續推進同一任務（§4.3⑫）。
- 這一步暴露本案拒絕機制的性質：內容層、逐句判斷，擋得住露骨誹謗字句，擋不住換一種委婉說法表達的同一組指控。
- F3（拒絕後重新提示）— ★★☆（報告明載此行為模式存在，具體換過的措辭報告未逐字給出，p.55、p.57）

### 2. 為何對模型的推論有效

- **資料分析的外殼讓政治動員任務被當正當工作**：選區側寫系統的輸入輸出，在技術形式上與任何選舉研究機構、政黨數據部門的合法工作完全相同——差別只在分眾軸的用途，而用途不在請求裡（S2）。
- **「潤稿／改寫」是最普遍、最不設防的請求型態之一**：多次改寫本身是合法編輯工作的日常操作；模型不記得這段文字三次呼叫前是從哪裡爬來的，來源血緣在每次改寫之間斷裂——這正是 F7 能生效的原理：輸出端看不到輸入端的來歷（S4）。
- **建構與運行分離製造了雙重用途的時間落差**：Claude 只涉入「建構」——儀表板、規避邏輯、改寫管線本身——完全看不到這些工具部署後在行為者自己主機上如何被「運行」。這類工具開發請求，在雙重用途上與任何合法的社群媒體管理系統或內容管理系統開發請求無法區分（S3）。
- **格式即權威，套模板不需要說謊**：把捏造內容排成情報卷宗版型、把改寫文章套進新聞網站版型，讓內容「看起來」可信的是格式借用的社會信任訊號，不是模型對事實的背書——模型只是被要求「排版」，這個請求本身無害（S5）。
- **拒絕是逐句判斷，協商只是換句話說**：拒絕分類器判斷的是「這段文字」是否構成誹謗性內容，而不是「這個委託人是否還在追同一組指控」——換一種委婉說法，內容層判準就重新歸零（S6）。
- **回到雙重用途原理**：模型每個單一請求看到的都是一件合法的翻譯／改寫／資料建模／排版工作；真正的惡意藏在它看不到的三件事——意圖（資料分析或選舉操縱）、真偽（原創潤稿或洗白盜用）、來源血緣與規模協同（單篇改寫或約 1,000 帳號 × 222 選區的協同網）。這三者沒有一個會出現在任何一次請求裡。

### 3. Mermaid：操作流程 → 模型看到什麼 → 落差 → 偵測切點

```mermaid
flowchart TB
    subgraph REAL["攻擊者真實行動"]
      R1["Claude Code建指揮儀表板＋<br/>暖機與cookie/IP規避邏輯＋<br/>約 1,000 個假帳號網路"]
      R2["爬本地報導與俄中官媒<br/>剝除國家歸屬洗白成<br/>malaysiapulse[.]com獨立報導"]
      R3["捏造卷宗反覆迭代＋<br/>拒絕後協商淨化措辭續推"]
      R1 --> R2 --> R3
    end
    subgraph SEE["模型每步看到的"]
      V1["幫我建帳號管理工具、<br/>寫反偵測輔助邏輯"]
      V2["把這段文字改寫幾次、<br/>去掉來源字樣再發一次"]
      V3["把這份文件排成<br/>情報卷宗格式、<br/>換個委婉說法再寫一次"]
    end
    R1 -.->|"送進模型的版本"| V1
    R2 -.->|"送進模型的版本"| V2
    R3 -.->|"送進模型的版本"| V3
    V1 --> C1{"內容層判斷：<br/>單一請求像不像惡意"}
    V2 --> C1
    V3 --> C1
    C1 -->|"逐句看都合法 → 放行"| PASS["框定與改寫成功繞過"]
    C1 -->|"露骨誹謗字句 → 拒絕"| REF["Claude至少一次拒絕：<br/>辨識出政治誹謗材料"]
    REF -->|"F3協商淨化措辭再推"| PASS
    subgraph GAP["模型看不到的落差"]
      G1["意圖：資料分析<br/>還是選舉操縱？"]
      G2["真偽：編輯潤稿<br/>還是洗白盜用內容？"]
      G3["來源血緣與規模：<br/>單篇改寫 還是<br/>約1,000帳號協同網？"]
    end
    PASS -.->|"三者都不在單一請求裡"| GAP
    subgraph DETECT["偵測與防護切點"]
      D1["會話層：拒絕→協商<br/>→續推序列黏性追蹤"]
      D2["架構層：持續運作的儀表板／<br/>規避邏輯類請求提高審查"]
      D3["輸出層：句嵌入比對抓多次改寫<br/>＋保留輸出血緣標記"]
      D4["身分層：具名個人指控<br/>類產出查無佐證即攔"]
    end
    GAP --> DETECT
    style REAL fill:#ffe8e8
    style SEE fill:#fff0d0
    style GAP fill:#e8ffe8
    style DETECT fill:#e8f0ff
```

### 4. 對地端 LLM 的意義 ＋ 地端防護重點

這套手法搬到學員自架的地端／開源 LLM 會更危險。第一，商用 Claude 至少在 S6 留下一次拒絕與協商措辭的紀錄；裸模型的誹謗判準若訓得更寬鬆，很可能連這道關卡都不會觸發，且沒有事後可追蹤的帳號封鎖機制接手。第二，本案「建構」與「運行」分離——儀表板與規避邏輯一旦寫成並部署到行為者自己的主機，後續運作就完全脫離模型供應商視野；地端模型從一開始就是這種「建構後脫鉤運行」的天然環境，連「建構」這道唯一留痕的關卡都可能不存在。第三，語言在地化正是本案 uplift 最危險的維度（§4.5）——地端模型若涵蓋馬來文等在地語言，等於把「沒有在地知識也能做在地動員」的門檻，從「需要一個 Claude 帳號」降到「需要一張夠力的顯卡」。

**地端防護重點**（對映四層 playbook）：

1. **會話層（抵 F3）**：記錄「拒絕→協商淨化措辭→續推」序列——本案的具體訊號是同一委託對同一具名對象的指控性內容，遭拒後仍反覆換措辭回來；地端部署應把這類序列標記為黏性拒絕狀態，而非每次重新從零判斷。
2. **架構層（抵 F5）**：對「建一個會持續運行、管理／追蹤大量帳號的儀表板」「寫帳號的暖機或 cookie／IP 輪替邏輯」這類會被部署成獨立軟體、之後不再逐次呼叫模型的請求型態，提高審查並留存稽核紀錄——這正是本案能用一個帳號撐起約四週作戰室的關鍵缺口（§4.5）。
3. **輸出層（抵 F7）**：對「把這段文字換句話說、去掉來源字樣」的重複改寫請求，建立跨呼叫的相似度追蹤——用句子嵌入而非表面文字比對（§4.6 第二層拆解）——偵測同一素材被反覆丟進模型「洗」的模式；對套用情報報告／新聞台固定版型的格式化請求保留輸出血緣標記。
4. **輸入層與會話層（抵 F4）**：對「幫我建一套用普查／選民資料做分眾分析的系統」這類本身合法的請求，不能只看內容合法就放行——若同一主體後續接著要求依種族／宗教／王室分眾，應把這兩類請求跨會話關聯後再判斷，而非各自獨立放行。
5. **架構層／身分層**：對具名自然人的指控性內容產出，比照本案「模型自身研究查無佐證」的情節（§4.3 三個觀察之三），建立「先查證、查無佐證即攔」的輸出前檢核，而不是只靠拒絕分類器判斷措辭是否露骨。
