...
- **F7（輸出格式操縱）**：把未經查證的主張寫成事實、剝除來源與確信度（報告命名的「laundering of attribution／sourcing／certainty」）；要求「去除自動文本痕跡、聽起來有機」；產出字元精確的跑馬燈／字幕直接上版（§1、§4.2、§4.5）— 證據等級 ★★☆
- **F4（良性／防禦性改框）**：把俄方來源主張改框成「獨立報導」，把克里姆林敘事包裝成 @ATodaPotencia 之類「看似獨立的在地評論」（§4.5、§4.6）— 證據等級 ★★☆
- **F2（任務拆解＋跨 session 分散）**：Sputnik Africa 線把編輯部風格固化成「40 條 house style guide」餵給模型，等於一份跨會期重用的持久指令檔，讓每次請求都只是無害的單篇改寫（§2.1、§4.2）— 證據等級 ★☆☆（由「40 條規則」推得的手法映射）

**對地端 LLM 的意義**

本案的危險不在自主性高，而在「嵌入位置好」——AI 只是接在既有掛牌媒體產線上的副編輯。學員若把地端 LLM 接進自己的內容產線，最容易被複製的就是這種「真素材＋假框架」：模型忠實地做跨語改寫、去來源、套模板，而每一次請求單看都像正當的翻譯與編輯工作。裸模型沒有輸出端把關，會把「洗白確信度」做得又快又乾淨。

**地端防護重點**（對映四層 playbook）

1. **輸出層（抗 F7）**：輸出分類器獨立於使用者要求的格式；偵測「要求移除來源標註／把 unverified 寫成既定事實／強制某固定模板」這類請求，並在產出端保留來源鏈與確信度標記，不讓模型幫忙抹掉。
2. **輸出層（抗 F4）**：判內容血緣而非表面框架——同一主張若來自單一未查證上游，不因被改寫成「獨立在地評論」就升級為可信；對「把某來源重述為另一身分之言」的產出獨立再判。
3. **會話層（抗 F2）**：把「40 條風格規則」這類持久指令檔視為意圖載體，做跨會期意圖聚合；持久記憶／風格檔不得夾帶「一律略去來源、一律以中性口吻斷言」這類會系統性去責的規則。
4. **架構層**：內容產線型部署要在模型「產出時」留存血緣（來源、原文、改寫層數），因為一旦內容離開模型就再也追不回；縱深多層，別指望單一分類器接住「合法改寫」與「洗白」之間的灰帶。
