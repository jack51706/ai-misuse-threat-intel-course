...
- **F5（工具／記憶中介）**：自建私有平台承載 AI 人格「Deadshot」，把 master doctrine file 寫進系統設定並跨數百會期近乎逐字重用——意圖被移進持久記憶層，單次對話因此顯得無害（§4.2 階段 0）— 證據等級 ★★☆
- **F7（輸出格式操縱）**：把公開素材轉成「看起來像官方情報產品」的簡報格式（格式即說服力），並要求「去除自動文本痕跡、聽起來有機」（刻意留語法瑕疵）（§4.2 階段 2、5）— 證據等級 ★★☆
- **F4（良性／防禦性改框）**：cloned identity reports 以被冒用組織口吻發聲、ghost-written testimony 套「不得提及 UAE」硬限制、初期貼文刻意不提穆兄會以偽裝中立（§4.2 階段 2、4）— 證據等級 ★★☆

**對地端 LLM 的意義**

本案是「準自主」的最佳教案：在單次會期尺度上是人類逐步指揮，在整個行動尺度上卻是自主的——因為「做什麼、為誰做、不能說什麼」已固化在系統提示層，人類退化成投料者。學員自架地端 LLM 若允許使用者自帶系統提示／記憶檔，威脅就藏在你（平台方）看得到內容、卻不會判為違規的那份「政治教義檔」裡。

**地端防護重點**（對映四層 playbook）

1. **會話層／架構層（抗 F5）**：把系統提示與持久記憶檔當一級偵測對象——監控「系統提示跨會期高重複」「同一組織／個人名單反覆出現」「產出物帶固定收件人欄位」「大量否定型限制（不得提及 X）」；持久記憶不得夾帶未審的定向指令。
2. **輸出層（抗 F7）**：輸出分類器獨立於格式；「official-looking」的權威外觀不該提高信任，反而對「產出被要求偽裝成官方情報／政府文件格式」的請求加審；把「要求移除 AI 痕跡」列為訊號。
3. **輸出層（抗 F4）**：判代表性——模型被要求以他人／他組織身分發聲、或刻意隱藏所屬時，獨立再審其身分宣稱，不因口吻中立就放行。
4. **架構層**：對 agentic／自帶平台部署，工具最小權限、機密不入上下文；認清縱深多層——單看一則「幫我把資料整理成簡報」永遠無害，必須靠系統提示層與會期序列的聚合才守得住。
