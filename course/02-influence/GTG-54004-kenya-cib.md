...
> 本附錄為第二階段技術深化，聽眾為技術高手。**上面第 0–12 節完全保留、未刪改**；本附錄只做「增補」——把第 6 節（協同叢集）、第 4.3 節（copy-all 小工具）、第 4.5 節（長期養帳號）、第 10.4(4) 節（LINE）四處的觀念，補到「技術高手能據以理解與親手防禦」的深度：形式化定義、演算法、實證數字、待環境驗證的偵測規則範例（示意；YARA / KQL / 虛擬碼），以及新增 4 張 Mermaid 圖。所有超出報告的技術主張一律標【研究員推論】；所有實證數字均附一手來源，見 A.0。**本附錄用的是新工作階段的 WebSearch 配額**（第 9 節「配額已耗盡」指的是第一階段）。
...
4. **向 LINE 台灣要「匿名遙測」**：要求提供**去識別化的「訊息首次出現時間 + 轉傳次數」**統計（可用差分隱私保護個資）。這一條若拿得到，就把 A.1.6 的**時間訊號**部分要回來——可能有助補足可見度，但取得成本、隱私影響與防禦成效仍待評估。
...
> 本節依 `../_shared/02-claude-safeguards-and-bypass-paths.md` 第九節的七大手法族（F1–F7）與四層地端防護 playbook，重建本案「攻擊者怎麼一步步驅動 Claude 產製影響力內容」。本案是全案**自主程度最低、對話式協助**的案例，卻因此最難靠內容訊號偵測——純防禦視角，不含可複製的越獄字串、不示範操弄話術。

### 1. 推測的操作序列

依 §4.1–4.2、4.5–4.6 與 Figure 17 重建。本案人類介入密度全案最高——每一步都有人在場——因此多數步驟是「人力決策＋模型工業化」的混合，不是模型端的技術繞過；標「不涉 F 族」的步驟仍列出，是為了誠實呈現手法的完整輪廓（每步標證據等級：★★☆ 述行為、合理重建／★☆☆ 純推測；最高一級「報告逐字原文」在影響力全章並不存在，故本案最高只到 ★★☆）：

- **S1｜人力定調，尚未觸及模型手法**：行為者在 OpenAI 平台已有處置紀錄後轉戰 Claude（累犯）；核心意識形態訊息——挺 Wandayi、打反對黨與 Gachagua／Kenyatta——與主題、talking points、hashtag「完全由人決定」，部分已預先擬好草稿。這步決定了後續每個請求送到模型面前時「表面上」是什麼。— ★★☆（p.76–77）
- **S2｜大量生成＋去 AI 痕跡（F7）**：下指令「每批精確 50 則、要看起來自發草根而非協同」；模型「轉換成 50 則有主題、算好字數、可跨平台發布的貼文」，並「人性化並精修」預擬文字。Figure 17 四則貼文即成品：每則 1–2 句、語氣像關心民生的公民，骨架卻完全相同。— ★★☆（p.75–77 三處逐字提及「50」與「humanize and refine」）
- **S3｜包裝成可部署散布介面（F5）**：「在數個工作階段中，這些被包裝成可直接部署的互動式 copy-all 小工具」——AI 交付的不只是文字，是散布的操作介面，讓不需理解策略的執行者一鍵複製，把「協調成本」而非「寫作成本」歸零。— ★★☆（p.77 逐字「interactive copy-all widget ready for deployment」；小工具實際長相為研究員依產品功能推論，故整體仍評 ★★☆）
- **S4｜人力散布，借真新聞洗白來源**：假帳號與本地網紅在 X 發布，引用真實新聞（Citizen TV、Berur News）、掛同順序 hashtag、@Wandayi；Claude 不碰平台。四則貼文全在真政策公告後 20 小時內上線，且共享同一則引用推文、同一張圖卡——這是 S2「同骨架換皮」在發布端留下的協同指紋，不是新手法，是 S2 的可觀測後果。— ★★☆（Figure 17 直接可見，時序由獨立查證的新聞日期佐證）
- **S5｜範本跨情境複用（F2，弱化版的「持久記憶」）**：同一套「50 則＋humanize＋copy-all」流程一字不差沿用到商業行銷（SHANKI／Elkins Marketer 人格、每第五則插連結）。本案沒有 SKILL.md 等技術性記憶檔證據，但行為上等同範本跨會期／跨情境接續——操作者把整條產線當可重複調用的 SOP，而非一次性任務。— ★★☆（p.76 明確用「verbatim」一詞）

### 2. 為何對模型的推論有效

- **每個單一請求都是道地的良性任務**：「把這些主題寫成 50 則社群貼文、字數算好、語氣自然一點」與任何行銷代操的日常請求在文字上沒有差別——模型看到的是一件內容改寫／潤飾工作，不是「政治動員」。
- **「去 AI 痕跡」是通用的風格請求，不是攻擊訊號**：humanize／refine 在正當使用情境裡極常見，模型沒有機制分辨「讓我的部落格聽起來自然一點」與「讓 50 個假帳號聽起來互不相識」。
- **「不要協同」只是一句形容詞，不是可驗證的技術動作**：要求「看起來自發草根」在請求端只改變語氣，模型無法驗證這句話背後有沒有 30 個帳號要同步發布——真偽（authenticity）的落差發生在模型看不到的下游，不在這句指令本身。
- **copy-all 小工具請求落在再正常不過的開發任務裡**：「做一個列表＋一鍵複製按鈕的網頁」是極通用的工具需求，模型無法從介面規格反推「這會被貼給 30 個網紅同步發文」。
- **來源洗白發生在模型完全不參與的階段**：引用真新聞、加 @ 官員是人類在 X 上手動完成的 quote-tweet，Claude 不曾看過這個組合，沒有機會判斷「這是在借媒體公信力洗白」。
- **本案能曝光，靠的不是內容分類器，是跨平台行為情報**：調查起點是 OpenAI 分享的「累犯」線報，不是 Claude 自己的輸出過濾——這正是雙重用途落差的核心：單一請求層級的內容審查，本質上抓不到只存在於請求之外的意圖、真偽、規模協同三種訊號。

### 3. Mermaid：攻擊者產製流程 → 模型所見 → 三個落差 → 偵測切點

```mermaid
flowchart TB
    subgraph REAL["攻擊者真實行動"]
      R1["人力定調：挺Wandayi・打反對黨<br/>供應主題／talking points／hashtag"]
      R2["下指令：每批50則<br/>要求人性化、看起來自發不協同"]
      R3["包裝成copy-all互動小工具<br/>跨數個session完成、ready for deployment"]
      R4["假帳號與本地網紅發布<br/>引用真新聞、同序hashtag、@Wandayi"]
      R5["同一套生成SOP verbatim複用<br/>於SHANKI／Elkins Marketer商業行銷"]
      R1 --> R2 --> R3 --> R4
      R2 -.-> R5
    end
    subgraph SEEN["模型每一步實際看到的"]
      M2["一件內容寫作任務：<br/>50則貼文、算好字數、語氣自然一點"]
      M3["一個網頁工具開發請求：<br/>列表加一鍵複製按鈕"]
    end
    R2 -->|"送進模型的只有這些"| M2
    R3 -->|"送進模型的只有這些"| M3
    subgraph GAP["模型看不到的三個落差"]
      G1["意圖落差：<br/>政治動員 還是 日常文案工作？"]
      G2["真偽落差 authenticity：<br/>30個獨立公民 還是 一人操盤？"]
      G3["來源血緣與規模落差：<br/>單則貼文 還是 選前長期協同網絡的一環？"]
    end
    M2 -.->|"單則請求看不出規模與真偽"| G1
    M2 -.-> G2
    M3 -.->|"介面請求看不出下游怎麼分發"| G3
    subgraph DETECT["偵測與防護切點"]
      D1["輸出層：humanize要求列可疑訊號<br/>近重複／語意叢集抓同骨架換皮"]
      D2["架構層：copy-all批次UI生成請求<br/>提高審查、人工複核"]
      D3["會話層：固定N則批次節拍<br/>固定hashtag順序做行為簽章"]
      D4["平台端：co-quote／co-media／<br/>hashtag序列一分鐘共現窗口抓協同"]
    end
    G1 -.-> D1
    G2 -.-> D1
    G3 -.-> D2
...
這套手法搬到學員自架的地端／開源 LLM 會特別棘手，因為它幾乎不需要任何「繞過安全機制」的技巧：每個請求本身就是良性任務，裸模型不會拒絕、也不需要被拒絕。更關鍵的是，本案真正的破口不是輸出過濾，而是 Anthropic 靠 OpenAI 的跨平台線報才查到「累犯」——地端部署通常沒有這種跨供應商情報網路，連這條「事後補救」的路徑都不存在。換言之，自架 LLM 若只做內容層防護，對這類「自主程度最低、對話式協助」的濫用幾乎是全盲的，必須把防線往會話與架構層前移。

1. **輸出層（抗 F7）**：把「humanize／去 AI 痕跡／讓內容看起來不像同一來源」列為需額外記錄的請求型態；對同一使用者、同一 session 的多次輸出做近重複與語意叢集比對，抓「同骨架換皮」的量產模式，而非逐則判斷內容是否有害。
2. **架構層（抗 F5）**：對「產出可一鍵批次複製／部署的介面（列表＋copy 按鈕、批次貼文表）」這類請求提高審查層級——這是把散布協調成本歸零的關鍵步驟；若地端代理具備生成可部署頁面的能力，預設要求人工複核。
3. **會話層（抗 F2）**：對「每次剛好 N 則、固定字數上限、固定 hashtag／@ 位置」的批次請求做行為簽章，而非逐則內容判斷；同一帳號重複相同批次規格，即使每次文字都不同，也該累積風險分數。
4. **架構層＋跨組織情報**：認清單一 guardrail 必然被繞過——本案連 Anthropic 自己的內容分類器都不是破案關鍵，靠的是跨平台線報；地端部署若無法取得同等情報網路，至少要保留足夠的請求層級日誌（批次大小、輸出結構、session 節律），讓事後被通報時能重建行為模式，而非完全沒有可回溯的紀錄。
