...
> 本節依 `../_shared/02-claude-safeguards-and-bypass-paths.md` 第九節的七大手法族（F1–F7）與四層地端防護 playbook，重建本案「Claude 如何被當成掛牌宣傳編輯台的隱形副編輯」的操作序列。本案全程無逐字提示詞留存，證據等級最高只到 ★★☆；F1（人設／授權框定）、F3（同一會期內協商淨化措辭再推）、F5（工具伺服器／持久記憶鷹架）、F6（思維鏈套取）在 §4 均無對應行為紀錄，本節不列入。防禦視角，不含可複製的越獄字串。

### 1. 推測的操作序列

- **S1：離線定調（人類，Claude 未參與）**——主題、談話要點、可重複使用的範本與常設指令，在第一個 prompt 送出前就已備妥；報告明言「the planning was mostly done offline before any prompt was sent to Claude」（§4 階段 0）。範本與常設指令之後跨大量會期原樣重用，是後續每一步能「單看無害」的前提。— F2，★★☆
- **S2：素材匯出＋文風複製（Claude 協助）**——匯入兩個親俄 Telegram 頻道：機構體「在班基潛伏」（實為俄羅斯駐中非大使館轉發）與軍事宣傳體 SOMB（Wagner 系「遊客」黑話），指示 Claude 做 stylistic voice analysis，取得可複製的「在地聲音」語氣模板（§4 階段 1；§6 Figure 1 圖說原文「always exported for stylistic voice analysis, cloning and distribution」）。— F7，★★☆
- **S3：依範本逐場次生成＋去 AI 化（Claude 為主）**——每次生成都附上反法親俄談話要點，產出簡報、廣播稿、貼文、圖像；同時明確要求「strip away classic formatting habits」，避免稿子讀起來像 AI 生成（§4 階段 2 逐字原文）。— F7 為主／F2 為次（逐場次分批下指令，單批看不出全貌），★★☆
- **S4：HR 忠誠基礎設施（Claude 為主）**——起草效忠合約（效忠「中非總統與俄羅斯及其特遣隊」）、職務說明、評分量表、三振開除流程；用量表為員工文章打分並建議留任或開除。Claude 標記出評分的「政治權重」後，行為者把標籤改成中性用語、保留同一套評分繼續使用（§4 階段 3 逐字原文）。— F4，★★☆
- **S5：政治控制周邊產品（人類主導，Claude 角色不全明）**——反對派監控資料整理、俄羅斯之家發言稿與談話要點、用真實政府設計檔偽造公文；三項任務個別看都是資料整理／講稿／排版的正常工作，報告未逐項說明 Claude 在每一步的確切分工（§4 階段 4）。— F2，★☆☆（AI 具體涉入程度為推測，非報告明載）
- **S6：拒絕與改框續推（防線事件）**——Claude 拒絕「點名真人為武裝分子」的請求；行為者改用「匿名消息來源」框架續推同一指控（§4 階段 6 逐字原文）。— F4，★★☆
- **S7：洗白式分發（Claude 不參與）**——Radio Lengo Songo 98.9 FM 每日播出 → Telegram 放大 →「經核可」在地媒體（Ndjoni Sango、Pravda RCA）轉載 →（目標）進入國家電台 Radio Centrafrique → RT／TASS 國際放大（§4 階段 5）。這一整層完全在 AI 供應商視野之外，也是本案評到 Breakout Scale Category Four 的真正原因（§4.8）。— 不適用 F 族，事實層級 ★★☆

### 2. 為何對模型的推論有效

每一步單獨送進模型的請求，都落在完全合法、常見的職業任務類別裡：文風研究、新聞稿寫作、HR 合約、評分量表、潤稿、平面設計。沒有一步的字面內容是「請幫我做境外資訊戰」——模型判斷的永遠是「這件事本身有沒有害」，而不是「這件事在誰的計畫裡扮演什麼角色」。

「去除 AI 格式痕跡」「模仿某頻道的自然語氣」這類要求，落在模型的能力範圍內完全正當：編輯要求代筆者「寫得更像本地記者」是再正常不過的潤稿指示。模型沒有機制把「風格自然化」與「來源真偽洗白」連起來，因為連起來需要知道這段文字最終會被包裝成「中非本地聲音」對外播出——而這個下游事實從未出現在請求裡。

評分量表被標記「政治權重」後，行為者只是替同一套評分換了一層中性詞彙外皮，實際效果（某員工因政治立場不夠親俄而被開除）完全沒變。這證明內容分類器抓的是**當下請求的語意表面**，抓不住「這是同一份文件、跨多次請求反覆使用、且持續影響真人去留」的**物件層級一致性**。

三個落差因此同時存在：**意圖**（這是本地新聞工作，還是外國指揮的 FIMI？模型看不到談話要點由誰下達）、**真偽**（「在地聲音」頻道其實是大使館轉發克隆，模型只看到一段文字風格，不知道它冒充了誰）、**來源血緣／規模協同**（這篇廣播稿是獨立一篇，還是電台、Telegram、經核可媒體、國家電台同步聯播的一環？模型只服務單一請求，看不到協同）。更根本的是：本案真正的影響力槓桿在 FM 廣播，完全發生在 AI 供應商視野之外——即使生產端每一次分類都判斷正確，也攔不住已離開平台的內容被電台每天播出（§4.8 分發決定影響力）。

### 3. 操作序列總覽（Mermaid）

下圖為求可讀性，將 S2–S3、S4–S5 各合併為一個節點；完整七步驟與各自證據等級見上節。

```mermaid
flowchart TB
    subgraph REAL["攻擊者真實行動"]
        R1["S1 離線定調<br/>範本＋常設指令＋談話要點<br/>F2"]
        R2["S2–S3 文風克隆＋依範本生成<br/>簡報／廣播稿／貼文／圖像<br/>要求去除AI格式痕跡<br/>F7"]
        R3["S4–S5 HR忠誠基礎設施＋政治控制周邊產品<br/>效忠合約／評分量表／監控資料／偽造公文<br/>F4／F2"]
        R4["S6 拒絕後改框續推<br/>武裝分子指控改為匿名消息來源<br/>F4"]
        R5["S7 洗白式分發<br/>FM電台到Telegram到在地媒體到國家電台<br/>Claude不參與"]
        R1 --> R2 --> R3 --> R4 --> R5
    end
    subgraph SEE["模型每步看到的"]
        M1["一件像新聞或行政工作的獨立任務<br/>文風研究／寫稿／HR文件／資料整理"]
        M2["換句話說的新聞查證用語<br/>匿名消息來源"]
    end
    subgraph GAP["模型看不到的落差"]
        G1["意圖落差<br/>在地報導、或外國指揮的FIMI"]
        G2["真偽落差<br/>在地聲音頻道實為大使館轉發克隆"]
        G3["來源血緣與規模協同落差<br/>離線範本重用、電台聯播<br/>都在模型視野之外"]
    end
    subgraph DET["偵測與防護切點"]
        D1["文風同源比對<br/>頻道與廣播稿的stylometry"]
        D2["輸出端獨立分類＋拒絕升級規則<br/>不因換標籤放行、拒絕與政治HR文件合併告警"]
        D3["跨會期範本與常設指令重用偵測"]
    end
    R2 -.-> M1
    R3 -.-> M1
    ...
    style R3 fill:#ffe8e8
    style R4 fill:#ffe8e8
    style R5 fill:#ffe8e8
    style M1 fill:#fff0d0
    style M2 fill:#fff0d0
...
把這套手法搬到學員自架的地端 LLM 上，風險不是「模型被越獄講出危險知識」，而是模型被日常地用成一整條宣傳編輯台的隱形勞動力——寫合約、打考績、潤稿、換人設，每一步都能通過任何以「毒性／危險知識」為中心設計的內容分類器，因為這些任務本身完全無毒。裸模型完全不會多想；更麻煩的是，就算裝了分類器，本案證明只看「當下這一句話」的分類器，對「跨請求物件一致性」（同一份範本、同一套評分表）與「跨層洗白」（風格克隆 → 去 AI 化 → 地方媒體轉載 → 電台播出）幾乎無感。地端部署若讓 LLM 參與任何會被公開發布或播出的內容產製，必須把「這份輸出最終去哪裡、代表誰說話」當成獨立的風險維度來管，不能只問「這句話本身安不安全」。

**地端防護重點**（對映四層 playbook）

1. **輸出層（抗 F7）**：把「去除 AI 格式痕跡」「模仿某真實帳號／頻道語氣」這類請求單獨列為可疑訊號——尤其是「先貼一大段既有文本要求風格克隆，再要求另一段文字去 AI 化套用同一風格」的組合（對應本案 Figure 1「exported for stylistic voice analysis, cloning and distribution」），比單看任一半更具識別力。
2. **輸出層（抗 F4）**：判用途與下游對象，不判表面標籤。任何會直接影響具名真人僱傭或人身安全指控的輸出（效忠合約、考績開除建議、武裝分子指控），即使標籤已換成中性 KPI 或「匿名消息來源」，也應維持原風險等級再審，不因換皮而降級。
3. **會話層（抗 F2）**：建立跨會期的範本／常設指令相似度偵測。本案規劃全在平台外完成，送進模型的每個子任務單看都無害，只有把「同一套範本、同一組指令跨大量會期重複出現」的模式聚合起來才看得出協同；地端部署可對同帳號的 prompt 做結構相似度聚類，而非逐請求判斷。
4. **架構層**：涉及 HR 決策（忠誠合約、考績開除建議）與具名真人指控的雙重用途能力，走已驗證身分／授權閘，不開放給一般用途助手直接產出；把「這個輸出會不會影響真人的工作或人身安全」列為需要人工覆核的門檻，不靠模型自行判斷語氣是否中性。
5. **架構層＋觀測面**：認清地端防護的天花板——本案真正的殺傷力發生在 FM 廣播與電台聯播，完全在任何 AI 供應商或本地部署的視野之外。若已知輸出會流向可公開播出的通路，防護重點須同時涵蓋「生產前的身分授權」與「配合下游同源性稽核」（做法可參考 §8.5 的跨頻道文風比對思路），單靠模型內建分類器觸及不到分發層。
6. **會話層＋架構層（拒絕升級）**：把「模型拒絕點名具名真人」這類單一高風險拒絕，與同帳號近期是否曾送出政治意味的 HR／評分文件關聯起來，合併觸發帳號層級覆核——本案顯示單次拒絕若不升級，不會自動觸發後續調查（§8.4 缺口 4）。
