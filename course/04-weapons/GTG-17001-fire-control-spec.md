...
**本案疑似用到的手法族**
- **F4 良性／防禦性改框**：把武器系統的工程協助框定為一般工程／模擬／研究，讓請求落在看似正當的一側 — 證據等級 ★☆☆（報告未逐字記載框架；但報告明說 Anthropic 已新增分類器偵測並封鎖武器開發請求，反證攻擊者需要框架來規避）。
- **F2 任務拆解**：把「產出一份射控規格」拆成一連串看似中性的工程子問題（訊號處理、座標轉換、時序），單看每步都不像武器 — ★☆☆。

**對地端 LLM 的意義** 學員若自架「工程助手」型地端 LLM，它沒有 Anthropic 那條武器開發分類器；上述「拆成中性工程子問題」的請求會被直接回答，因為裸模型不會把分散的子任務拼回「這是射控系統」。

**地端防護重點**（對映四層 playbook）
1. **會話層（抵 F2）**：對工程類地端模型做跨請求主題聚合，把一連串工程子問題還原成「整體在造什麼」，而非逐題判斷。
2. **架構層（身分閘）**：武器級雙重用途工程能力，用經驗證身分＋用途授權放行（呼應第五節、CVP），不靠 prompt 內容。
3. **輸出層**：對「完整系統規格／整合設計」類輸出設人工覆核關卡。
4. **治理**：地端模型使用納入機構的出口管制與雙重用途研究審查。
