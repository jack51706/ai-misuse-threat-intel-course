...
報告未逐字記載本案提示；但報告明說 Anthropic **已新增分類器偵測並封鎖武器開發請求**，反推攻擊者必須用「框架＋拆解」才能取得協助。合理的操作序列是：

1. **立情境框架（F4，★☆☆）**：把射控系統的工程協助包裝成「一般訊號處理／控制系統／學術模擬」，讓每個請求落在看似正當的一側。
2. **拆解為中性工程子問題（F2，★☆☆）**：座標轉換、濾波、時序同步、彈道解算——**單看每一題都與合法工程無異**，武器語意被拆散。
3. **攻擊者端整合（★☆☆）**：把子答案在自己這邊拼回一份完整射控規格；**「整合成武器系統」這個動作不在任何單一請求裡**。

### 為何對模型的推論有效

內容分類器判斷的是**單一請求的表面內容**。「這個座標轉換怎麼算」與合法工程逐字元相同，惡意只存在於「這些子答案被整合成一套射控系統」的攻擊者端意圖——而那**不在模型視野內**。這與 GTG-10007 附錄 H 的雙重用途落差同源，只是領域換成武器工程。

```mermaid
flowchart TB
    G["武器系統工程目標<br/>（射控規格）"] --> DEC["拆成中性工程子問題<br/>座標轉換·濾波·時序（多次請求）"]
    DEC -->|"每題只送這個"| SEE["模型每題看到的：<br/>一般工程問題，與合法研究無異"]
    SEE --> C1{"內容/武器分類器<br/>逐題判斷"}
    C1 -->|"中性子題 → 放行"| PASS["框架＋拆解成功繞過"]
    C1 -->|"出現武器關鍵詞 → 攔"| BLOCK["分類器攔下（本案分類器已上線）"]
    SEE -.->|"這三者不在單題裡"| GAP["落差：整合意圖／授權／最終用途"]
    PASS --> INT["攻擊者端整合成武器系統"]
    ID["治理防線：身分閘＋跨請求主題聚合＋交付前人工審核"] -.->|"框架冒充不了驗證授權"| C1
...
學員若自架「工程助手」型地端 LLM，它**沒有那條武器開發分類器**；上述「拆成中性子題」的請求會被直接回答，裸模型不會把分散子任務拼回「這是射控系統」。四層防護：

1. **會話層（抵 F2）**：對工程類地端模型做**跨請求主題聚合**，把一連串工程子問題還原成「整體在造什麼」，而非逐題判斷。
2. **架構層（身分閘）**：武器級雙重用途工程能力，用**經驗證身分＋用途授權**放行，不靠 prompt 內容——這是唯一能擋「框架」的防線。
3. **輸出層**：對「完整系統規格／整合設計」類輸出設**人工覆核**關卡。
4. **治理**：地端模型使用納入機構的**出口管制與雙重用途研究審查**。
