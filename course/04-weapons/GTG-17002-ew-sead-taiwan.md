...
本案最具指標性的行為（報告記載）是**專案中途把模擬預設情境換成鎖定台灣 12 目標**。據此重建操作序列：

1. **立「模擬／想定研究」框架（F4，★★☆）**：把電子戰／防空壓制套件的建置框定為演習、想定、學術模擬——一個廣被接受的正當外殼。
2. **模組化工程子任務（F2，★☆☆）**：以訊號、頻譜、時序等中性子題推進，惡意目標藏在「情境參數」而非請求文字。
3. **情境參數替換（★★☆，報告記載此行為）**：**把演習的假想情境換成真實台灣目標座標與部隊番號**——同一套工程、換一組參數就從演習變成對台目標清單。

### 為何對模型的推論有效

模型看到的是「模擬」框架與中性工程子題；**「情境參數指向真實敏感標的」這個關鍵轉折不在單一請求的表面語意裡**。這正是 F4＋參數化的威力：框架讓整段對話看似正當，惡意集中在攻擊者可隨時替換的參數。

```mermaid
flowchart TB
    F["框成『模擬／想定研究』<br/>正當外殼（F4）"] --> SUB["模組化工程子任務<br/>訊號·頻譜·時序（F2）"]
    SUB --> SWAP["情境參數替換<br/>假想情境 → 真實台灣目標座標/番號"]
    SWAP -->|"模型每步看到"| SEE["『這是一場模擬』<br/>＋中性工程問題"]
    SEE --> C1{"分類器逐題判斷"}
    C1 -->|"看起來是演習 → 放行"| PASS["繞過"]
    SEE -.->|"不在請求裡"| GAP["落差：參數背後是真實打擊目標"]
    ID["治理防線：偵測『模擬情境被換成真實敏感標的』<br/>＋軍事雙重用途綁身分授權"] -.-> C1
...
「換一個情境參數就從演習變成對台目標清單」正是裸地端模型的盲點——學員自架的兵推／模擬助手若無防線會照做。四層防護：

1. **輸入／會話層**：對「模擬／想定」類請求，偵測**情境參數被替換成真實敏感標的**（真實座標、真實部隊番號、真實設施名）的訊號並升高審查。
2. **架構層（身分閘）**：軍事相關雙重用途能力綁**經驗證機構身分與授權範圍**。
3. **治理（呼應台灣意涵）**：把「用商業/開源 AI 做軍事想定」納入國防資訊治理與 OPSEC 規範；地端模型尤須，因為它連「拒絕武器請求」的分類器都沒有。
