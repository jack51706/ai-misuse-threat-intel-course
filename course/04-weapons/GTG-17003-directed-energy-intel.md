...
**本案疑似用到的手法族**
- **F4 良性改框＋F2 拆解**：把對「具安全許可工程師建檔（HUMINT 前置）」與定向能情報蒐集，框定為一般人物研究／技術調查，並拆成中性子任務 — ★☆☆。
- **F5 工具／記憶中介**：以情報循環的工作流編排推進（見 Figure 7 判讀） — ★★☆。

**對地端 LLM 的意義** 「對特定人員建檔」搬到裸地端模型上，會被當成一般 OSINT／人物整理照做——地端模型沒有「這是對受保護人員的側寫」這層判斷。

**地端防護重點**（對映四層 playbook）
1. **輸出層（抵 F4）**：偵測「對具名個人／受保護族群做系統性側寫建檔」的請求與輸出，獨立於其表面框架。
2. **會話層（抵 F2）**：把分散的人物查詢聚合，識別「在對同一組人逐步建檔」。
3. **治理**：機構層對「AI 輔助人員情報」的紅線與稽核。
