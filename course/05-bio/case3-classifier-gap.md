...
**本案疑似用到的手法族**
- **F4 良性／防禦性改框（本案是報告支持的範例）**：報告 p.135 逐字指出，因研究的敘述**明確聚焦於「減毒（attenuation）」這個看似良性的面向**，Claude 據此提供資訊、**未被分類器攔下** — 證據等級 **★★★（報告有原文）**。這是 F4「同一份雙重用途知識、換個良性框架就通過」最清楚的一案。
- **F2／存取**：以**轉售中繼帳號**（一對多、服務十多名客戶）稀釋帳號層風險 — ★★☆。

**對地端 LLM 的意義** F4 攻擊的是「用內容判斷意圖」這個判準本身——裸地端模型同樣用內容判斷，因此**同樣會被良性框架繞過**。加再多規則也擋不掉，因為善用與惡用的文字相同（報告 p.137 的原理性上限）。

**地端防護重點**（對映四層 playbook）
1. **輸出層＋判用途不判框架（抵 F4）**：對雙重用途領域，判斷**下游用途與整體脈絡**而非表面「減毒/防禦」措辭；必要時降載。
2. **會話層**：關聯中繼帳號背後的多使用者（一對多結構會稀釋你的偵測）。
3. **治理（決定性）**：本案正是報告推「可信任使用者審核」的最強論據——把守護點從內容移到經驗證身分。地端更需如此，因為它連分類器都沒有。
