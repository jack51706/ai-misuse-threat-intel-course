...
**本案疑似用到的手法族**
- **手法反例（重要）**：本案的行為者**意圖外顯**，因此被生物安全分類器直接攔下並啟動威脅調查——正是報告 p.131「露出明顯惡意，往往代表這個行為者不夠老練」的示範。也就是說，本案**幾乎沒有成功的提示操作**，是「不用 F 族、硬闖被擋」的對照組 — 證據等級 ★★☆。
- 存取面走**路徑 A**（ZDR 轉售平台＋美國 VPS），非提示操作。

**對地端 LLM 的意義** 攔下本案的是 Anthropic 的**生物安全分類器**。學員自架的裸開源 LLM **沒有這層分類器**——同一個「意圖外顯」的請求，在地端模型上會被直接回答。本案最大的地端教訓是：**你以為模型會擋的，裸模型不會擋**。
