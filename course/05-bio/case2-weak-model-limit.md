...
> 依 `../_shared/02-claude-safeguards-and-bypass-paths.md` 第九節的七大手法族（F1–F7）與四層地端防護 playbook。**本模組維持治理／偵測視角，全程不記述任何生物技術內容**；本節只談框架手法的樣態、偵測與地端模型的防護治理。

**本案疑似用到的手法族**
- **F1 人設／可信機構情境**：行為者具真實研究背景，在「可信的機構脈絡」下推進，使請求看似正當 — ★★☆（報告記載其機構情境與存取規避）。
- 存取面走**路徑 A**（美國 VPS＋隱私郵件＋自動生成帳號名）規避地理與身分管制。分類器最終未攔截、而是**降載到最弱模型**壓制能力提升。

**對地端 LLM 的意義** Anthropic 的防線是「分級降載——讓你用，但只給最弱模型」。學員自架**單一開源模型時沒有分級可降**：要嘛給全部能力、要嘛全擋。本案顯示「模型分級」是一種裸地端模型缺席的重要防線。

**地端防護重點**（對映四層 playbook）
1. **輸出層（能力降載）**：對高風險領域，地端可仿效「降載」——把敏感請求路由到**能力較弱的模型或受限模式**，而非全能力回應。
2. **架構層（身分閘）**：可信機構情境無法從內容判斷真偽，需**帳號外的機構驗證**。
3. **治理**：呼應本案外溢效應——嚴防護把需求推向弱/無防護模型，機構自架時要接住這個風險，而非複製它。
