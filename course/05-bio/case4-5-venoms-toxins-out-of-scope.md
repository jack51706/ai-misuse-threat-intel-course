...
**本案疑似用到的手法族**
- **F4 良性／治療改框**：兩案都把新化合物研究**框定為治療目標**（止痛、抗憂鬱等），使其落在分類器「設計上不涵蓋」的範圍 — ★★☆。
- **F7 輸出格式操縱（本案是報告支持的範例）**：Case 5 的研究者**刻意指示 Claude 對敏感標的保持「低擬真度（low fidelity）」**、模糊化描述——主動要求模型降低輸出精細度以規避 — 證據等級 **★★★（報告有原文）**。
- 屬**路徑 C（設計上不涵蓋）**：分類器刻意不管此範圍，非漏接。

**對地端 LLM 的意義** F7「要求模型自我模糊化」與 F4「治療框架」在裸地端模型上一樣有效，且地端模型**連 Anthropic 那條刻意的範圍界定都沒有**——範圍外的雙重用途研究在地端模型上完全無阻。
...
3. **治理（最終防線）**：報告結論——分類器不能是唯一防線，必須配「安全過濾＋可信任存取」。地端模型既無過濾也無存取審核時，機構的 IBC/雙重用途審查就是唯一防線。
