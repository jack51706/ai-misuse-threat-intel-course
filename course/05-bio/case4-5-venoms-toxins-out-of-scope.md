...
### 推測的操作序列（治理層重建）——F7 有報告逐字支持的一案

兩案都把新化合物研究**框定為治療目標**，落在分類器「設計上不涵蓋」的範圍；Case 5 更**主動要求模型模糊化**。操作樣態：

1. **治療框架（F4，★★☆）**：把新化合物研究框成止痛、抗憂鬱等醫療用途，落在分類器刻意不管的範圍（路徑 C）。
2. **輸出格式操縱（F7，★★★，報告有原文）**：Case 5 的研究者**刻意指示 Claude 對敏感標的保持「低擬真度（low fidelity）」**、模糊化描述——主動要求模型降低輸出精細度以規避。
3. **規避意識（★★☆）**：Case 4 引用雙重用途文獻、Case 5 主動要求模糊化，都顯示行為者知道自己在做什麼。

### 為何對模型的推論有效

F4 讓研究落在分類器**刻意不涵蓋**的範圍（政策留白，非漏接）；F7 進一步讓行為者**主動塑形輸出**以避開任何格式敏感的偵測。兩者疊加，內容層幾乎無從施力。

```mermaid
flowchart TB
    REQ["新化合物研究請求"] --> F4["框成治療目標（F4）"]
    F4 --> SCOPE{"落在分類器設計範圍內?"}
    SCOPE -->|"否：新化合物/專業雙重用途（路徑C）"| OUT["設計上不涵蓋 → 放行"]
    REQ --> F7["主動要求『低擬真度』模糊化（F7）"]
    F7 -->|"塑形輸出避開格式偵測"| OUT
    OUT --> GAP["共同缺口：意圖無法從內容判斷<br/>＋範圍界定的政策留白"]
    GAP --> FIX["需要：安全過濾＋可信任存取＋機構審查<br/>（分類器不能是唯一防線）"]
    style OUT fill:#f0d0d0
...
F7「要求模型自我模糊化」與 F4「治療框架」在裸地端模型上一樣有效，且地端模型**連 Anthropic 那條刻意的範圍界定都沒有**。四層防護：

1. **輸出層（抵 F7）**：偵測「要求降低擬真度／模糊化敏感標的／移除警語」這類**對輸出格式的操縱請求**，獨立於請求的表面正當性。
2. **判用途不判框架（抵 F4）**：治療框架不能作為放行依據；雙重用途毒素研究需機構層審查。
3. **治理（最終防線）**：報告結論——分類器不能是唯一防線，須配「安全過濾＋可信任存取」。地端既無過濾也無存取審核時，機構的 IBC／雙重用途審查就是唯一防線。
