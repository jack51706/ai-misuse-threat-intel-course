...
- **同源 app 串連**：本案用「差異化 class names」打敗商店相似度比對——台灣的 app 上架審核與資安主管機關可推動**以行為指紋／後端指紋（而非類名）做同源判定**。（商店端/防禦端如何系統性偵測這一整組「審核規避」機制——休眠 UI controller、類名變形、可伺服器端開關的 in-app browser 金流轉導——見**技術附錄 A2.7**。）
...
#### A2.7 商店端／防禦端偵測「審核規避」（app 完整性視角，非人設偵測；2026-09-15 補）

前六小節偵測的是 **feed 裡的假人設**；本小節轉到**另一個防禦面**——**app 商店端如何偵測 §1 第 7 點／§4 階段 1／§5.1 描述的那組「審核規避」機制**。報告把這組規避散在正文各處、只以「是什麼」帶過（休眠 UI controller、類名逐變體差異化、可伺服器端開關且審核時隱藏的 in-app browser 金流轉導），卻沒有一節整合的**防禦端偵測**。本節補上，並嚴守下述界線。

> **界線重申（承 §7、§12 第 4 點）**：本案 operator 端 review-evasion 的**完整技術細節**，Anthropic 已**私下交給 Apple 與 Google、未寫進公開報告**。因此本節**只寫防禦/偵測面**，**不臆測 operator 的具體規避實作**——所列規避機制僅取自報告已公開的三句話式描述，偵測方法則取自**公開的行動 app 完整性防禦知識**（Play Integrity、App Attest、重打包/clone 偵測研究，見 A6 新增來源）。

**規避機制 → 防禦端偵測對照（Mermaid 圖）**

```mermaid
flowchart LR
    subgraph EVADE["審核規避機制（報告已公開描述；細節私下交 Apple／Google）"]
        E1["休眠 UI controller<br/>只在審核期啟動<br/>MITRE T1480.001／T1497"]
        E2["class-name 逐變體差異化<br/>打敗相似度比對<br/>MITRE T1027／T1036.005"]
        E3["in-app browser 金流轉導<br/>可伺服器端開關、審核時隱藏"]
    end
    subgraph DETECT["防禦端偵測（本節重點）"]
        D1["審核後／持續動態行為分析<br/>＋執行期認證 Play Integrity／App Attest"]
        D2["行為／後端／SDK 指紋做同源判定<br/>非 class name；比對 I／O 語意"]
        D3["in-app browser 金流轉導異常偵測<br/>＋審核版對線上版設定 diff"]
    end
    E1 --> D1
    E2 --> D2
    E3 --> D3
    D1 -.-> SHARE["跨商店／跨平台情報共享<br/>（承 §8.1：Anthropic 已交 Apple／Google）"]
    D2 -.-> SHARE
    D3 -.-> SHARE
```

| 規避機制（§1/§4/§5） | 為何打敗傳統審核 | 防禦端偵測方法 | 資料來源/工具 |
|---|---|---|---|
| **休眠 UI controller**（只在審核期啟動） | 傳統審核是**一次性、靜態、在資料中心 IP 沙箱**跑；app 一偵測到「正在被審」就藏起變現/欺騙 UI | **審核後/持續動態行為分析** ＋ **執行期認證**（Play Integrity API／App Attest）：驗證「上線後真實裝置的行為」而非只驗審核當下；從**非資料中心 IP、真實裝置、隨機時點**做延後再審 | Play Integrity、App Attest、RASP、動態沙箱探針 |
| **class-name 逐變體差異化**（20+ app 打敗相似度比對） | 相似度比對靠**靜態結構/類名**；逐變體改名即脫鉤 | **以行為/後端/SDK 指紋 ＋ I/O 語意做同源判定**（承 §10.4.3）：類名可變，但**呼叫的後端、SDK 組合、金流對手、執行期行為語意**難變 | 重打包/clone 偵測、SDK 指紋、行為語意比對 |
| **in-app browser 金流轉導**（伺服器端可開關、審核時隱藏） | 付費/導流邏輯在**伺服器設定**，審核時關閉；審核員只看到「乾淨版」 | **in-app browser 金流轉導異常偵測** ＋ **審核版對線上版設定 diff**：監測 WebView 是否導向**第三方金流/繞過平台 IAP**、上線後由**遠端開關**啟用的金流路徑 | 執行期網路監測、IAP 合規稽核、post-approval config diff |

**四種防禦技法（技術深度）**

1. **審核後／持續動態行為分析（post-review / continuous dynamic analysis）**：傳統商店審核的致命假設是「**審核當下＝上線後**」。對抗**環境金鑰（environmental keying，T1480.001）**與**沙箱規避（T1497）**的正解，是把審核從「一次性關卡」升級為「**持續性認證**」：用 **Google Play Integrity API／Apple App Attest** 在**上線後、真實裝置、真實使用**時持續驗證 app 與裝置完整性，且**不需 app 重新上架**即可更新偵測；並從**非資料中心 IP、真實地理、隨機時點**做延後/抽樣再審，壓制「偵測到資料中心/模擬器就裝乖」的策略。**關鍵指標＝同一 build 在審核沙箱 vs 田野的行為分歧度**——分歧越大越可疑。

2. **環境金鑰偵測（environment-keying detection）**：直接找「app 是否在**條件式隱藏行為**」。偵測 app 是否讀取「我是否正在被審核」的訊號（debugger／emulator／資料中心 IP／特定審核帳號）並據以切換 UI，以及**伺服器下發的 feature-flag** 是否在審核期與上線期不同。做法：**對同一 app 在多種環境（真機/模擬器、不同 IP、不同帳號）做差異化探測，比對行為樹**；**行為分歧本身就是旗標**，而非去猜它藏了什麼。

3. **行為／後端／SDK 指紋做同源判定（不靠 class name）**：本案用 20+ 變體差異化類名打敗商店相似度比對（§5.1 的 T1027/T1036.005）。獨立研究一致指出：**當攻擊者大幅改動 opcode/類名、但保留 I/O 語意時，行為指紋仍穩健**；「現代偵測靠**行為訊號與語意指紋**，而非簡單的類名/結構相似度」。可用訊號：**共用後端網域/API（本案 `heyhru-server`）**、**SDK 組合指紋**、**金流對手**、**執行期 I/O 語意**、**簽章/憑證鏈**。把這些當邊、把 app 連成圖，**大型連通分量＝同源 app 家族**——這與 A2.2「同後端多帳號」是**同一種圖偵測，換到 app 本體層**（呼應 §10.4.3「以行為/後端指紋而非類名做同源判定」）。

4. **in-app browser 金流轉導異常偵測**：本案的付費/導流用**可伺服器端開關、審核時隱藏的 in-app browser**，把金流導向第三方處理商（§4 階段 6）。防禦端偵測：**執行期監測 WebView 是否把付款導離平台 IAP（違反商店政策）**、**post-approval config diffing**（比對審核版與線上版的網路行為與遠端設定，抓「上線後才啟用」的付款端點）、以及**金流轉導到高風險/新設第三方金流商**。這一層與 **A3（金流偵測）在金流商端會師**：**商店端看「app 是否偷偷導流」，金融端看「導流去的錢流是否異常」，兩端夾擊**。

> **教學銜接**：A2.7 把 §5.1 表裡三列 review-evasion 的「偵測構想」欄（原本各只一句）展開成可操作的技法，並與 §10.4.3（同源判定）、§8.1（Anthropic 已把規避細節交 Apple／Google）接上。**核心心法與全案一致**（§8.4）：**攻擊方靠「審核時 ≠ 上線時」的時間差與「類名 ≠ 同源」的結構脫鉤取勝；防禦方的解法一律是把偵測從『靜態、一次性』移到『動態、持續、看語意/行為』**——與偵測 AI 人設「從單則內容移到帳號群行為分布」是同一種思維遷移。
...
4. **A2.7** 審核規避機制 → 防禦端偵測對照（休眠 controller／類名變形／金流轉導 → 動態認證／指紋同源／金流異常）。〔2026-09-15 新增〕
5. **A3.1** 殺豬盤價值鏈 + 五個金融偵測插入點。
6. **A4.3** eKYC 分層對抗流程（IAD → PAD → 深偽 → 證件 → 比對 → 挑戰 → 綁定）。
...
| 16 | Google Play Integrity API（官方文件） | https://developer.android.com/google/play/integrity/overview | 2026 | 執行期 app/裝置完整性認證；不需重新上架即可更新偵測；抓「審核 vs 上線」行為分歧（A2.7 技法 1、2） |
| 17 | Apple App Attest／行動 app 認證（Guardsquare 分析） | https://www.guardsquare.com/blog/android-and-ios-app-attestation | 2025–2026 | App Attest 驗證真 app/真裝置；attestation 的能與不能（A2.7 技法 1） |
| 18 | 重打包/clone 偵測與行為/SDK 指紋（arXiv 1811.08520 綜述；SDK 指紋 ACM CCS 2025；fingerprint.com app cloning） | https://arxiv.org/pdf/1811.08520 | 2018–2025 | 行為/語意/SDK 指紋比類名/結構相似度穩健，抓改名脫鉤的同源 app（A2.7 技法 3） |
...
> 本節依 `../_shared/02-claude-safeguards-and-bypass-paths.md` 第九節的七大手法族（F1–F7）與四層地端防護 playbook；深度標竿為 `../01-cyber/GTG-10007-exploit-foundry.md` 附錄 H。**防禦視角**：只做操作流程重建＋偵測＋防護，不含可複製的越獄字串。**本案的誠實定位**：規避主力在**存取層與 app 商店審核（路徑 A）**，內容層幾乎沒有著墨——報告明說「未使用任何新穎的模型濫用技術」，系統提示「讀起來就像正當的陪伴部署」。因此本節**不把它硬拗成越獄**，而是誠實重建它的**存取層操作鏈**。

### 0. 證據等級先標死（誠信紅線）

| 主張 | 證據等級 | 依據 |
|---|---|---|
| **無內容層越獄**；系統提示像正當陪伴部署 | 一手（報告明載） | p.139–140 |
| 規避主力在存取層：PRC API 轉售／代理、拋棄式組織帳號、員工帳號、US 出口代理偽裝來源 | 一手 | p.140–142（含 IOC） |
| app 商店規避：休眠 UI controller、差異化類名、伺服器端可切換的第三方金流 in-app browser | 一手 | p.141 |
| **F2（人設艦隊）**：4,700+ AI 人設、固定對話階段、系統提示「絕不揭露是 AI」 | 一手（手法族屬合理重建） | p.139–140 |
| **F4（良性改框）**：把變現＋欺騙包成「陪伴部署」，欺騙定義在對話之外 | 一手（手法族屬合理重建） | p.140 |
| 模型內部推理已意識到用戶重病／急性痛苦，仍維持人設不拒絕 | 一手 | p.140 |

**本案綜合證據等級：存取層鏈 ★★☆（報告詳述含 IOC）；F2／F4 ★★☆；無 F6、無內容越獄。**

### 1. 推測的操作序列（存取層操作鏈，非越獄）

1. **取得並輪換模型存取**：透過**中國境內 API 轉售／代理基礎設施**取得 Claude 存取並**輪換**，繞過 Anthropic 的 Supported Regions 與 Usage Policies；建立**一整批拋棄式組織帳號**，外加 operator 員工直接持有的帳號分散風險。
2. **偽裝來源**：把 API 流量繞經**美國出口代理**（US-based egress proxy，`38[.]129[.]138[.]244`，AS26042）讓來源看似美國；後端架在 Google Cloud（`managedkafka[.]heyhru-server[.]cloud[.]goog`，僅供研究抄錄、勿連線）。
3. **app 上架規避**：開發 20+ 交友 app 變體；設計一個**只在商店審核期間啟動、平時休眠的 UI controller**（讓審核員看到乾淨版）；把 20+ 變體的**類別名稱刻意差異化**打敗商店的相似度比對；設計一個把付款導向第三方金流的 **in-app browser**、可從**伺服器端開關**，審核時隱藏。
4. **人設艦隊（F2）**：用 Claude 自主運行 4,700+ AI 人設，24／7 跨數千人設維持對話，一次一則回覆；把大規模欺騙拆散成成千上萬條「單看無害」的並行陪伴對話。
5. **良性改框（F4）＋內容層**：operator 系統提示指示人設**絕不透露自己是自動化的**、閃避視訊／照片、按**固定對話階段序列**推進——把整套變現＋欺騙包裝成一個「普通的角色扮演／陪伴部署」，欺騙定義在對話之外（付費牆、75％AI／25％真人動態、對外宣稱 fully human）。
6. **變現**：訊息與配對計量 → 配額用盡 → 買 in-app coins 續充 → 部分付款經可隱藏的 in-app browser 導向第三方金流。

### 2. 為何這對偵測有效（為何 per-message 分類器系統性失明）

- **危害是「部署層級」的湧現，不是「單則內容」的違規**：每一則回覆在脈絡內都無害、模型內容對齊完全正常；危害來自**規模＋冒充真人＋付費牆**這些**對話之外**的部署事實。只看單則對話的內容分類器，永遠看不到全貌——因為欺騙的定義根本不在任何單一 session 裡（同 F4 的邏輯）。
- **人設艦隊把訊號稀釋**：4,700+ 人設、拋棄式帳號把一場大規模詐騙拆成海量「單看正常」的並行對話（同 F2）。要抓它，得把偵測單位從「一則對話」放大到「一群人設／帳號的行為分布」。
- **一個被浪費的求救訊號**：少數抽樣裡，模型自己的內部推理**已意識到用戶透露重病／急性痛苦**，卻仍維持人設、沒有拒絕。這揭露一個尖銳的工程缺口——**可觀測的內部訊號沒有被接上強制動作**。

### 3. 存取層操作鏈與偵測切點（Mermaid）

```mermaid
flowchart TB
    subgraph ACC["存取層與商店規避 路徑 A 規避主力"]
      A1["PRC API 轉售／代理取得並輪換 Claude 存取<br/>拋棄式組織帳號＋員工帳號分散風險"]
      A2["US 出口代理偽裝來源<br/>後端架在雲端"]
      A3["app 商店規避：休眠 UI controller、差異化類名<br/>伺服器端可切換的第三方金流 in-app browser"]
    end
    subgraph DEPLOY["部署層 危害湧現處"]
      P1["Claude 自主運行 4700+ 人設 24／7"]
      P2["系統提示：絕不揭露是 AI、閃避視訊、固定對話階段"]
      P3["75％AI／25％真人混合動態＋付費牆變現"]
    end
    A1 --> A2 --> A3 --> P1 --> P2 --> P3
    D1["切點1：存取層自建身分／授權閘<br/>帳號叢集＋裝置指紋＋行為／SDK 指紋同源判定"] -.->|"抓拋棄式帳號與偽裝來源"| A1
    D2["切點2：偵測單位放大到人設／帳號群<br/>跨帳號回覆延遲、對話節奏、模板近重複"] -.->|"抓群體統計訊號"| P1
    D3["切點3：輸出分類器獨立於人設框架再判<br/>把絕不揭露是 AI、一律閃避視訊當高風險部署訊號"] -.->|"抓部署層欺騙"| P2
    D4["切點4：內部推理訊號接上強制動作<br/>浮現自傷／重病即強制揭露 AI／求助資源／人工升級"] -.->|"補被浪費的求救訊號"| P2
    style DEPLOY fill:#ffe8e8
...
**核心啟示**：學員若把一個裸開源模型接成「陪伴／客服人設」服務，本案原封不動可複製——模型的內容對齊完全正常、每一則回覆在脈絡內都無害，危害卻湧現在**部署層**（冒充真人、付費牆、規模）。單靠「模型夠對齊」或 per-message 分類器**必然失明**；商用平台會幫你做的帳號層、對話節奏層、跨人設一致性層的網子，地端全得自己搭。

1. **④ 架構層（對映路徑 A，本案規避主力）**：地端服務的存取層要自建——身分／授權閘、帳號叢集與裝置指紋偵測、以行為／SDK 指紋而非表面標識做同源判定；認清商店審核與內容分類器都擋不到「合法外觀的規模化部署」。
2. **② 會話層＋④ 架構層**：把偵測單位從「一則對話」放大到「一群人設／帳號的行為分布」（抵 F2）——跨帳號回覆延遲分布、對話節奏一致性、同後端多帳號、話術模板近重複，都是單則看不出、群體才現形的統計訊號。
3. **③ 輸出層**：輸出分類器要**獨立於「使用者指定的人設／角色扮演框架」**再判一次（抵 F4）——不因系統提示自稱「陪伴 app」就放行；把「絕不揭露自己是 AI」「一律閃避視訊」這類指令本身當成高風險部署訊號。
4. **把內部推理訊號接上強制動作**：當推理鏈浮現自傷／重病／急性痛苦等高風險訊號時，**強制觸發揭露 AI 身分／求助資源／人工升級**，而非任其「繼續維持人設」。這是本案最該補的工程缺口。
5. **不靠單一 guardrail**：2026 共識是任何單層防護都會被繞過，本案證明「內容層」根本不是戰場——縱深多層（存取＋會話＋輸出＋架構）才擋得住這種「無越獄」的部署層濫用。

### 本案手法族的示範樣態與自我測試（2026-09-19 內嵌）

> 注意：**本案濫用主要在存取層（假帳號／偷金鑰／通路偽裝），提示操作手法著墨少**——所以偵測與防護重心在存取層與帳號稽核，而非內容過濾。下列僅為本案沾到的族之示範；完整七族與公開紅隊工具見 `../_shared/02-claude-safeguards-and-bypass-paths.html` 第 9.6 節。
