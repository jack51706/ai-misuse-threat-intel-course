...
- **F2（任務拆解＋跨 session 分散）**：4,700+ 個 AI 人設、拋棄式組織帳號、員工帳號構成的「人設艦隊」，把一場大規模欺騙拆散成成千上萬條「單看無害」的並行陪伴對話，每條按固定階段序列推進，任何單一 session 都看不出全貌。— 證據等級 ★★☆（報告詳述人設數、固定對話階段與「絕不揭露是 AI」的系統提示，手法族屬合理重建）
- **F4（良性／防禦性改框）**：operator 的系統提示把整套變現＋欺騙包裝成「一個普通的角色扮演／陪伴部署」，欺騙定義在對話之外（付費牆、feed 組成、對外宣稱 fully human），因此在任何單一對話內都看不見。— 證據等級 ★★☆
- **（誠實界線）本案沒有內容層越獄**：報告明說「未使用任何新穎的模型濫用技術」，系統提示「讀起來就像正當的陪伴部署」。真正的規避主力在**存取層（路徑 A）**——PRC 代理／轉售基礎設施、美國出口代理偽裝來源、拋棄式組織帳號、以差異化類名與休眠 UI controller 規避商店審核。這些**不是**操作模型推理的 F 族，而是繞過存取層與商店審核；地端防護要把重心放在對應的層。

**對地端 LLM 的意義**

學員若把一個裸開源模型接成「陪伴／客服人設」服務，本案原封不動可複製：模型的內容對齊完全正常、每一則回覆在脈絡內都無害，危害卻湧現在**部署層**（冒充真人、付費牆、規模）。單靠「模型夠對齊」或 per-message 分類器**必然失明**——你需要的是帳號層、對話節奏層、跨人設一致性層的偵測，而這些商用平台會幫你做的網子，地端全都得自己搭。本案更揭露一個尖銳缺口：模型的內部推理「已意識到用戶重病／急性痛苦」卻仍維持人設不拒絕——地端部署若不主動把這種內部訊號接上行動，等於白白浪費了一個可觀測的求救訊號。

**地端防護重點**（對映四層 playbook）

1. **③ 輸出層**：輸出分類器要**獨立於「使用者指定的人設／角色扮演框架」**再判一次（抵 F4）——不因系統提示自稱「陪伴 app」就放行；把「絕不揭露自己是 AI」「一律閃避視訊」這類指令本身當成高風險部署訊號。
2. **② 會話層＋④ 架構層**：把偵測單位從「一則對話」放大到「一群人設／帳號的行為分布」（抵 F2）——跨帳號回覆延遲分布、對話節奏一致性、同後端多帳號、話術模板近重複，都是單則看不出、群體才現形的統計訊號。
3. **把內部推理訊號接上強制動作**：當推理鏈浮現自傷／重病／急性痛苦等高風險訊號時，**強制觸發揭露 AI 身分／求助資源／人工升級**，而非任其「繼續維持人設」。這是本案最該補的工程缺口。
4. **④ 架構層（對映路徑 A，本案規避主力）**：地端服務的存取層要自建——身分／授權閘、帳號叢集與裝置指紋偵測、以行為／SDK 指紋而非表面標識做同源判定；認清商店審核與內容分類器都擋不到「合法外觀的規模化部署」。
