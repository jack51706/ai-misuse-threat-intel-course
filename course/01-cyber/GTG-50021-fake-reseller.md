...
> 本節依 `../_shared/02-claude-safeguards-and-bypass-paths.md` 第九節的七大手法族（F1–F7）與四層地端防護 playbook，說明本案的濫用型態，並給自架開源 LLM 的防護重點。**誠實前提（本案最重要的一句話）：GTG-50021 是全六案裡最不適合畫「操作模型流程圖」的一個**——因為報告從未描述它用 Claude 執行任何攻擊步驟。**AI 在本案是商品、誘餌與戰利品，攻擊者根本不操作模型推論。** 因此本節不硬畫一條不存在的「越獄／驅動序列」，改畫**存取／供應鏈**，並把「為何不畫操作流程圖」本身當成一個教學點。防禦視角，不含可複製的越獄字串。

### 1. 為何本案不適合畫「操作流程圖」，改畫存取／供應鏈

前五案（10007／20006／50014／50020／50029）至少都有「攻擊者驅動某個模型（自己的或受害者的）」的動作可重建。GTG-50021 沒有：

- 報告描述的是一樁**詐騙＋憑證竊取的生意**，不是一次「模型被操作」的事件。硬套 F1–F7 或畫一條驅動序列，等於**捏造報告沒說的東西**——這正是課程誠信紅線要避免的。
- 唯一沾到提示操作的，是**同章 p.29 對「同類行為者」的通則觀察**：多名行為者對客戶自建 LiteLLM 做 prompt injection 竊生產金鑰（**F5** 的 gateway 面向）。但那是章節通則，**未必逐項適用於 GTG-50021 本身**——證據等級 ★☆☆（通則描述，非本案專屬）。
- **教學價值**：這種「AI 不是被操作、而是被當成詐騙標的與贓物」的型態，是報告「AI supply chain as target, loot, and attack compute」主線的純粹樣本。把它和 50014／50020 並讀，學員就能分清「濫用模型的**內容**」與「濫用模型的**存取供應鏈**」是兩種不同的威脅。

### 2. 推測的存取／供應鏈序列（把生意的每一步標出來）

每步標證據等級（★★★ 逐字原文／★★☆ 述行為、合理重建／★☆☆ 純推測）。本案主體全在存取層，故多為一手：

1. **以假 Claude 折扣轉售當誘餌（bait）**：化名 "kl1zy" 經營假折扣轉售站，吸引想省錢的使用者註冊。— 一手（報告 p.29）
2. **把流量靜默代理到別的模型＋植入憑證收割器（deceive）**：使用者以為在用 Claude，流量被偷偷導到別的（更便宜）模型；同時植入收割器。— 一手（報告 p.29、附錄 H 收割器機制）
3. **收割註冊者的 Anthropic 憑證（loot）**：把註冊者交出的金鑰／帳號當戰利品。— 一手（報告 p.29）
4. **偷來的金鑰／session token 當耗材輪替到用盡（compute／cover）**：轉售網路把贓憑證輪替使用，攻擊活動歸因到金鑰合法擁有者。— 一手（報告 p.38、附錄 I LLMjacking 鏈）

### 3. Mermaid：誠實版存取／供應鏈（非操作流程）

```mermaid
flowchart TB
    NOTE["本案 AI＝商品／誘餌／戰利品，攻擊者不操作模型推論<br/>無『越獄／驅動序列』可畫 → 改畫存取／供應鏈"]
    B1["① 假 Claude 折扣轉售站當誘餌<br/>化名 kl1zy・吸引省錢使用者 bait"]
    B1 --> B2["② 流量靜默代理到別的模型<br/>＋植入憑證收割器 deceive"]
    B2 --> B3["③ 收割註冊者 Anthropic 憑證 loot"]
    B3 --> B4["④ 贓金鑰／session token 當耗材輪替到用盡<br/>compute＋cover：歸因到金鑰主"]
    GEN["同章通則（非本案專屬・★☆☆）：<br/>他人對客戶自建 LiteLLM 做注入竊金鑰 F5 gateway 面向"]
    B4 -.->|"相鄰手法・僅供對照"| GEN
    DET["偵測切點（全在存取層與協定層）：<br/>①協定層驗真——thinking 簽章回驗 附錄 F／G<br/>②金鑰異常地理／ASN・用量離群<br/>③端點側收割器偵測 附錄 H"]
    B2 -.-> DET
    B4 -.-> DET
    style NOTE fill:#f5f5f5
    style DET fill:#e8f0ff
    style GEN fill:#fff0d0
```

### 4. 對地端 LLM 的意義 ＋ 地端防護重點

**意義**：學員最可能踩的坑是「模型存取供應鏈」——為省成本走灰市／非授權轉售，等於把金鑰與流量交給可能**靜默換模型、植入收割器**的中間人；而你若自架閘道，它本身就是別人 prompt injection 竊金鑰的單一劫掠點。本案提醒：**威脅不一定來自「模型被操作」，也可能來自「你取得模型的那條供應鏈」本身就是陷阱。**

**地端防護重點**（重心在存取層與架構層）
1. **架構層／採購**：只透過授權通路取得模型存取；灰市轉售站可能靜默換模型——用協定層指紋（如 thinking 簽章回驗，見附錄 F／G）驗真，別靠模型自我宣稱。
2. **架構層**：憑證事件＝端點事件——金鑰外洩要同時處理「金鑰」與「洩漏金鑰的端點」，否則收割器換一把新的照樣回傳（附錄 H.5）。
3. **架構層**：自建閘道（LiteLLM／OpenClaw）當正式邊界設備硬化（修補、認證、隔離、正式金鑰不放容器環境變數），避免一次倒出整排下游金鑰。
4. **存取層**：AI 金鑰＝正式憑證（秘密管理、輪替、最小權限、用量上限、IP 允許清單），壓縮 loot／compute／cover 的變現鏈（可部署偵測見附錄 J）。

### 本案手法族的示範樣態與自我測試（2026-09-19 內嵌）

> **本案 F1–F7 幾乎不適用**：AI 是商品／誘餌／戰利品，攻擊者並不操作模型的推論，濫用發生在存取層與供應鏈。防護重心請放在存取層（可信任來源採購、金鑰稽核、假轉售商偵測）。七族示範樣態與公開紅隊工具見 `../_shared/02-claude-safeguards-and-bypass-paths.html` 第 9.6 節。
