...
**本案疑似用到的手法族**
- **F2（任務拆解＋跨 session／子代理分散）**：多代理框架（PentAGI 類）以子代理分工做認證前後偵察、code review、跨模型 vetting findings，把軟體工程的 CI／code review 文化搬進攻擊流程 — 證據 ★★☆（報告述行為）。
- **F5（工具／記憶中介）**：agentic coding 框架＋自建 lab harness＋子代理編排，把「發現漏洞→寫 exploit→建靶→除錯」壓進同一次 session；並自架 Tor LLM-gateway 做第三方模型路由（Claude 擋了就換別家）— 證據 ★★☆（報告 p.34；IOC 表列 onion LLM-gateway）。
- **路徑 A（存取層規避）**：整月跑在偷來的 API 金鑰上、本地 proxy 輪替把攻擊流量混進合法擁有者流量、Tor gateway 跨供應商路由——每一項都為了讓「單一模型單一 session 的內容過濾」失效 — 證據 一手（報告 p.34、IOC）。

**對地端 LLM 的意義**　一個人用開源 agentic 框架就打出「國家級規模」並做出 fafsearch 這種 doxxing 聚合平台。同一套框架跑在地端自架模型上，供應商層的內容過濾與封號完全不存在——F2／F5 的自動化與地端「無網子」疊加，是最該讓學員警惕的組合。
