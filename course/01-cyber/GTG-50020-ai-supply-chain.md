...
> 第二階段技術深化增補（2026-09-14）。本附錄不改動前文任何內容，只把「技術高手能據以理解與防禦」的層次補齊：攻擊鏈的實際命令與 API 呼叫、CVE 的利用原理與受影響版本、示意等級的偵測規則範例（需依自家 schema、版本與基線驗證），以及用 Mermaid 重畫的關鍵流程。所有偵測規則僅供**防守**用途；IOC 一律保留 defang、嚴禁連線。
...
**偵測範例（示意；需依自家反向代理、欄位與正常流量驗證）**：

> 成熟度校正（2026-09-19）：下列規則是未經指定環境驗證的教學範例。[離線作業](../10-practice/05-50020-student.md) 僅驗證合成 job 事件的 Python 關聯，沒有執行提示注入、秘密存取或網路請求，也不驗證本節後端規則。
