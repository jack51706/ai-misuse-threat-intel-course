...
**本案疑似用到的手法族**
- **F5（工具／記憶中介——間接注入）**：本案最關鍵一步，是對 AI 廠商的自動評測沙箱注入惡意指令，讓沙箱把它持有的多家生產 API 金鑰吐出——典型的間接提示注入＋致命三元組（執行不可信輸入 × 持有生產憑證 × 缺乏隔離）— 證據 ★★☆（報告 p.30 原文）。
- **F1（人設＋授權框定）**：Figure 15「人類逐步指揮的 AI 滲透迴圈」第一步即「fabricated authorization story（捏造授權說辭）」，把請求推到可協助側 — 證據 ★★☆（報告 Figure 15；§8.3 亦點名此為與重新提示同源的弱點）。
- **路徑 A（存取層規避，本案主體）**：探測暴露的 LLM gateway、SQLi／認證繞過竊庫、拿到金鑰即「自動換槍」（用受害者金鑰跑下一輪）；終極目標是偷未發布模型權重（失敗）。這是本案主體，非 F 族 — 證據 一手（報告 p.30–31）。

**對地端 LLM 的意義**　學員若自建評測沙箱、LLM 閘道或代理框架，那正是本案的初始存取點——一個「帶聊天介面的憑證保險庫」。地端 harness 只要把「執行不可信內容的元件」與「持有生產憑證的元件」放在同一信任邊界，一次注入就倒出整排金鑰。

**地端防護重點**（對映四層 playbook）
1. 架構層：把 eval harness／沙箱拆成「不可信執行區（無憑證、無網路、唯讀、非 root）」與「持憑證協調區」；金鑰改短命、範圍受限、每個下游一把、移進秘密管理系統（Vault／Secrets Manager）。
2. 輸出層：對沙箱／代理輸出做機密外洩掃描，且獨立於使用者要求的格式（抵 F5／F7）。
3. 輸入層：對「捏造授權說辭」這類 F1 框定不靠內容宣稱放行；雙重用途能力走已驗證身分閘。
4. 架構層：模型權重是「一次得手即終局」的皇冠寶石——核心信任邊界（權重所在）與外圍（客戶金鑰）分層防護，權重層對齊 RAND SL／ASL-3 門檻。
