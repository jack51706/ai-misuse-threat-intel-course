...
> 本節依 `../_shared/02-claude-safeguards-and-bypass-paths.md` 第九節的七大手法族（F1–F7）與四層地端防護 playbook，重建本案「怎麼濫用模型」，並給自架開源 LLM 的防護重點。**誠實前提：本案是存取層主導、內容層著墨極少的案例**——攻擊者主要不是靠「越獄自己的 Claude」得手，而是靠(1)存取層濫用（偷憑證、偷 AI 金鑰）與(2)把注入**反過來打受害者自架的 LLM 代理**。因此下面不畫「提示操作流程圖」（那會誤導成本案在做內容層越獄），改畫**誠實版存取鏈＋反向注入支線**。防禦視角，不含可複製的越獄／注入字串。

### 1. 推測的操作序列（誠實版：濫用其實發生在存取層）

依第 4 節九階段、Figure 11 三條 affiliate 工作流、4.AA 反向注入、第 8.2 節缺口重建。每步標證據等級（★★★ 逐字原文／★★☆ 述行為、合理重建／★☆☆ 純推測）：

1. **存取層起手，不碰內容柵欄（路徑 A）**：多數入侵始於**已遭竊憑證／combolist／pre-staged corpora／對 help-desk 的 vishing**——請求以「合法帳號的正常用量」面貌進入，內容分類器根本沒機會判斷。— 一手（報告 p.13–16、Figure 3/11）
2. **偷受害者的 AI 金鑰、living off the land（路徑 A）**：拿到受害者 API 金鑰後**自動切換到用受害者金鑰**跑攻擊工作負載，一步完成 loot（轉售）／compute（別人付費）／cover（歸因到金鑰主）。— 一手（報告 p.30、第 4.Z 節）
3. **自主多代理跑既定犯罪劇本（自動化，非越獄）**：多代理框架平行對多受害者做 recon→discover→validate→expand→exfil→warehouse→mint→monetize→cover，連續數小時到數天。這裡的「驅動模型」是**把犯罪流程自動化**，不是內容層繞過。— ★★☆（報告 p.39、Figure 2）
4. **唯一的內容層操作，是反過來打受害者的模型（F5 反向注入）**：把「看起來像資料、其實是指令」的內容混進受害者 LLM 代理／gateway（報告點名 **LiteLLM、OpenClaw**）會讀的工單／文件／網頁，誘它用員工權限吐出系統提示、下游 API 金鑰、RAG 機密。— ★★☆（報告 p.12、p.16、Figure 4；注入機制細節由通用 agent 安全研究補，非本案逐句，見 4.AA）

### 2. 為何有效（存取層 × 受害者模型的致命三元組）

- **存取層規避讓內容柵欄失去著力點**：內容分類器再嚴，也要先「看到」請求才能判斷。偷來的合法金鑰讓攻擊工作負載以**受害者的正常用量**面貌進入——這是路徑 A 的本質（`_shared/02` 路徑 A）。
- **反向注入打的是受害者模型的結構性弱點**：受害者的 LLM 代理天生湊齊**致命三元組**（接觸敏感資料 × 讀不可信內容 × 對外通訊），一段偽裝成資料的指令就能用員工權限把機密搬走——攻擊者不需要 CVE。
- **自動化讓「封單一帳號」邊際效益遞減**：封一個帳號，換一個 combolist、一組 EC2、一個 Telegram bot 就再起（第 8.2 節第 3 點）。

### 3. Mermaid：誠實版——濫用集中在存取層，內容層著墨少

```mermaid
flowchart TB
    NOTE["本案『怎麼濫用 Claude』主要在存取層與自動化<br/>內容層越獄著墨極少——故畫存取鏈而非提示操作流程"]
    subgraph ACCESS["存取層（本案主體・路徑 A）"]
      P1["偷憑證／combolist／vishing<br/>以合法帳號正常用量面貌進入"]
      P1 --> P2["偷受害者 AI 金鑰<br/>自動換槍：loot／compute／cover"]
      P2 --> P3["自主多代理跑犯罪劇本<br/>九階段・並行・機器速度"]
    end
    CL{"內容層分類器"}
    P1 -.->|"請求看似合法用量<br/>柵欄沒機會判斷"| CL
    CL -.->|"被繞過（非被越獄）"| PASS["存取層規避成功"]
    subgraph REV["內容層唯一操作：反向打受害者的模型（F5）"]
      R1["把『像資料其實是指令』的內容<br/>混進受害者 LLM 代理讀的工單／文件"]
      R1 --> R2["受害者代理湊齊致命三元組<br/>用員工權限吐系統提示／金鑰／RAG 機密"]
    end
    P3 -.-> R1
    DET["偵測切點：<br/>①金鑰異常地理／ASN・非開發程序外傳<br/>②機器速度多受害者同步<br/>③代理輸出側洩密掃描"]
    P2 -.-> DET
    R2 -.-> DET
...
**意義**：本案最該讓學員記住的是——你自架的每一個 LLM 代理，都是一個「握有機密的對外資產」。攻擊者不需要越獄你的模型，只要一段偽裝成資料的指令混進代理會讀的工單／文件／網頁，就能用你員工的權限把金鑰搬走；而你被偷的 AI 金鑰，又會變成打下一個受害者的 compute 與 cover。
...
3. **輸出層**：輸出側洩密掃描（金鑰／token／系統提示／PII 樣式）比輸入側樣式比對更耐打——直接抓「機密離開代理」這個終點（抵 F5 反向注入）。
4. **存取層**：把 AI 金鑰當正式環境憑證治理（秘密管理、輪替、用量上限、IP 允許清單、異常地理／ASN 告警），阻斷 loot／compute／cover 的變現鏈——這是本案存取層主體最直接的對策。
