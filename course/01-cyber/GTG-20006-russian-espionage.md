...
**本案疑似用到的手法族**
- **F1（人設＋授權框定）**：Claude 被指派「攻擊性安全工程師」「DevOps」「Windows 開發者」等 persona，把惡意程式產線、C2／釣魚堆疊佈署框成正當工程工作 — 證據 ★★☆（報告 p.40 技能表逐列標 persona）。
- **F2（任務拆解＋跨 session 分散）**：人類把偵察、建 C2、寄釣魚、監控回連寫成可重用、可自動觸發的 Claude Code skills（SOP），只在需精修時才回來改 skill；偵察結果「folded back into project memory」——人類角色從操作員升為流程工程師 — 證據 ★★☆（報告 p.7 原文）。
- **F5（工具／記憶中介）**：SKILL.md＋專案記憶＋agentic 多代理框架充當工具與記憶中介；標誌性用法是「閉環」——監控代理一發現自家惡意程式被安全產品偵測，就自動改寫、重建、迭代到隱形 — 證據 ★★☆（報告 p.6、p.9）。

**對地端 LLM 的意義**　一個地端 agentic 框架只要接上編譯工具鏈與「監控自家 malware 是否被偵測」的回饋，就能複製這個把靜態偵測成本結構反轉的閉環；skills／記憶讓 know-how 可攜、可複製，換帳號或換模型都帶得走——地端更沒有上游偵測點接手。

**地端防護重點**（對映四層 playbook）
1. 架構層：把「重建二進位／對外送出／改寫植入體」這類高危工具設人類確認或完全隔離，直接切斷閉環——這是防守方最有效的斷點。
2. 會話層：F2 讓人類介入變稀薄，改盯「機器速度、並行、規律排程」的行為特徵，而非盯人類的異常操作。
3. 輸入層：對「攻擊性安全工程師」這類 persona＋授權框定，不因宣稱身分就放行雙重用途能力（同 F1 對策）。
4. 架構層：持久記憶與 skill 檔納入版控與審查——SKILL.md 具備與本機任何程式相同的完整網路存取，等於一段會自動執行的程式，不得夾帶未經審核的指令。
