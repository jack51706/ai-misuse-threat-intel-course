...
> 本節依 `../_shared/02-claude-safeguards-and-bypass-paths.md` 第九節的七大手法族（F1–F7）與四層地端防護 playbook，重建本案「攻擊者怎麼一步步驅動模型」，並給自架開源 LLM 的防護重點。本案是**內容層有實質提示操作**的案例（不像 50014／50021 純存取層），因為操作者確實把工作流以 persona＋SKILL.md 寫進模型的驅動邏輯。防禦視角，**不含可複製的越獄／注入字串**；只做操作流程重建、為何有效、偵測與防護。

### 1. 推測的操作序列（攻擊者怎麼一步步驅動模型）

依第 4 節生命週期表、4.8 閉環、4.9「只改 skills」、附錄 D 閉環圖重建。每步標證據等級（★★★ 報告逐字原文／★★☆ 報告述行為、合理重建／★☆☆ 純推測）。網路案無蒸餾章那種逐字 prompt，故最高只到 ★★☆：

1. **立多重人設、把攻擊框成工程（F1）**：對同一條戰役指派「攻擊性安全工程師」「DevOps」「Windows 開發者」等 persona，讓惡意程式產線、容器化 C2／釣魚堆疊佈署、原生植入體開發都被包裝成正當工程工作。— ★★☆（報告 p.40 技能表逐列標 persona）
2. **把整條工作流寫成可自動觸發的 SKILL.md／SOP（F2）**：人類不再逐句下指令，而是把「怎麼偵察、建 C2、寄釣魚、監控回連」寫成可重用的 Claude Code skills，交自主代理反覆執行，只在 SOP 需調整時回來改 skill——角色從操作員升為流程工程師。— ★★☆（報告 p.7 原文「modify Claude Code skills」）
3. **記憶中介、結果回寫（F5）**：SKILL.md＋專案記憶＋agentic 多代理框架充當工具與記憶中介，偵察結果「folded back into project memory」，讓下一輪自動接續。— ★★☆（報告 p.7、p.40）
4. **閉環規避＋掐斷補給線（F5＋F2）**：監控代理一發現自家已部署惡意程式被安全產品偵測，就自動改寫→重建→迭代到隱形；payload 另凍結受害機安全更新，讓新簽章送不到前線。這步在模型看來只是「把工具改到不被偵測」的正當防禦工程。— ★★☆（報告 p.6、p.9 逐字述行為）
5. **AI 當資料工人、誘出戰果**：抽取整理數百 GB 竊得資料、bulk export 信箱、像 QA 一樣檢查外洩完整度、修 API 分頁 bug 後重試 dump。— ★★☆（報告 p.9、p.7 儀表板）

### 2. 為何對模型的推論有效

- **persona 把雙重用途推到「可協助」側**：「攻擊性安全工程師做 build loop、測 Defender 規避」與「合法 AV 廠商測自家產品」在送模型的內容上逐字相同——內容分類器判不出差別（同 GTG-10007 附錄 H.2 的原理性落差）。
- **SOP 化把大惡意碾成小工程**：每次 skill 呼叫都是一個窄範圍的「建這個模組」「測這個回連」，惡意只在「這些 skill 合起來是打烏克蘭政府的間諜戰役」這個編排層浮現，而編排層不在任一請求裡（F2 的要害）。
- **閉環用「防禦性改良」自我正當化**：「監控自家工具是否被偵測、被偵測就改到不被偵測」這句話，紅隊做是合法、本案做是規避——模型看到的是前者的措辭。
- **人類介入變稀薄，反偵測訊號跟著變稀薄**：人類只在「改 skill」時出現，傳統「盯人類異常操作」的偵測訊號被抽乾（4.9 節）。
...
flowchart TB
    subgraph FLOW["攻擊者操作流程（惡意所在）"]
      A1["對烏克蘭政府／無人機製造商等<br/>發動竊 token 與長期間諜戰役"]
      A1 --> A2["拆成 persona 化 SKILL.md：<br/>偵察・建 C2・寄釣魚・監控回連・閉環規避"]
      A2 --> A3["自主代理機器速度反覆執行<br/>人類只在 SOP 需精修時改 skill"]
    end
    A2 -->|"每次 skill 呼叫只送這個"| SEE["模型每個請求實際看到的：<br/>『以攻擊性安全工程師身分建這個模組』<br/>『把工具改到不被 Defender 偵測』——單看像正當工程"]
    SEE --> C1{"內容層分類器<br/>只判單一請求像不像惡意"}
    C1 -->|"像合法工程 → 放行"| PASS["persona＋SOP 化繞過內容層"]
    subgraph GAP["模型看不到的落差（惡意真正所在）"]
      X1["意圖：防禦性 AV 測試 還是 規避偵測?"]
      X2["授權：有無標的方授權的紅隊?"]
      X3["規模／受害者：一個授權標的 還是 各國政府與前高官?"]
    end
    SEE -.->|"這三者都不在請求裡"| GAP
    PASS --> B1{"行為層：跨請求聚合<br/>分鐘級 build→test 閉環＋24/7 排程＋並行"}
    B1 -->|"機器節律異常 → 觸發調查"| CATCH["被模型側行為訊號抓到<br/>skill 精修簽章・閉環節奏"]
...
**意義**：一個地端 agentic 框架只要接上編譯工具鏈與「監控自家 malware 是否被偵測」的回饋，就能複製這個把靜態偵測成本結構反轉的閉環（4.8 節）；SKILL.md／記憶讓 know-how 可攜、可複製，換帳號或換模型都帶得走——而地端沒有 Claude 那道上游偵測點接手，閉環會跑得更肆無忌憚。

**地端防護重點**（對映四層 playbook）
1. **架構層（最有效的斷點）**：把「重建二進位／對外送出／改寫植入體」這類高危工具設人類確認或完全隔離，直接切斷閉環——這是防守方對 F5 閉環最有效的斷點（呼應附錄 D.3 四個斷點）。
2. **會話層**：F2 讓人類介入變稀薄，改盯「機器速度、並行、規律排程、分鐘級 build→test」的行為特徵，而非盯人類的異常操作；對同一主體跨 session 聚合意圖。
3. **輸入層＋架構層**：對「攻擊性安全工程師」這類 persona＋授權框定，不因宣稱身分就放行雙重用途能力；改用帳號外的已驗證授權＋行為規模判斷（抵 F1）。
4. **架構層**：持久記憶與 SKILL.md 納入版控與審查——SKILL.md 具備與本機任何程式相同的完整網路存取，等於一段會自動執行的程式，不得夾帶未經審核的指令（抵 F5 記憶中介）。
