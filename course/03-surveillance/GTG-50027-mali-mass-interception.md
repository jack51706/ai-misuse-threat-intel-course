...
- **F5（工具／記憶中介＋地端模型部署）**：平台是一個 agentic 系統工程專案（多代理編排），且**「完全在地端、以本地模型運行」**——Anthropic 封號只中斷了顧問的軟體與設計活動，**停不了已部署的平台本身**。— 證據等級 ★★☆
- **F4（良性／技術改框）**：「移除令狀檢查」可被包裝成「把某布林參數預設值從 true 改成 false」這種純技術／資料工程描述；「拒絕明確側寫、卻放行監控軟體工具請求」的不對稱同樣適用。— 證據等級 ★★☆

**對地端 LLM 的意義（本案是「地端 LLM 沒有內建防線」的活教材，特別強調）**：這是全課最尖銳的一案。當危害從「使用雲端 API」轉為「用 AI 做出可地端運行的系統」，帳號封鎖、速率限制、輸入／輸出分類器、Usage Policy、地區封鎖——所有作用在「API 邊界」上的管制**全部失效**。運行期改用地端開放權重模型後：沒有 API 呼叫、沒有帳號、沒有對外流量、沒有 IOC。**供應商的執法面 ＝ API 邊界；地端部署把危害搬到邊界之外，執法面整個消失。** 這正是本課「自架開源 LLM ＝ 裸模型、防線要你自己一層層搭」論點的最強證據。

**地端防護重點**（對映四層 playbook）

1. **④架構層（核心）**：認清「部署後不可收回」——開放權重一旦落地，能力就是那份權重檔本身，無法遠端撤銷、打補丁或停用（半邊界問題）。治理必須區分「管制**使用**」與「阻止**能力交付**」，後者一旦完成即不可逆。
2. **判準（怎麼認出這種案子）**：問三題——產出是一次性交付物還是持續依賴 API 的服務？成品能否無網路／地端運行？移除供應商後危害是否照常存在？三題都指向「交付物／可地端／照常存在」即本失效模式。
3. **③輸出層（抵 F4）**：對「移除法律／安全保障」的請求，不因它被包成「純技術參數變更」而放行——判元件的**作用**（拆除令狀檢查），而非表面措辭。
4. **治理層**：既然事後封鎖對地端成品無效，真正能改變結果的介入點必須落在「設計協助的當下」（拒絕建置無令狀元件）與「用途／資料的法律治理」（監控須有司法令狀）。**地端模型自己不會拒絕——防線只能來自部署者的架構與法制。**
