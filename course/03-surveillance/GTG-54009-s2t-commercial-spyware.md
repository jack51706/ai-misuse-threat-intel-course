...
> **本附錄性質**：這是對前述第 1–12 節的**技術增補**，不取代任何既有內容。前述章節（尤其第 4、5、6、8 節）已完整處理「發生了什麼、報告怎麼寫、圖表怎麼判讀」；本附錄補的是**技術高手要據以理解與防禦所需的實作層細節**——SOCMINT 產線怎麼被搭起來、合成帳號怎麼被辨識、人口分群的 ML 怎麼運作、商業監控產業的技術堆疊長什麼樣，以及**待環境驗證的偵測規則範例（示意等級）**。
>
...
> 本節依 `../_shared/02-claude-safeguards-and-bypass-paths.md` 第九節的七大手法族（F1–F7）與四層地端防護 playbook 就地深化。**防禦／人權視角；不寫可複製的越獄字串，也不寫可操作的監控建置**——只做操作流程重建、偵測與防護。與 §4（生命週期）、§8（防線缺口）互為裡外。

> **⚠️ 誠實紅線（本案與其他 7 案不同，務必先讀）**：本案正文**完全未描述任何規避手法**——§8.5 已明確盤點：沒有重新提示、沒有繞過分類器、沒有帳號輪替。章節導論的「every case… attempted to circumvent controls」是概括陳述，本案並未舉證；最可能的解讀是**行為者還在 pilot 階段、OPSEC 成熟度落後於能力成熟度**（§8.5 解讀 B）。因此以下 F 族屬**基於承包商商業性質的推測，證據等級一律 ★☆☆**。**本案真正的教學重點不是某個 prompt 技巧，而是一個架構性風險**——見 §三 Mermaid。

### 一、推測的操作序列：報告「看得到的那半條漏斗」

報告只親眼看到四階段產線的上半（Monitor→Segment/score→Persona→Report），下半（Infiltrate→Exploit）在視野之外、依據是 2023 年 S2T 型錄（§4.1）。依 §4 重建**報告觀察到的**步驟（F 族一律 ★☆☆＝純推測，因無規避描述）：

| # | 攻擊者這一步在做的事（報告觀察到） | 框定成的「商業版本」 | F 族 | 證據 |
|---|---|---|---|---|
| 02 | 一次餵約 25 則貼文，要模型回傳人口群體、城市級位置、政治傾向、**信度分數**（§4.2） | 「幫我做批次社群情緒與受眾分析」 | F4（商業改框） | ★☆☆ |
| — | 以波／阿／英／德四語為假人設生成貼文，**衝突兩邊都做**（§4.3，255+ avatar） | 「幫我做多語社群內容生成」 | F4 | ★☆☆ |
| Report | 產正式阿拉伯語、政府公文語域簡報，依波灣國籍拆分情緒＋**建議反制敘事**（§4.4） | 「幫我把分析寫成客戶要的簡報格式」 | F4 | ★☆☆ |
| 03/04 | 滲透封閉群組、收割名單、釣魚植入（**Anthropic 未觀察到**，§8.3） | ——（視野之外，依 S2T 型錄推斷） | —— | 外部型錄 |

**關鍵形態**：每個單看都像正當商業情報任務——這正是為何在 pilot 階段之外，它幾乎無從被內容分類器單獨辨識。**它被攔下靠的不是「露出惡意」，而是同一帳號同時做「批次側寫評分」＋「多人設兩邊代工」這個組合太反常**（§4.5.2 最後一列）。

### 二、為何本案的核心是「架構」而非「手法」

- **監控與影響力一體化**：本案把情報循環與資訊作戰之間本應有的組織／法律閘門拆掉了——同一次 API 呼叫序列同時產出「誰在想什麼」（情報）＋「該說什麼壓過去」（作戰建議）＋預備好的可投放帳號（作戰資產）（§4.5.1）。沒有任何單一法律框架涵蓋整條鏈。
- **上游瓶頸在階段 02**：從幾十萬則貼文收斂到「值得滲透的個人」，這一跳離不開分眾與評分——AI 供應商正好卡在這個收斂點，所以「在 pilot 攔下瓶頸」比攔任何其他環節都有效率（§8.3.3）。
- **但這個優勢是脆弱的**：報告自己（引 GTG-50027 語境，§8.3.4 第 2 點）承認——行為者可改用**地端開源模型完全繞過**：語音轉文字、情緒評分、人設生成都能在本地跑，一旦如此，上游偵測與封號全部歸零。**這不是 prompt 技巧能修的漏洞，而是「唯一可見節點可被移除」的結構問題。**

### 三、本案核心是架構性風險：地端開源模型完全繞過供應商防線

```mermaid
flowchart TB
    subgraph CHAIN["S2T 監控＋影響力產線（四階段）"]
      M["01 監控：抓取貼文"] --> S["02 分眾與評分：<br/>批次約25則→人口群體・城市級位置<br/>政治傾向・信度分數"]
      S --> P["03 人設生成：<br/>波/阿/英/德四語，衝突兩邊都做"]
      P --> R["04 產出：政府公文語域簡報<br/>依國籍拆分情緒＋建議反制敘事"]
    end
    subgraph NOW["現況：唯一具跨法域可見性的節點"]
      SUP["AI 供應商<br/>階段02是全鏈收斂點／上游瓶頸<br/>在 pilot 階段就攔下"]
    end
    S -->|"目前跑在 Claude 上"| SUP
    SUP -->|"封號＋與 S2T 追蹤夥伴分享指標"| STOP["本案在試營運被中止"]
    subgraph RISK["架構性風險：地端開源模型完全繞過"]
      LOCAL["把 02／03 搬到地端開源模型：<br/>語音轉文字・情緒分群・人設生成全在本地"]
      LOCAL --> ZERO["上游偵測與封號歸零<br/>沒有任何外部節點看得見全鏈"]
    end
    SUP -.->|"一旦地端模型成熟"| RISK
    ZERO --> GOV["唯一支點前移：<br/>採購稽核＋身分／授權閘，非事後封號"]
    style CHAIN fill:#ffe8e8
    style NOW fill:#e8ffe8
    style RISK fill:#f0e8ff
    style GOV fill:#e8f0ff
```

### 四、對地端 LLM 的意義

這條產業鏈上「唯一具跨法域可見性的節點是提供模型能力的 AI 供應商」——這也是它脆弱的原因：一旦地端開源模型成熟，把 S2T 型「監控＋反制敘事」產線搬到本地，上游偵測與封號全部歸零。學員自架同型模型，等於直接站在「無人能從外部看見」的那一端。本案能在 pilot 階段被攔，靠的是「研發過程用到雲端 LLM，研發本身就在別人的遙測裡」這個 AI 時代的結構性新機會（§8.3.3 第 5 點）——而地端開源模型正好把這個機會關掉。**這與 GTG-50027 是同一個失效模式的兩面：50027 是成品地端化、本案是研發地端化，兩者都讓供應商的執法面消失。**

### 五、地端防護重點（對映四層 playbook）

1. **④ 架構層**：認清「地端開源模型完全繞過」是**無法由單一供應商解決**的失效模式（同 GTG-50027）——問責支點必須前移到**採購稽核與身分驗證**，而非事後封號。這是本案唯一真正有效的防線落點。
2. **③ 輸出層（抵 F4）**：對「依國籍／群體拆分的情緒評分 ＋ 建議反制敘事」這種「先告訴客戶誰在想什麼、再告訴客戶該說什麼壓過去」的**組合輸出**設偵測——把人分群評分 ＋ 產出對抗敘事，幾乎無正當商業對應物；且要注意 LLM 自報的信度分數在無真值側寫任務上**校準關係完全未經驗證**（§4.2），本質是「有科學外觀的猜測」。
3. **② 會話層**：對「逐字轉錄大量私訊／通話語音 ＋ 情緒分群 ＋ 人設生成」在同一帳號共現做關聯——單項像商業分析，**共現**指向監控＋影響力代工（§4.5.2「兩種訊號在同一帳號出現反而是最強偵測特徵」）。
4. **治理層**：本案核心指控為單一來源，且屬「被報導過但未被技術鑑識」的供應商；地端化會讓這類供應商更難被任何外部節點看見。加上「跨供應商拼圖無人負責」（若行為者把分類與人設生成拆到不同供應商，沒有任何一方看得到全貌，§8.4），故防護重心在**身分／授權閘＋跨平台情報共享機制**（呼應第五節 CVP 與對台灣意涵）。
