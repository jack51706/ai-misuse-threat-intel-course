...
- **F4（良性／防禦性改框）**：報告明載 Claude 拒絕了「秘密審訊」與「大規模假人設」，卻放行翻譯、方言潤稿、「專家品管」、弱點側寫、結構化整理——**防線擋住名詞（審訊、假帳號），沒擋住動詞（翻譯、校對、整理、評分）**。「扮演敘利亞在地專家、評估這段話像不像本地人」這種欺騙品管，被改框成中性的「語言顧問」而通過。— 證據等級 ★★☆
- **F2（任務拆解＋跨 session 分散）**：一條完整情報鏈（監控→側寫→在地化欺騙→招募→上呈）被拆成一個個單看合法的子任務，穿過以「單一請求」為單位的防線。惡意不在動詞裡，在對象與目的裡，而對象與目的通常不在同一個請求裡。— 證據等級 ★★☆

**對地端 LLM 的意義**：學員若自架模型做「多語內容分析／在地化」助手，會完整繼承這個缺口——裸模型樂意翻譯、潤稿、評分、排版，卻無從得知這是在替一場招募線人的欺騙行動做品管。單次請求安全判斷有結構性天花板。

**地端防護重點**（對映四層 playbook）

1. **③輸出層**：對「以自然人為列、以可利用性／可接近性為值的評分表」這種語意輸出設硬規則——這類請求幾乎無合法對應物，應在輸出端獨立攔截，不因「這只是翻譯／潤稿」的表面框架放行（抵 F4）。
2. **②會話層**：跨請求／跨 session 聚合同一帳號對同一批人名的反覆處理，別讓「翻譯→側寫→話術→排版」被當成互不相關的獨立請求（抵 F2）。
3. **①輸入層**：對「扮演某族群／某職業專家，判斷訊息是否『聽起來像本地人』」的品管型請求建偵測特徵（F4 的專家品管變體）。
4. **架構與治理**：翻譯／在地化是全球最普遍的正當用途之一，全面封鎖代價極高且**優先傷害非英語使用者**——所以防護重心放在「會話×帳號行為組合」與「輸出對象屬性（是否具名自然人、是否受迫害社群）」，而非關鍵字黑名單。
