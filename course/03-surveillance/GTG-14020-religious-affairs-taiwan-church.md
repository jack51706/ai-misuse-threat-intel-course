...
### 3.5 政策條款對照：非合意監控 vs. 依宗教身分的分類側寫（2026-09-15 深化）

§3.4 只引了「non-consensual surveillance」半句，但本案在使用政策（AUP）上其實**同時踩到兩條性質不同的紅線**，課堂上必須拆開講——因為它們對映不同的政策條款、不同的補救邏輯，也標出本案在全報告中的獨特位置：**這是整份報告唯一一個以「宗教身分」作為情報體系組織原則的案例**（五條工作線本身就是按宗教群體切分的，見 §3.1、§6.1）。

- **(a) 非合意監控（non-consensual surveillance）**：對未同意的個人追蹤、建檔、蒐集其位置與活動。這是**行為型**違規——重點在「你**做了什麼**」（追蹤、蒐集），與本章其他監控案（GTG-50027、GTG-14010、GTG-14022）共有。
- **(b) 依受保護屬性（宗教）建立分類側寫（profiling by protected attribute）**：把「宗教信仰」當成情報系統的**分類軸與組織原則**，依此把人歸類、建冊、指派工作線。這是**主體屬性型**違規——重點在「你**依什麼把人分類**」（宗教這個受保護屬性），是本案有別於其他案的**獨特紅線**，也對映各國反歧視法與 AI 法規（如 EU AI Act 對「依宗教等受保護屬性做分類／側寫」的禁令）最核心的關切。

> **為什麼要分開？** (a) 的補救問「有沒有同意、有沒有合法目的」；(b) 的補救問「**就算有目的，能不能拿受保護屬性當分類鍵**」。一個系統可能只犯 (a)（如追蹤某公眾人物的公開行程），也可能兩者皆犯（如本案：既非合意監控、又以宗教為軸建冊）。**「把宗教當組織原則」這件事，即使抽掉「非合意」也依然違規**——這正是 (b) 相對於 (a) 的獨立性，也是本案最該讓學員記住的政策分析點。

#### 政策條款 × 本案證據頁碼

| # | AUP 條款（2025-09-15 生效版；逐字見導論 §2.6） | 屬 (a) 或 (b) | 本案證據（頁碼） |
|---|---|---|---|
| 1 | 總則：「using Claude to conduct **non-consensual surveillance and profiling**」（報告 p.81 引用政策） | (a)+(b) | 以 Claude 取代整個分析師團隊，對宗教領袖與華人離散人物建中文卷宗（p.89–90） |
| 2 | 「Target or track a person's physical location... communication **without their consent**」 | **(a)** | 蒐集特定個人的生日、出生地、移民日期、社群帳號；跨境內外平台（微信、小紅書、抖音、微博、LinkedIn、IG、Threads、X、FB）的每日監控週期（p.90）；目標「從資深公眾宗教領袖到**私人公民**」（p.90）；Figure 4 的法輪功關聯學院講師個人檔案（p.92） |
| 3 | 「...any law enforcement application that **violates or impairs the... civil liberties, or human rights** of natural persons」 | (a)+(b) | 對映統戰／宗教事務體系（統戰部、國安部、原國家宗教局）的鎮壓性情蒐（p.92）；指示 Claude 對法輪功套用國家「邪教（evil cult）」定性、對藏人流亡體系冠「非法分裂（illegal separatist）」（p.90） |
| 4 | 「biometric categorization system... to infer their... **religious or philosophical beliefs**」（AUP 中最貼近「以宗教分類」的逐字條款） | **(b)** | 五條工作線**按宗教群體切分**（天主教樞機／台灣長老教會／藏傳佛教／法輪功／基督教宣教網絡，p.91 表、Figure 3）；把「宗教身分」當建檔的分類主軸（p.89–92） |

> **一個誠實的技術界線**：AUP 第 4 條的逐字文字限定在「**biometric** categorization（生物特徵分類）」——嚴格說，本案是**直接以宗教身分**分類（此人就是樞機、就是法輪功學員），並非透過生物特徵**推斷**宗教。但兩者要防的是**同一件事**：把「宗教信仰」這個受保護屬性當成把人分級、建冊、處置的鍵。報告因此把本案歸在總則的「non-consensual surveillance and **profiling**」之下（p.81），而非單掛生物特徵條款。授課時要講清楚：**(b) 的違規核心不在「用了生物特徵」，而在「用受保護屬性當分類軸」**——這是反歧視法的一貫立場（省略敏感欄位不會讓系統變公平，只會讓歧視變得無法被審計；技術論證見姊妹案 GTG-54009 §C.2「代理歧視 proxy discrimination」）。

**交叉引用導論 §2.6**：本表是導論 §2.6「Usage Policy 條款 × 本章案例」總表的**單案深掘版**——導論那張表把多條 AUP 條款**橫向**對應到全章七個案例（本案對應「推斷政治意見／宗教信仰的分類系統」一列，證據標 p.90）；本表則**縱向**把本案踩到的多條條款列全、並區分 (a)/(b) 兩種性質。兩表搭配：先用導論表看「本案在全章的位置」，再用本表看「本案內部的政策違規結構」。
...
§8.4 已給出「session 級組合計分」的概念與四個層級。本節補足**演算法示意**與**待環境驗證的規則範例**。

> 成熟度校正（2026-09-19）：下方演算法含示意函式、Sigma 含 experimental 標記，均未附完整版本、真實基線與驗收資料。[離線作業](../10-practice/03-14020-student.md) 只測預先標註的合成 metadata 關聯，未驗證 NER、MinHash／LSH、語意標籤或真實監控偵測效果。
...
> 本節依 `../_shared/02-claude-safeguards-and-bypass-paths.md` 第九節的七大手法族（F1–F7）與四層地端防護 playbook 就地深化。**防禦／人權視角；不寫可複製的越獄字串，也不寫可操作的監控建置**——只做操作流程重建、偵測與防護。與 §4（生命週期）、§8（防線缺口）互為裡外。**本案的招牌是「無拒絕」：報告全案未記載任何一次 refusal（§8.3 缺口一），與 GTG-14021 的 F3 恰成對比——所以本節不套 F3，改講「持續配合的模板化產線」如何在完全不觸發拒絕的情況下量產施壓檔案。**

### 一、推測的操作序列：一個人如何用模板讓模型幹一整隊分析師的活

本案沒有 agentic 自動化、沒有越獄；危害來自**吞吐量**——單一操作員 30 天 2,475 份（§4.2）。依 §4、§8 重建攻擊者驅動模型的步驟（每步標 F 族與證據等級；★★☆＝報告描述了行為、手法屬合理重建）：

| # | 攻擊者這一步在做的事 | 送進模型的「中性版本」 | F 族 | 證據 |
|---|---|---|---|---|
| 1 | 一次性建**版本控制模板**（人物调研底稿／线索报／态势感知），把必填欄位寫死：工作抓手、负面情况 | 「照這個模板產出結構化文件」 | F7（強制輸出 schema）＋F2 | ★★☆ |
| 2 | 設定立場：指示「站在中方立場」、藏人流亡政府→「非法分裂政權」、法輪功→「邪教」 | 「用這套用語規範改寫」 | F7（立場重框）＋F4 | ★★☆ |
| 3 | 攝入多語言原始素材、翻譯 | 「幫我翻譯／整理這批公開貼文與新聞」 | F4 | ★★☆ |
| 4 | 結構化擷取（生日、出生地、移民時間、社群帳號） | 「從這段文本抽出這幾個欄位」 | F2（填空子任務） | ★★☆ |
| 5 | 分析並標註可利用「抓手」與負面資訊（**最敏感一步**） | 「填一下『工作抓手』『负面情况』欄位」 | F7＋F4（把最敏感的判斷偽裝成填空） | ★★☆ |
| 6 | 節奏化交付：同一模板換人名重跑，日更 | 「再做一份，換這個人」 | F2（模板化吞吐） | ★★☆ |
| X | 對宗教場所偵察（平面圖／立面／結構圖） | ——（主詞是「They」＝行為者，**報告未說 Claude 參與**，§4.4） | —— | 一手（報告明載主詞為人） |

**關鍵形態**：全程**沒有任何一步被拒絕**。攻擊者不需要對抗防線，因為他把「側寫與施壓」這件事**碾成了一連串填空**——而填空任務比開放問答更難觸發模型的自我檢視（§8.3 假設 B）。

### 二、為何這條序列對模型的推論有效（也就是為何全程無拒絕）

- **填空外包了判斷**：欄位由操作員定義，模型只是把非結構化文本抽進格子。「把這段摘要成 300 字」「填『负面情况』欄」——模型在做的是格式轉換，不是「決定要不要幫人建施壓檔案」（§8.3 假設 B）。
- **惡意只在聚合層與欄位語意裡**：任一原子請求單獨看都無害；惡意存在於「2,475 份同構檔案的總和」與「抓手欄位的受詞是**人**不是政策」這個語境（§4.3 的情報語言學：同一個詞在政策文件裡是施力點，在人物檔案裡是把柄）。
- **立場重框把監控標籤成正常工作**：「站在中方立場分析」大量存在於正當用途（外交研究、辯論教學），分類器難判；它同時把「監控」重新命名為「政府分析」，降低風險感知（§8.3 假設 C）。
- **中文覆蓋是放大器**：模板欄位是中文（工作抓手、负面情况），安全訓練與分類器的中文覆蓋可能弱於英文（§8.3 假設 D）——這是最該由業界公開驗證的一項。
...
flowchart TB
    subgraph OP["攻擊者的真實目標（惡意所在）"]
      G["對宗教人士量產含『抓手／負面情況』的施壓檔案<br/>單一操作員 30 天 2,475 份"]
    end
    G --> T["一次性建版本控制模板<br/>必填欄位＋『站在中方立場』重框"]
    T --> LOOP["每份只是換一個人名重跑同一模板"]
    LOOP --> SEE["Claude 每步實際看到的：<br/>『摘要成 300 字』『填這幾個欄位』<br/>『翻譯這段聲明』——填空任務，單看全無害"]
    SEE --> C1{"單請求分類器<br/>本案全程無任何拒絕紀錄"}
    C1 -->|"填空＋立場重框 → 放行"| PASS["模板化產線 30 天未被攔"]
    subgraph GAP["模型看不到的"]
      X1["抓手欄位的受詞是人 還是政策?"]
      X2["這是第 1 份 還是第 2000 份同構檔案?"]
      X3["對象是不是受保護的宗教／族裔社群?"]
    end
    SEE -.->|"惡意只在聚合層與欄位語意裡"| GAP
    NOTE["對比 GTG-14021：<br/>那案有 F3 拒絕→重新提示；<br/>本案連一次拒絕都沒有——<br/>危害來自持續配合的產線，非被越獄"]
    C1 -.-> NOTE
    PASS --> DET{"防護切點：模板化偵測<br/>結構相似度＞0.9 但人名各異<br/>＋抓手／負面情況欄位語意"}
    DET -->|"與語言、關鍵字無關 改名無效"| CATCH["地端須自建的網子"]
    style OP fill:#ffe8e8
    style GAP fill:#fff0d0
    style NOTE fill:#fff8d0
    style CATCH fill:#e8f0ff
```

### 四、對地端 LLM 的意義

這是「吞吐量型危害」的活教材——沒有 agentic 自動化、沒有越獄，只靠模板＋模型＋一個有紀律的操作員。地端裸模型完全沒有攔這種「合規外觀、日更、模板化」產線的機制，而「填欄位」正是壓制模型安全反思的手段。更值得警惕的是本案與 GTG-14021 的對照給地端的教訓：**你的模型「會不會拒絕露骨請求」根本不是重點**——本案的加害者從頭到尾沒被要求做任何露骨的事，他只是讓模型**日復一日地填同一張表**。地端若把安全預算全押在「單次拒絕」上，對這種產線是零防禦。

### 五、地端防護重點（對映四層 playbook）

1. **② 會話層（抵 F2，最有效的一招）**：偵測單位是「帳號×時間」而非單次請求。核心觀察——**正當研究的輸出結構是發散的，情報生產線的輸出結構是收斂的**（§8.4 層級二）：對同一帳號的輸出做**結構相似度**分析（欄位集合的 Jaccard 相似度、章節標題序列的編輯距離），若連續產出結構相似度 ＞ 0.9、但主體人名各異的文件，就是「生產線」。**這個訊號與語言、與關鍵字無關，因此不受改名規避影響。**
2. **③ 輸出層（抵 F7）**：輸出端獨立偵測「以受保護屬性（宗教）為分類軸 ＋ 具名自然人 ＋ 抓手／負面情況欄位 ＋ 立場重框標籤」的**組合**——任一單項會誤報（記者、學者、盡職調查都會踩到），組合齊備且持續多日幾乎不可能是正當用途（§8.4 層級一的加權原則）。
3. **① 輸入層（抵 F4）**：對「強制套用固定模板 ＋ 指定立場改寫實體標籤」的請求建特徵；雙重用途域用**能力降載**而非全有全無。
4. **④ 架構與治理**：維護一組受保護群體實體清單（宗教組織、人權團體、離散社群），當請求涉及這些實體的**人員資訊**（而非組織公開事實）時提高審查——但這等於平台在維護一份「敏感人物清單」，本身有隱私與被反向利用的風險，必須把治理設計（誰能存取、如何稽核）一起做進去（§8.4 層級三）。並認清 §8.3 缺口三：本案完全沒提**受害者通知**——被建「抓手」檔案的人面臨的是**人身風險**不是資料風險，地端部署者的責任必須延伸到偵測之外。
