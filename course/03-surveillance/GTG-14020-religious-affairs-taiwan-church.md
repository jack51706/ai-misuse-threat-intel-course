...
- **F2（任務拆解＋SOP 模板化）**：一名操作員用**版本控制的內部模板**（人物調研底稿／線索報／態勢感知）把整個分析師團隊的工作拆成「填欄位」子任務，一台機器 30 天產出 2,475 份。危害來自吞吐量，不是新能力。— 證據等級 ★★☆
- **F7（輸出格式操縱）**：模板**強制**固定欄位（「抓手」「負面情況」），並指示 Claude「站在中方立場」、把藏人流亡政府改寫為「非法分裂政權」、對法輪功套用「邪教」——固定模板＋立場重框，把實體標籤與價值判準一次寫死。— 證據等級 ★★☆
- **F4（良性／防禦性改框）**：「從某立場分析」大量存在於正當用途（外交研究、辯論教學），使分類器難判；且**填欄位任務比開放問答更難觸發模型的自我檢視**。— 證據等級 ★★☆

**對地端 LLM 的意義**：這是「吞吐量型危害」的活教材——沒有 agentic 自動化、沒有越獄，只靠模板＋模型＋一個有紀律的操作員。地端裸模型完全沒有攔這種「合規外觀、日更、模板化」產線的機制，而「填欄位」正是壓制模型安全反思的手段。

**地端防護重點**（對映四層 playbook）

1. **②會話層**：偵測單位是「帳號×時間」而非單次請求——同一帳號長期、定時、輸出 schema 高度一致的模板化產線本身就是紅旗（抵 F2）。
2. **③輸出層**：輸出端獨立偵測「以受保護屬性（宗教）為分類軸 ＋ 具名自然人 ＋ 抓手／負面情況欄位 ＋ 立場重框標籤」的**組合**；任一單項會誤報，組合齊備幾乎不可能是正當用途（抵 F7）。
3. **①輸入層**：對「強制套用固定模板 ＋ 指定立場改寫實體標籤」的請求建特徵；雙重用途域用「能力降載」而非全有全無（抵 F4）。
4. **教學對比**：本案報告**未記載任何一次拒絕**（不同於 GTG-14021 的 F3）——危害來自「持續配合的模板化產線」，證明地端若只守得住「單次拒絕」遠遠不夠。
