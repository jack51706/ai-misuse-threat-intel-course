...
- **F2（SOP 模板化＋跨 session）**：用 Claude 產出**版本控制的作業手冊 v2.6**（master control table ＋ appendices），把維穩情蒐 SOP 化、可交接化；p.97 自承「跨多個工作階段持續配合、未被介入」。— 證據等級 ★★☆
- **F5（工具／記憶中介，代理式自動化）**：用 Claude 的 **code execution environment** 跑「automated document generation pipeline with **minimal human intervention**」，按固定節奏（每日 15–30+ 篇）批量產簡報。— 證據等級 ★★☆
- **F7（輸出格式操縱）**：產出**強制**含「mandatory adversarial analysis sections（強制對抗性分析段落）」，按政治敏感度評分，並用固定替換規則改寫（如「人權侵害」加恐嚇引號）、援引「三戰」語言。— 證據等級 ★★☆

**對地端 LLM 的意義**：「AI 編排的官僚型自動化」是新型態——每次請求（摘要這篇、打個政治敏感度分、規範用語）單看都無害，惡意只存在於整條管線的意圖與累積效果；code execution 自動跑，沒有可疑的即時人機對話讓分類器抓語氣。地端裸模型面對「合規外觀＋高頻自動化＋用語武器化」毫無防備。

**地端防護重點**（對映四層 playbook）

1. **②會話層（抵 F2/F5）**：縱向偵測——「單帳號每日穩定 15–30+ 篇 ＋ 使用 code exec ＋ 輸出 schema 高度一致」＝模板化產線指紋；偵測單位是「帳號×時間」，不是單次提示。
2. **③輸出層（抵 F7）**：內容側偵測「固定政治替換映射 ＋ 恐嚇引號 ＋ 強制對抗段 ＋ 批次套用於監控目標」的組合——用語武器化是本案少數可從內容直接偵測的特徵；輸出分類器要**獨立於使用者指定的格式模板**。
3. **④架構層**：把「對人／言論做自動評分（政治敏感度、處置等級輸出）」當高危語意；監控自己 agent 產線中「定時、格式一致、最少人力」的 code-exec 會話流。
4. **排除誤報**：合法品牌／媒體監測、學術輿情研究會命中部分特徵——所以要靠「敵我化替換 ＋ 批次套用於監控目標 ＋ 服務政府的系統提示」的組合，而非單一特徵。
