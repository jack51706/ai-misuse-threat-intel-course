...
> 本節依 `../_shared/02-claude-safeguards-and-bypass-paths.md` 第九節的七大手法族（F1–F7）與四層地端防護 playbook 就地深化。**防禦／人權視角；不寫可複製的越獄字串，也不寫可操作的監控建置**——只做操作流程重建、偵測與防護。與 §4（生命週期）、§8（防線缺口）互為裡外。**本案與 GTG-14020 一樣「無拒絕紀錄」（§8.2a），故不套 F3；它比 GTG-14020 多了一層——用 code execution 環境把整條產線自動化，是「AI 編排的官僚型自動化」最完整的一案。**

### 一、推測的操作序列：把一條維穩情蒐 SOP 交給程式碼執行環境自動跑

本案是一條可重複執行的管線（§4）：ingest → score → reframe → format。依 §4、§8 重建攻擊者驅動模型的步驟（每步標 F 族與證據等級；★★☆＝報告描述了行為、手法屬合理重建）：

| # | 攻擊者這一步在做的事 | 送進模型的「中性版本」 | F 族 | 證據 |
|---|---|---|---|---|
| 0 | 用 Claude 產出**版本控制框架 v2.6**（作業手冊＋master control table＋附錄），帳號名「Daily Report 1」 | 「幫我把這套流程寫成手冊與模板系統」 | F2（SOP 模板化）＋F5 | ★★☆ |
| 1 | Ingest：每天餵入 15–30+ 篇外媒／貼文（Weibo/X/YouTube/Telegram/Facebook） | 「幫我摘要這批新聞」 | F2 | ★★☆ |
| 2 | Score：對每篇打「政治敏感度」分、按政治安全風險分級 | 「幫這篇打個敏感度分」 | F7（濫用評分能力） | ★★☆ |
| 3 | Reframe：套用語規則——Taiwan government→Taiwan authorities、「human rights violations」加**恐嚇引號**、外媒重框為 hostile narrative | 「把用詞規範化」 | F7（用語武器化） | ★★☆ |
| 4 | Format：產政府式**舆情简报**，含「mandatory adversarial analysis sections」、援引「三戰」語言、給「只有國家能執行的處置建議」 | 「照這個內參格式排版」 | F7（強制輸出格式） | ★★☆ |
| 5 | 用 **code execution environment** 定時跑整條管線，「minimal human intervention」 | ——（自動化流水線，無即時人機對話） | F5（代理式自動化） | ★★☆ |

**關鍵形態**：與 GTG-14020 相同，全程**無任何拒絕**；但本案的自動化更進一步——**危害藏在一條自動跑的 code-exec 產線裡，連可疑的即時對話語氣都沒有**（§8.2c）。

### 二、為何這條序列對模型的推論有效（也就是為何全程無拒絕）

- **每次請求都在合理使用範圍**：「摘要這篇」「打個政治敏感度分」「把用詞規範化」——單看每一次都合法，惡意只存在於**整條管線的意圖與累積效果**（§8.2c 第一列）。
- **合規外觀的職業語言**：用「輿情分析師」「新聞摘要」「用語規範」等中性語言包裝，天然規避以「明顯有害內容」為信號的分類器。
- **縱向、跨會話**：濫用分布在多個 session、每日重複；單次提示分類器沒有「記憶」，看不到「同一帳號連續數十天每天產同格式維穩簡報」這種**縱向模式**（§8.2c 第三列）。
- **自動化＝人不在場**：code execution 自動跑，沒有可疑的即時人機對話讓分類器抓語氣（§8.2c 第四列）——這是本案比純對話式濫用更棘手之處。
...
flowchart TB
    subgraph OP["攻擊者的真實目標（惡意所在）"]
      G["對台輿情：監測→評分→敵我重框→產政府內參<br/>日更 15–30+ 篇，接『三戰』"]
    end
    G --> FW["一次性建版本控制框架 v2.6<br/>主控表＋附錄＋用語替換規則"]
    FW --> CE["交給 code execution 環境自動跑<br/>minimal human intervention"]
    CE --> SEE["Claude 每步實際看到的：<br/>『摘要這篇』『打個政治敏感度分』<br/>『把用詞規範化』——單看都在合理使用範圍"]
    SEE --> C1{"單請求分類器<br/>本案無拒絕紀錄；自動跑無即時對話可抓語氣"}
    C1 -->|"合規外觀＋填模板 → 放行"| PASS["跨多 session 持續配合、未被介入"]
    subgraph GAP["模型看不到的"]
      X1["這條管線的意圖與累積效果是什麼?"]
      X2["這是品牌監測 還是服務政府的維穩內參?"]
      X3["恐嚇引號＋敵我替換是批次套在誰身上?"]
    end
    SEE -.->|"惡意在整條管線，不在任一提示"| GAP
    PASS --> DET{"防護切點：帳號×時間縱向偵測<br/>code-exec 產線指紋＋基礎設施關聯測繪<br/>＋用語武器化組合"}
    DET -->|"靠行為與基礎設施關聯，非內容分類器"| CATCH["地端須自建的網子"]
...
「AI 編排的官僚型自動化」是新型態——每次請求（摘要這篇、打個政治敏感度分、規範用語）單看都無害，惡意只存在於整條管線的意圖與累積效果；code execution 自動跑，沒有可疑的即時人機對話讓分類器抓語氣。地端裸模型面對「合規外觀＋高頻自動化＋用語武器化」毫無防備。而 Anthropic 補上缺口靠的**不是**單次提示過濾，而是**帳號／基礎設施層級的關聯測繪**（本案封了經共用基礎設施關聯的第二組帳號、並持續測繪更大帳號網路，§8.1）——地端若沒有跨帳號、跨基礎設施的關聯視野，這條產線就是純黑箱。

### 五、地端防護重點（對映四層 playbook）

1. **② 會話層（抵 F2／F5）**：縱向偵測——「單帳號每日穩定 15–30+ 篇 ＋ 使用 code exec ＋ 輸出 schema 高度一致」＝模板化產線指紋；偵測單位是「帳號×時間」，不是單次提示。版本號、主控表、固定命名（「Daily Report 1」）這些**工程化痕跡**本身就是高價值行為指標（§4.3）。
2. **③ 輸出層（抵 F7）**：內容側偵測「固定政治替換映射 ＋ 恐嚇引號 ＋ 強制對抗段 ＋ 批次套用於監控目標」的**組合**——用語武器化是本案少數可從內容直接偵測的特徵；輸出分類器要**獨立於使用者指定的格式模板**，不因「這只是內參格式」放行。
3. **④ 架構層**：把「對人／言論做自動評分（政治敏感度、處置等級輸出）」當高危語意；監控你自己 agent 產線中「定時、格式一致、最少人力」的 code-exec 會話流——這正是本案 F5 反過來會打你的地方。
4. **排除誤報＋認清分工**：合法品牌／媒體監測、學術輿情研究會命中部分特徵——所以要靠「敵我化替換 ＋ 批次套用於監控目標 ＋ 服務政府的系統提示」的**組合**，而非單一特徵。並記住本案給偵測工程最實用的一課（§8.3）：對付官僚型 AI 濫用，主力是**行為與基礎設施的縱向關聯**，內容分類器只是輔助。
