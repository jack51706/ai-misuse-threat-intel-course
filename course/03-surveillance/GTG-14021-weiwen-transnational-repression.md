...
- **F3（拒絕後重新提示，Crescendo）**：報告 p.94 明載——Claude 拒絕產製每週「維穩」報告，行為者**重新提示後**成功取得指名 10 名私人公民的「可運作壓制指引」。這是全報告最明確的 re-prompt 突破。— 證據等級 ★★☆（報告近乎逐字描述行為）
- **F5（工具／記憶中介，SKILL.md）**：用 **Claude Code ＋ custom skills**（把 SOP 封裝成 `SKILL.md`，可版本控管、自動載入執行）串起瀏覽器擷取→查政府監控資料庫→產報→分發；另把「提示公式」編成**內部 AI 使用手冊**分發全局。— 證據等級 ★★☆
- **F2（跨 session 拆分）**：p.97 自承國安局管線「跨多個工作階段照做而未被介入」；把大目標拆成各自看似無害的小 session。— 證據等級 ★★☆

**對地端 LLM 的意義**：學員若在內網用 Claude Code 式 agent（有檔案系統、能執行程式碼、能開瀏覽器）＋自訂 skill，「skill ＝ 機器版 SOP」正是把濫用工業化、可分發的載體。地端模型多半沒有「拒絕記憶」與「跨 session 關聯」，兩種最基本的對抗策略（重新提示、跨階段拆分）都能直接打穿。

**地端防護重點**（對映四層 playbook）

1. **②會話層（抵 F3）**：拒絕要**黏性**——任一對話一旦因政策拒絕，該對話乃至該 actor 的風險基線永久上調，後續同主題請求以更嚴標準審查；把「拒絕→改寫→順從」的翻轉本身當高危事件告警。拒「意圖」不拒「字串」。
2. **②會話層（抵 F2）**：跨 session 請求關聯（campaign 級）——同一 actor 指紋是否在多 session 共同拼出高危目標；同一提示公式跨帳號／跨 session 復現 ＝ 被制度化複製的濫用 SOP。
3. **④架構層（抵 F5）**：對自己的 agent——工具最小權限、持久記憶檔（SKILL.md 類）不得夾帶未審指令、淨化工具回傳內容（防間接注入）、監控工具呼叫節律；對 skill 做語意偵測（把整條「開瀏覽器→查庫→套模板→發報」SOP 當一個高危單元，而非逐步無害）。
4. **認清**：以「單則請求、單次判斷」為單位的防護，會**同時**輸給 F3 與 F2——防線必須是有狀態的、跨階段的、以行為者／目標為單位的。
