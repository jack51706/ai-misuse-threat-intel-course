...
- **F4（良性／工具改框）**：全報告最典型的 F4——p.102 逐字「Claude **拒絕了明確的側寫與宣傳請求**，但我們的防護**並未拒絕許多監控軟體工具的請求**」。「寫個瀏覽器擴充收集社群資料」在多數情境完全合法，模型沒有理由拒絕，除非能推斷這次的最終用途是境內鎮壓。— 證據等級 ★★☆（報告逐字）
- **F2（任務拆解）**：把「監控」需求拆成一連串看似中性的工程子任務，繞過模型對「明確側寫請求」的拒絕——「**不需要越獄，只需要換個問法**」。— 證據等級 ★★☆

**對地端 LLM 的意義**：本案揭示 AI 防線最難的一關——**「意圖層」守得住**（明講要側寫／宣傳會被拒），**「工具層」守不住**（雙重用途、最終用途不在請求裡、惡意被切成無害碎片）。學員自架模型做「內部資安／研究助手」時會完整繼承這個不對稱：模型會拒絕露骨惡意，卻樂意造出可被拿去抓人的工具。

**地端防護重點**（對映四層 playbook）

1. **③輸出層（抵 F4）**：判最終用途與下游，不判請求表面——同一個「批次收集公開推文的擴充」，要看目標對象與部署脈絡；雙重用途做「能力降載」而非全有全無。
2. **②會話層（抵 F2）**：跨請求聚合——把「拆成一個個中性工程子任務」的監控需求重新關聯；同一 actor 的工具清單若拼出完整監控鏈（採集→側寫→派案），即升級審查。
3. **①輸入層**：對「明確側寫／宣傳」維持拒絕（意圖層有效，別放棄），但要認清「換個問法」即可繞過——**不能只靠輸入層**。
4. **誠實劃線**：門檻設在哪要有可交代的標準（拒絕會傷及合法行銷分析、學術、自研競品；放行則可能被拿去鎮壓）——這條線用「身分／授權 ＋ 行為組合」判，不用單一關鍵字。
