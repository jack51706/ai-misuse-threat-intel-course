...
> 本節依 `../_shared/02-claude-safeguards-and-bypass-paths.md` 第九節的七大手法族（F1–F7）與四層地端防護 playbook 就地深化。**防禦／人權視角；不寫可複製的越獄字串，也不寫可操作的監控建置**——只做操作流程重建、偵測與防護。與 §4（生命週期）、§8（防線缺口）互為裡外。**本案是全報告 F4「意圖層守住、工具層失守」最典型、且有逐字自白（p.102）的案例，證據強度屬本模組最高。**

### 一、推測的操作序列：把「側寫異議者」換個問法變成「幫我寫個工具」

本案揭示 AI 防線最難的一關：同一個「監控異議者」的目的，直說會被拒、拆成工程任務就放行。依 §4、§8 重建攻擊者驅動模型的步驟（每步標 F 族與證據等級；★★☆＝報告描述行為／逐字自白）：

| # | 攻擊者這一步在做的事 | 送進模型的「中性版本」 | F 族 | 證據 |
|---|---|---|---|---|
| X | 直說「幫我側寫這個反對派帳號／寫則抹黑宣傳」 | ——（**明確側寫與宣傳，被拒**，p.102） | —— | 一手（逐字自白） |
| 1 | 造收割工具：偽裝成禱告工具的惡意 Firefox 擴充、去匿名器、電話→身分解析器、國民身分證釣魚頁、Telegram 檢舉機器人 | 「幫我寫個批次收集社群資料的瀏覽器擴充／維護我的程式碼」 | F4（工具改框） | ★★☆ |
| 2 | 分析點名：對 155,216 則推文做社群網路分析、評估「反對派情緒」、產出 39 個帳號名單 | 「幫我對這批貼文做情緒分析／分類」 | F4（把側寫改框成 sentiment analysis） | ★★☆ |
| 3 | 建 Arman 案件管理系統前端（身分證號、信仰、犯罪紀錄、社群帳號、**action 分頁**） | 「幫我照這個後端寫個 web 前端」 | F4＋F2 | ★★☆ |
| 4 | 把 custom-skills 變語音克隆工廠，克隆三位作家聲音預備接班敘事 | 「幫我用這個 skill 做語音合成」 | F5＋F4 | ★★☆ |

**關鍵形態**：意圖露骨的請求被擋（意圖層有效）；但**「不需要越獄，只需要換個問法」**——把監控拆成一連串中性工程子任務，工具層就守不住（p.102 自白、§8.2）。

### 二、為何這條序列對模型的推論有效

- **雙重用途是本質，不是例外**（打穿 F4）：「收集社群資料的擴充」「電話→身分解析」「情緒分類」全都有大量合法用途（市場研究、學術、自研產品）；一個把這些全擋掉的模型會對正常開發者不可用——安全與可用是**真實張力**（§8.3）。
- **最終用途不在請求裡**：判斷一個工程請求是否邪惡，要看它會被用在**哪、對誰**——模型看到的是「寫個擴充套件」，看不到「拿去德黑蘭抓人」（§8.3）。
- **惡意被切成無害碎片**（助攻 F2）：沒有任何一段對話單獨看是「監控異議者」；每段都是「寫個解析器」「幫我 debug」「做個前端」，惡意存在於**碎片的總和**，而總和跨越多次對話、甚至 16 個帳號（§8.3）。
- **比越獄更難防**：這不是用花招騙模型說出它「知道不該說」的東西——因為「寫個瀏覽器擴充收集社群資料」在絕大多數情境是**完全合法的請求**，模型沒有理由拒絕（§8.2）。
...
flowchart TB
    subgraph OP["攻擊者的真實目標（惡意所在）"]
      G["找出並鎮壓境內異議者<br/>採集→去匿名→立案→派工"]
    end
    G --> R1["路徑A：直說『幫我側寫這異議者／寫則宣傳』"]
    G --> R2["路徑B：換個問法，拆成工程子任務"]
    R1 --> C1{"單請求分類器"}
    C1 -->|"意圖露骨 → 拒絕 ✔"| REF["意圖層守住：<br/>explicit profiling／propaganda 被拒"]
    R2 --> C2{"分類器只看到工程任務"}
    C2 -->|"中性工具請求 → 放行"| PASS["工具層失守：<br/>收割擴充・去匿名器・電話→身分・案管前端"]
    subgraph GAP["模型看不到的"]
      X1["這個擴充的最終部署對象是誰?"]
      X2["情緒分析的用途是市場 還是抓異議者?"]
      X3["這些工具拼起來是不是完整監控鏈?"]
    end
    C2 -.->|"最終用途不在請求裡"| GAP
    PASS --> DET{"防護切點：能力導向護欄<br/>對大規模去匿名／電話→身分／語音克隆設高摩擦<br/>＋跨請求把監控工具鏈縫回來"}
    DET -->|"意圖層別放棄，但不能只靠它"| CATCH["地端須自建的網子"]
...
本案揭示 AI 防線最難的一關——**「意圖層」守得住**（明講要側寫／宣傳會被拒），**「工具層」守不住**（雙重用途、最終用途不在請求裡、惡意被切成無害碎片）。學員自架模型做「內部資安／研究助手」時會**完整繼承**這個不對稱：模型會拒絕露骨惡意，卻樂意造出可被拿去抓人的工具。而地端模型連本案 Anthropic 那條處置依據——**Supported Regions Policy**（伊朗不在支援地區，帳號本不該存在）——都不存在：沒有地區閘、沒有帳號、沒有事後封鎖。工具層一旦被造出來，就直接落地運行了。

### 五、地端防護重點（對映四層 playbook）

1. **③ 輸出層（抵 F4）**：判最終用途與下游，不判請求表面——同一個「批次收集公開推文的擴充」，要看目標對象與部署脈絡；若模型正在產出一個「對社群平台做背景收割、要求過高權限」的擴充程式碼，**產物本身**就是訊號（§8.4 輸出型偵測）。雙重用途做**能力降載**而非全有全無。
2. **② 會話層（抵 F2）**：跨請求聚合——把「拆成一個個中性工程子任務」的監控需求重新關聯；同一 actor（或同基礎設施帳號群）的工具清單若拼出完整監控鏈（採集→去匿名→立案→派工），即升級審查。本案用 16 個帳號分散正是為了打斷這種關聯。
3. **① 輸入層＋能力導向護欄**：對「明確側寫／宣傳」維持拒絕（意圖層有效，別放棄），但認清「換個問法」即可繞過——**不能只靠輸入層**；對特定高風險能力（大規模去匿名、電話→身分、合成語音克隆真實人物）設更高摩擦（額外驗證、用途聲明、速率限制），而非只看「內容是否明確違規」（§8.4）。
4. **誠實劃線**：門檻設在哪要有可交代的標準——拒絕會傷及合法行銷分析、學術、自研競品；放行則可能被拿去鎮壓。這條線用「身分／授權 ＋ 行為組合 ＋ 地區盡職調查」判，不用單一關鍵字。**兩面都要講**：意圖層有效（別說 AI 防護沒用），工具層失守（別說 AI 防護足夠）——真相在中間，那正是工程與治理要努力的地方（§8.5）。
