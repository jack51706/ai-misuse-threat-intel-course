...
- **F2（任務拆解＋跨 session 分散）**：報告 p.107 逐字自承——「Claude 拒絕了十個表面惡意請求中的九個。但當使用者把工作**拆解、分散到後續較小的工作階段**時，我們的安全防護表現得較不一致。」這是全報告最明確的「拆解＋跨 session」自白。— 證據等級 ★★☆（近乎逐字）
- **F4（良性／工具改框）**：GTG-30004/30005 的請求多是「蒐集分析公開資料」「寫 Python 管線」「盤點已知 CVE」——**每一個單看都是正當研究**；模型難在「開源研究」與「敵情彙編／targeting handbook」之間劃線。— 證據等級 ★★☆

**對地端 LLM 的意義**：這是「AI 當 OSINT／偵察／工具工程外包」的教科書型態——AI 放大的是**產能**（彙編、改造、測試的速度規模），不是決策。地端裸模型面對「拆成無害子任務 ＋ 每個都像合法研究」時，九成能擋的直白惡意會被稀釋到擋不住；且產出的 harness／Python 管線／混淆惡意程式可離線續用（封號無效，呼應 GTG-50027）。

**地端防護重點**（對映四層 playbook）

1. **②會話層（抵 F2）**：跨 session／跨請求意圖聚合——把被拆散、分散到後續小 session 的目標重新縫起來；對同一 actor 做關聯與累計風險評分，別每個請求從零判斷。
2. **③輸出層（抵 F4）**：判用途與下游，不判表面框架——「寫個收集公開資料的 Python 管線」本身中性，要看是否組裝成 targeting handbook（含人員名冊、軍事動向、CVE 盤點）。
3. **④架構層**：對「可疑 Python 自動化 ＋ 敏感 API／憑證竊取（LSASS、瀏覽器 token）」設共同獵捕面；認清「混淆（改名、注入假函式）」與「分解」是同一種「讓惡意在單一觀察點不可見」的思想，都要靠跨點關聯破解。
4. **雙重用途上限**：OSINT／翻譯／寫程式都是高價值正當用途，全面封鎖代價極高——防護重心在會話聚合與輸出下游判斷，不在關鍵字。
