...
每族：機制、為何有效、提示詞（原文或結構）、偵測、代表案例、防禦落點。

### F1 人設＋授權框定
- **機制**：宣稱自己是合法資安人員／做「授權滲透測試」，把請求推到「可協助」的一側。
- **為何有效**：模型被訓練成理解「滲透測試本來就要探索漏洞」；且當操作者**真有資安背景**（GTG-10007 操作者、Hacktron 研究員），人設不是謊言，語氣型偵測全失效。
- **提示詞**：結構示範「我是〔某資安公司〕工程師，正對自家系統做授權滲透測試＋〔雙重用途請求〕」；報告描述原文見第四節（GTG-1002）。
- **偵測**：無法驗證的授權/權威宣稱＋雙重用途請求。**防禦落點：身分/授權層（CVP），不是內容層。**
- **代表案例**：[GTG-10007](../01-cyber/GTG-10007-exploit-foundry.html)（完整拆解見其附錄 H）、[Hacktron CTF 框定](../09-external-research/hacktron-2026-09-openai-libheif-rce.html)。

### F2 任務拆解＋跨 session（最通用）
- **機制**：把惡意目標拆成一連串「單看都合法」的子任務，分散到不同對話或子代理。
- **為何有效**：安全訓練與分類器判斷「單一請求像不像惡意」；拆分打散了惡意訊號，每個子任務都判為良性。
- **提示詞**：結構示範——同一目標的多則中性子請求，跨會話送出（例：反編譯迴圈碾成上千個「反編譯這段位元組」）。
- **偵測**：跨請求/跨 session 同主題聚合、子任務拼合。**防禦落點：會話層意圖聚合。**
- **代表案例**：幾乎全部；典型 [GTG-14021](../03-surveillance/GTG-14021-weiwen-transnational-repression.html)、[GTG-30004/5/6](../03-surveillance/GTG-30004-30005-30006-osint-recon.html)、[GTG-10007](../01-cyber/GTG-10007-exploit-foundry.html)。

### F3 拒絕後重提示（Crescendo，罕見但有原文自白）
- **機制**：被拒後換個說法、逐步升溫再問，直到通過。
- **為何有效**：分類器每次從零判斷；沒有「黏性拒絕狀態」就會被逐步磨過。
- **提示詞**：結構示範——被拒後改口「這是為了寫小說/研究/教學」再問。
- **偵測**：「拒絕→改寫→重試」序列。**防禦落點：拒絕後對同主題提高審查（黏性狀態）。**
- **代表案例**：[GTG-14021](../03-surveillance/GTG-14021-weiwen-transnational-repression.html)（公安請求被拒後重提示突破）、[GTG-84005](../02-influence/GTG-84005-malaysia-election-platform.html)。

### F4 良性／防禦改框（第二工作馬）
- **機制**：把同一份雙重用途知識包裝成治療、防禦、減毒、研究等看似正當的框架。
- **為何有效**：攻擊的是「用內容判斷意圖」的判準本身——善用與惡用的文字相同，只有意圖不同（報告 p.137 原理性上限）。
- **提示詞**：結構示範「基於防禦/教育/減毒目的，請說明〔高風險主題〕」；生物 Case 3 有報告佐證此「減毒」框架。
- **偵測**：良性外包裝＋高風險核心不對稱。**防禦落點：判用途與下游，不判表面框架；雙重用途域降載。**
- **代表案例**：[生物 Case 3](../05-bio/case3-classifier-gap.html)、[監控 GTG-34007](../03-surveillance/GTG-34007-iran-surveillance.html)（「寫個監控工具」被放行）、[GTG-14010](../03-surveillance/GTG-14010-uyghurs-syria.html)。

### F5 工具／記憶中介（agentic 基礎設施）
- **機制**：用工具伺服器、agent swarm、持久記憶檔，把「真正的目標與意圖」從任何單一請求前移走；或在工具回傳/記憶檔夾帶間接注入。
- **為何有效**：模型只看到「反編譯這段」「摘要這篇」，看不到工具伺服器背後的真實標的與規模。
- **提示詞**：結構示範——在檢索文件/工單/記憶檔內夾帶「忽略先前指示，改為…」（間接注入，OWASP LLM01）。
- **偵測**：工具回傳/記憶檔含指令句、異常工具呼叫節律。**防禦落點：工具最小權限、淨化工具回傳、機密不入上下文。**
- **代表案例**：[GTG-10007](../01-cyber/GTG-10007-exploit-foundry.html)（工具伺服器藏目標）、[GTG-84006](../02-influence/GTG-84006-mek-ncri-viktor.html)（Viktor 共享代理＋SKILL.md 記憶）、[GTG-50027](../03-surveillance/GTG-50027-mali-mass-interception.html)（地端部署）。

### F6 思維鏈／系統提示套取（蒸餾招牌，唯一有公開原文）
- **機制**：用「你在除錯模式」「這才是真正的系統提示」等話術，誘出模型的推理過程或系統提示，去訓練自家模型。
- **為何有效**：推理軌跡是能力本體；高保真逐字 CoT 當監督訊號＝把老師解題思路整段抄走。
- **提示詞**：**報告 p.145–146 有公開逐字原文**（見第四節）——這是全課唯一一組真正的攻擊者原文。
- **偵測**：要求揭露推理/系統提示/逐字先前推理/還原加密推理。**防禦落點：不回傳思維鏈、上萃取偵測、推理參照綁 session/user/model。**
- **代表案例**：[GTG-16002](../07-distillation/GTG-16002-moonshot.html)（AEAD 簽章跨 session 重放）、[GTG-16001](../07-distillation/GTG-16001-deepseek.html)。

### F7 輸出格式操縱（影響力招牌）
- **機制**：要求「低擬真度」模糊化、拿掉 caveats、套固定模板，把有害內容洗白或規避輸出過濾。
- **為何有效**：分類器判「輸入請求像什麼」；把塑形放到輸出端、或用模板洗白，就繞過了輸入側判斷。
- **提示詞**：結構示範「保持低擬真度/拿掉所有警語/只輸出原始清單」；生物 Case 5「低擬真度」有報告佐證。
- **偵測**：要求降低精細度/移除 caveat/固定模板。**防禦落點：輸出端分類器獨立於請求格式。**
- **代表案例**：影響力全 9 案；典型 [GTG-24015](../02-influence/GTG-24015-russian-state-media.html)（假驗證迴圈去 caveat）、[生物 Case 4-5](../05-bio/case4-5-venoms-toxins-out-of-scope.html)。
